...
import spectra.dynamic_scene.contracts;
...
namespace spectra::dynamic_scene {
        ...
        [[nodiscard]] scene::Transform make_transform(const SpectraTransform& transform, const std::string_view context) {
        ...
        [[nodiscard]] scene::Scene::SceneEntityRef make_entity_ref(const SpectraEntityRef& entity, const std::string_view context) {
        ...
        [[nodiscard]] scene::Scene::PreviewMaterial make_material(const SpectraMaterial& material) {
        ...
        [[nodiscard]] scene::Scene::PreviewLight make_light(const SpectraLight& light) {
        ...
        [[nodiscard]] scene::CameraProjection camera_projection(const SpectraCamera& camera, const std::string& name) {
        ...
        [[nodiscard]] scene::Scene::Camera make_camera(const SpectraCamera& camera) {
        ...
        [[nodiscard]] scene::Scene::Mesh make_mesh(const SpectraMesh& mesh, const bool dynamic) {
            ...
            const std::span<const SpectraMeshVertex> vertices = abi_span(mesh.vertices.data, mesh.vertices.count, std::format("Dynamic scene mesh \"{}\" vertices", name));
        ...
        [[nodiscard]] scene::Scene::Sphere make_sphere(const SpectraSphere& sphere, const bool dynamic) {
        ...
        [[nodiscard]] scene::Scene::PointCloud make_point_cloud(const SpectraPointCloud& point_cloud, const bool dynamic) {
            ...
            const std::span<const SpectraPoint> points = abi_span(point_cloud.points.data, point_cloud.points.count, std::format("Dynamic scene point cloud \"{}\" points", name));
        ...
        [[nodiscard]] scene::Scene::VolumeGrid make_volume(const SpectraVolume& volume, const bool dynamic) {
            ...
            const std::span<const SpectraVolumeChannel> channels = abi_span(volume.channels.data, volume.channels.count, std::format("Dynamic scene volume \"{}\" channels", name));
            for (const SpectraVolumeChannel& channel : channels) {
        ...
        [[nodiscard]] scene::Scene::ViewportCameraVisualImage make_viewport_camera_visual_image(const SpectraViewportCameraVisualImage& image, const std::string& name) {
        ...
        [[nodiscard]] scene::Scene::ViewportCameraVisual make_viewport_camera_visual(const SpectraViewportCameraVisual& visual, const bool dynamic) {
        ...
        [[nodiscard]] scene::Scene::ViewportSegmentSet make_viewport_segment_set(const SpectraViewportSegmentSet& segment_set, const bool dynamic) {
            ...
            const std::span<const SpectraViewportSegment> segments = abi_span(segment_set.segments, std::format("Dynamic scene viewport segment set \"{}\" segments", name));
            ...
            const std::span<const SpectraColor> colors = abi_span(segment_set.colors, std::format("Dynamic scene viewport segment set \"{}\" colors", name));
        ...
        [[nodiscard]] scene::Scene::ViewportVoxelGrid make_viewport_voxel_grid(const SpectraViewportVoxelGrid& voxel_grid, const bool dynamic) {
        ...
        void append_debug_attachments(scene::Scene::DebugAttachmentSet& attachments, const SpectraSceneItems& items, const bool dynamic, const std::string_view context) {
            for (const SpectraViewportSegmentSet& segment_set_view : abi_span(items.viewport_segment_sets, std::format("{} viewport segment sets", context)))
                attachments.viewport_segment_sets.push_back(make_viewport_segment_set(segment_set_view, dynamic));
            for (const SpectraViewportVoxelGrid& voxel_grid_view : abi_span(items.viewport_voxel_grids, std::format("{} viewport voxel grids", context)))
                attachments.viewport_voxel_grids.push_back(make_viewport_voxel_grid(voxel_grid_view, dynamic));
            for (const SpectraViewportCameraVisual& visual_view : abi_span(items.viewport_camera_visuals, std::format("{} viewport camera visuals", context)))
                attachments.viewport_camera_visuals.push_back(make_viewport_camera_visual(visual_view, dynamic));
        }

        void append_document_view(scene::Scene::Document& document, const SpectraDocumentView& view, std::set<std::string>& material_names, std::set<std::string>& light_names) {
            if (view.struct_size != sizeof(SpectraDocumentView)) throw std::runtime_error("Dynamic scene document view ABI size mismatch");
            ...
            for (const SpectraMaterial& material_view : abi_span(view.items.materials, "Dynamic scene document materials")) {
            ...
            for (const SpectraLight& light_view : abi_span(view.items.lights, "Dynamic scene document lights")) {
            ...
            for (const SpectraCamera& camera_view : abi_span(view.items.cameras, "Dynamic scene document cameras")) document.cameras.push_back(make_camera(camera_view));
            for (const SpectraMesh& mesh_view : abi_span(view.items.meshes, "Dynamic scene document meshes")) document.meshes.push_back(make_mesh(mesh_view, false));
            for (const SpectraSphere& sphere_view : abi_span(view.items.spheres, "Dynamic scene document spheres")) document.spheres.push_back(make_sphere(sphere_view, false));
            for (const SpectraPointCloud& point_cloud_view : abi_span(view.items.point_clouds, "Dynamic scene document point clouds")) document.point_clouds.push_back(make_point_cloud(point_cloud_view, false));
            for (const SpectraVolume& volume_view : abi_span(view.items.volumes, "Dynamic scene document volumes")) document.volumes.push_back(make_volume(volume_view, false));
            append_debug_attachments(document.debug_attachments, view.items, false, "Dynamic scene document debug attachments");
        ...
        [[nodiscard]] scene::Scene::FrameSnapshot make_frame_snapshot(const SpectraFrameView& view, const scene::Scene::FrameInfo& frame, const std::set<std::string>& material_names) {
            if (view.struct_size != sizeof(SpectraFrameView)) throw std::runtime_error("Dynamic scene frame view ABI size mismatch");
            scene::Scene::FrameSnapshot snapshot{.cursor = scene::Scene::make_frame_cursor(frame)};
            for (const SpectraCamera& camera_view : abi_span(view.items.cameras, "Dynamic scene frame cameras")) snapshot.cameras.push_back(make_camera(camera_view));
            for (const SpectraMesh& mesh_view : abi_span(view.items.meshes, "Dynamic scene frame meshes")) {
            ...
            for (const SpectraSphere& sphere_view : abi_span(view.items.spheres, "Dynamic scene frame spheres")) {
            ...
            for (const SpectraPointCloud& point_cloud_view : abi_span(view.items.point_clouds, "Dynamic scene frame point clouds")) {
            ...
            for (const SpectraVolume& volume_view : abi_span(view.items.volumes, "Dynamic scene frame volumes")) {
            ...
            append_debug_attachments(snapshot.debug_attachments, view.items, true, "Dynamic scene frame debug attachments");
            return snapshot;
        }
        [[nodiscard]] OptionKind make_open_option_kind(const std::uint32_t kind, const std::string_view context) {
            switch (kind) {
                case SPECTRA_DYNAMIC_SCENE_OPTION_TEXT: return OptionKind::Text;
                case SPECTRA_DYNAMIC_SCENE_OPTION_DIRECTORY_PATH: return OptionKind::DirectoryPath;
                case SPECTRA_DYNAMIC_SCENE_OPTION_FILE_PATH: return OptionKind::FilePath;
                case SPECTRA_DYNAMIC_SCENE_OPTION_CHOICE: return OptionKind::Choice;
                case SPECTRA_DYNAMIC_SCENE_OPTION_BOOL: return OptionKind::Bool;
                case SPECTRA_DYNAMIC_SCENE_OPTION_FLOAT: return OptionKind::Float;
                case SPECTRA_DYNAMIC_SCENE_OPTION_UNSIGNED_INTEGER: return OptionKind::UnsignedInteger;
        ...
        [[nodiscard]] bool parse_bool_default(const std::string_view value) {
            ...
            throw std::runtime_error("Dynamic scene option bool value must be true or false");
        }
        ...
        [[nodiscard]] OptionSchema make_open_option_schema(const SpectraOptionSchema& schema, const std::string_view context) {
            OptionSchema converted{
            ...
            const std::span<const SpectraOptionChoice> choices = abi_span(schema.choices.data, schema.choices.count, std::format("{} choices", context));
            if (converted.kind == OptionKind::Choice && choices.empty()) throw std::runtime_error(std::format("{} choice option must provide at least one choice", context));
            if (converted.kind != OptionKind::Choice && !choices.empty()) throw std::runtime_error(std::format("{} non-choice option must not provide choices", context));
            ...
            for (std::size_t choice_index = 0u; choice_index < choices.size(); ++choice_index) {
                const SpectraOptionChoice& choice = choices[choice_index];
                OptionChoice converted_choice{
            ...
            if (converted.kind == OptionKind::Choice && !converted.default_value.empty() && !choice_values.contains(converted.default_value)) throw std::runtime_error(std::format("{} default value '{}' is not one of its choices", context, converted.default_value));
            if (converted.kind == OptionKind::Bool && !converted.default_value.empty()) static_cast<void>(parse_bool_default(converted.default_value));
            if (converted.kind == OptionKind::Float && !converted.default_value.empty()) static_cast<void>(parse_float_default(converted.default_value, std::format("{} default value", context)));
            if (converted.kind == OptionKind::UnsignedInteger && !converted.default_value.empty()) static_cast<void>(parse_unsigned_integer_default(converted.default_value, std::format("{} default value", context)));
            return converted;
        }

        [[nodiscard]] std::vector<OptionSchema> make_open_option_schemas(const SpectraOptionSchemaSpan schemas, const std::string_view context) {
            const std::span<const SpectraOptionSchema> schema_span = abi_span(schemas.data, schemas.count, context);
            std::set<std::string> schema_keys{};
            std::vector<OptionSchema> converted{};
            converted.reserve(schema_span.size());
            for (std::size_t schema_index = 0u; schema_index < schema_span.size(); ++schema_index) {
                OptionSchema schema = make_open_option_schema(schema_span[schema_index], std::format("{} {}", context, schema_index));
        ...
        [[nodiscard]] ControlAction make_control_action(const SpectraControlAction& action, const std::string_view context) {
        ...
        [[nodiscard]] std::vector<ControlAction> make_control_actions(const SpectraControlActionSpan actions, const std::string_view context) {
            const std::span<const SpectraControlAction> action_span = abi_span(actions.data, actions.count, context);
        ...
        [[nodiscard]] bool control_setting_kind_supported(const OptionKind kind) {
            return kind == OptionKind::Choice || kind == OptionKind::Bool || kind == OptionKind::Float || kind == OptionKind::UnsignedInteger;
        }

        void validate_control_setting_schema(const OptionSchema& setting, const std::string_view context) {
        ...
        void validate_control_setting_value(const OptionSchema& schema, const std::string& value, const std::string_view context) {
            switch (schema.kind) {
                case OptionKind::Choice:
                    if (!std::ranges::any_of(schema.choices, [&value](const OptionChoice& choice) { return choice.value == value; })) throw std::runtime_error(std::format("{} setting '{}' value '{}' is not one of its choices", context, schema.key, value));
                    return;
                ...
                case OptionKind::Float:
                    ...
                    return;
                case OptionKind::UnsignedInteger:
        ...
        [[nodiscard]] std::vector<OptionSchema> make_control_setting_schemas(const SpectraOptionSchemaSpan schemas, const std::string_view context) {
            std::vector<OptionSchema> converted = make_open_option_schemas(schemas, context);
            for (const OptionSchema& setting : converted) {
        ...
        [[nodiscard]] std::vector<ControlSettingValue> make_control_setting_values(const std::span<const SpectraControlSettingValue> setting_span, const std::span<const OptionSchema> schemas, const std::string_view context) {
            std::set<std::string> setting_keys{};
            std::map<std::string, const OptionSchema*> schema_by_key{};
            for (const OptionSchema& schema : schemas) schema_by_key.emplace(schema.key, &schema);
        ...
        [[nodiscard]] ControlStatus make_control_status(const SpectraControlStatusView& view, const std::string_view context) {
            if (view.struct_size != sizeof(SpectraControlStatusView)) throw std::runtime_error(std::format("{} ABI size mismatch", context));
            ...
            const std::span<const SpectraControlMetric> metrics = abi_span(view.metrics.data, view.metrics.count, std::format("{} metrics", context));
            ...
            for (std::size_t metric_index = 0u; metric_index < metrics.size(); ++metric_index) {
                const SpectraControlMetric& metric = metrics[metric_index];
            ...
            const std::span<const SpectraControlActionState> action_states = abi_span(view.action_states.data, view.action_states.count, std::format("{} action states", context));
            std::set<std::string> action_ids{};
            ...
            for (std::size_t action_index = 0u; action_index < action_states.size(); ++action_index) {
                const SpectraControlActionState& action = action_states[action_index];
        ...
        [[nodiscard]] std::vector<ControlLogEntry> make_control_logs(const std::span<const SpectraControlLogEntry> entries, const std::string_view context) {
            ...
            for (std::size_t entry_index = 0u; entry_index < entries.size(); ++entry_index) {
                const SpectraControlLogEntry& entry = entries[entry_index];
        ...
        [[nodiscard]] std::vector<ControlImage> make_control_images(const std::span<const SpectraControlImage> images, const std::string_view context) {
            ...
            for (std::size_t image_index = 0u; image_index < images.size(); ++image_index) {
                const SpectraControlImage& image = images[image_index];
        ...
        [[nodiscard]] ControlScalarSample make_control_scalar_sample(const SpectraControlScalarSample& sample, const std::string_view context) {
        ...
        [[nodiscard]] std::vector<ControlScalarSeries> make_control_scalar_series(const std::span<const SpectraControlScalarSeries> series_span, const std::string_view context) {
            ...
            for (std::size_t series_index = 0u; series_index < series_span.size(); ++series_index) {
                const SpectraControlScalarSeries& series = series_span[series_index];
                ...
                const std::span<const SpectraControlScalarSample> samples = abi_span(series.samples, std::format("{} series '{}' samples", context, converted_series.id));
        ...
        [[nodiscard]] ControlSnapshot make_control_snapshot(const SpectraControlSnapshotView& view, const std::span<const ControlAction> actions, const std::span<const OptionSchema> setting_schemas, const std::string_view context) {
            if (view.struct_size != sizeof(SpectraControlSnapshotView)) throw std::runtime_error(std::format("{} ABI size mismatch", context));
