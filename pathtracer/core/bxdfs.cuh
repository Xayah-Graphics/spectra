#ifndef SPECTRA_PATHTRACER_CORE_BXDFS_H
#define SPECTRA_PATHTRACER_CORE_BXDFS_H
...
namespace spectra {
    ...
    public:
        ...
        DiffuseBxDF() = default;
        __host__ __device__ DiffuseBxDF(SampledSpectrum R) : R(R) {}
        ...
        __host__ __device__ pstd::optional<BSDFSample> Sample_f(Vector3f wo, Float uc, Point2f u, TransportMode mode, BxDFReflTransFlags sampleFlags = BxDFReflTransFlags::All) const {
            ...
            return BSDFSample(R * InvPi, wi, pdf, DiffuseReflection);
        }
        ...
        __host__ __device__ BxDFFlags Flags() const {
            return R ? DiffuseReflection : Unset;
        }
    ...
    public:
        ...
        __host__ __device__ DiffuseTransmissionBxDF(SampledSpectrum R, SampledSpectrum T) : R(R), T(T) {}
        ...
        __host__ __device__ pstd::optional<BSDFSample> Sample_f(Vector3f wo, Float uc, Point2f u, TransportMode mode, BxDFReflTransFlags sampleFlags = BxDFReflTransFlags::All) const {
            ...
            if (uc < pr / (pr + pt)) {
                ...
                return BSDFSample(f(wo, wi, mode), wi, pdf, DiffuseReflection);
            } else {
                // Sample diffuse BSDF transmission
                Vector3f wi = SampleCosineHemisphere(u);
                if (wo.z > 0) wi.z *= -1;
                Float pdf = CosineHemispherePDF(AbsCosTheta(wi)) * pt / (pr + pt);
                return BSDFSample(f(wo, wi, mode), wi, pdf, DiffuseTransmission);
            }
        }
        ...
        __host__ __device__ static constexpr const char* Name() {
            return "DiffuseTransmissionBxDF";
        }
        ...
        __host__ __device__ BxDFFlags Flags() const {
            return ((R ? DiffuseReflection : Unset) | (T ? DiffuseTransmission : Unset));
        }
    ...
    public:
        ...
        __host__ __device__ DielectricBxDF(Float eta, TrowbridgeReitzDistribution mfDistrib) : eta(eta), mfDistrib(mfDistrib) {}

        __host__ __device__ BxDFFlags Flags() const {
            BxDFFlags flags = (eta == 1) ? Transmission : (Reflection | Transmission);
            return flags | (mfDistrib.EffectivelySmooth() ? Specular : Glossy);
        }
    ...
    public:
        ...
        __host__ __device__ ThinDielectricBxDF(Float eta) : eta(eta) {}

        __host__ __device__ SampledSpectrum f(Vector3f wo, Vector3f wi, TransportMode mode) const {
            return SampledSpectrum(0);
        }

        __host__ __device__ pstd::optional<BSDFSample> Sample_f(Vector3f wo, Float uc, Point2f u, TransportMode mode, BxDFReflTransFlags sampleFlags) const {
        ...
        __host__ __device__ Float PDF(Vector3f wo, Vector3f wi, TransportMode mode, BxDFReflTransFlags sampleFlags) const {
            return 0;
        }

        __host__ __device__ static constexpr const char* Name() {
            return "ThinDielectricBxDF";
        }
    ...
    public:
        ...
        __host__ __device__ ConductorBxDF(const TrowbridgeReitzDistribution& mfDistrib, SampledSpectrum eta, SampledSpectrum k) : mfDistrib(mfDistrib), eta(eta), k(k) {}

        __host__ __device__ BxDFFlags Flags() const {
            return mfDistrib.EffectivelySmooth() ? SpecularReflection : GlossyReflection;
        }
        ...
        __host__ __device__ pstd::optional<BSDFSample> Sample_f(Vector3f wo, Float uc, Point2f u, TransportMode mode, BxDFReflTransFlags sampleFlags = BxDFReflTransFlags::All) const {
            ...
            if (mfDistrib.EffectivelySmooth()) {
                ...
                return BSDFSample(f, wi, 1, SpecularReflection);
            }
    ...
    public:
        ...
        __host__ __device__ TopOrBottomBxDF& operator=(const TopBxDF* t) {
            top    = t;
            bottom = nullptr;
            return *this;
        }

        __host__ __device__ TopOrBottomBxDF& operator=(const BottomBxDF* b) {
            bottom = b;
            top    = nullptr;
            return *this;
        }
        ...
        __host__ __device__ pstd::optional<BSDFSample> Sample_f(Vector3f wo, Float uc, Point2f u, TransportMode mode, BxDFReflTransFlags sampleFlags = BxDFReflTransFlags::All) const {
            return top ? top->Sample_f(wo, uc, u, mode, sampleFlags) : bottom->Sample_f(wo, uc, u, mode, sampleFlags);
        }
    ...
    public:
        ...
        LayeredBxDF() = default;
        __host__ __device__ LayeredBxDF(TopBxDF top, BottomBxDF bottom, Float thickness, const SampledSpectrum& albedo, Float g, int maxDepth, int nSamples) : top(top), bottom(bottom), thickness(std::max(thickness, std::numeric_limits<Float>::min())), g(g), albedo(albedo), maxDepth(maxDepth), nSamples(nSamples) {}
        ...
        __host__ __device__ BxDFFlags Flags() const {
            ...
            BxDFFlags flags = Reflection;
            if (IsSpecular(topFlags)) flags = flags | Specular;
            ...
            if (IsTransmissive(topFlags) && IsTransmissive(bottomFlags)) flags = flags | Transmission;
                ...
                for (int depth = 0; depth < maxDepth; ++depth) {
                    ...
                    if (!albedo) {
                        ...
                        if (0 < zp && zp < thickness) {
                            ...
                            Float wt = 1;
                            if (!IsSpecular(exitInterface.Flags())) wt = PowerHeuristic(1, wis->pdf, 1, phase.PDF(-w, -wis->wi));
                            ...
                            if (!ps || ps->pdf == 0 || ps->wi.z == 0) continue;
                    ...
                    if (z == exitZ) {
                        ...
                        if (!IsSpecular(nonExitInterface.Flags())) {
                            ...
                            Float wt = 1;
                            if (!IsSpecular(exitInterface.Flags())) wt = PowerHeuristic(1, wis->pdf, 1, nonExitInterface.PDF(-w, -wis->wi, mode));
            ...
            for (int depth = 0; depth < maxDepth; ++depth) {
                ...
                if (albedo) {
                    ...
                    if (0 < zp && zp < thickness) {
                        ...
                        if (!ps || ps->pdf == 0 || ps->wi.z == 0) return {};
                        f *= albedo * ps->p;
                        pdf *= ps->pdf;
                        specularPath = false;
                        w            = ps->wi;
...
#undef interface
                ...
                if (bs->IsTransmission()) {
                    BxDFFlags flags = SameHemisphere(wo, w) ? Reflection : Transmission;
                    flags |= specularPath ? Specular : Glossy;
                    if (flipWi) w = -w;
                ...
                if (SameHemisphere(wo, wi)) {
                    ...
                    if (wos && wos->f && wos->pdf > 0 && wis && wis->f && wis->pdf > 0) {
                        if (!IsNonSpecular(tInterface.Flags()))
                            ...
                            pstd::optional<BSDFSample> rs = rInterface.Sample_f(-wos->wi, r(), {r(), r()}, mode);
                            if (rs && rs->f && rs->pdf > 0) {
                    ...
                    if (IsSpecular(toInterface.Flags()))
                        ...
                        pdfSum += (toInterface.PDF(wo, -wis->wi, mode) + tiInterface.PDF(-wos->wi, wi, mode)) / 2;
                }
            }
    ...
    private:
        ...
        __host__ __device__ static Float Tr(Float dz, Vector3f w) {
    ...
    public:
        ...
        HairBxDF() = default;
        __host__ __device__ HairBxDF(Float h, Float eta, const SampledSpectrum& sigma_a, Float beta_m, Float beta_n, Float alpha);
        __host__ __device__ SampledSpectrum f(Vector3f wo, Vector3f wi, TransportMode mode) const;
        __host__ __device__ pstd::optional<BSDFSample> Sample_f(Vector3f wo, Float uc, Point2f u, TransportMode mode, BxDFReflTransFlags sampleFlags) const;
        __host__ __device__ Float PDF(Vector3f wo, Vector3f wi, TransportMode mode, BxDFReflTransFlags sampleFlags) const;
        ...
        __host__ __device__ static RGBUnboundedSpectrum SigmaAFromConcentration(Float ce, Float cp);
        __host__ __device__ static SampledSpectrum SigmaAFromReflectance(const SampledSpectrum& c, Float beta_n, const SampledWavelengths& lambda);
    ...
    private:
        ...
        __host__ __device__ static pstd::array<SampledSpectrum, pMax + 1> Ap(Float cosTheta_o, Float eta, Float h, SampledSpectrum T) {
        ...
        __host__ __device__ static inline Float Phi(int p, Float gamma_o, Float gamma_t) {
            return 2 * p * gamma_t - 2 * gamma_o + p * Pi;
        }

        __host__ __device__ static inline Float Np(Float phi, int p, Float s, Float gamma_o, Float gamma_t) {
        ...
        __host__ __device__ pstd::array<Float, pMax + 1> ApPDF(Float cosTheta_o) const;
    ...
    private:
        ...
        __host__ __device__ static Float theta2u(Float theta) {
            return std::sqrt(theta * (2 / Pi));
        }

        __host__ __device__ static Float phi2u(Float phi) {
            return phi * (1 / (2 * Pi)) + .5f;
        }

        __host__ __device__ static Float u2theta(Float u) {
            return Sqr(u) * (Pi / 2.f);
        }

        __host__ __device__ static Float u2phi(Float u) {
            return (2.f * u - 1.f) * Pi;
        }
    ...
    public:
        ...
        __host__ __device__ NormalizedFresnelBxDF(Float eta) : eta(eta) {}

        __host__ __device__ BSDFSample Sample_f(Vector3f wo, Float uc, Point2f u, TransportMode mode, BxDFReflTransFlags sampleFlags) const {
            if (!(sampleFlags & BxDFReflTransFlags::Reflection)) return {};

            // Cosine-sample the hemisphere, flipping the direction if necessary
            Vector3f wi = SampleCosineHemisphere(u);
            if (wo.z < 0) wi.z *= -1;
            return BSDFSample(f(wo, wi, mode), wi, PDF(wo, wi, mode, sampleFlags), DiffuseReflection);
        }
        ...
        __host__ __device__ BxDFFlags Flags() const {
            return BxDFFlags(Reflection | Diffuse);
        }
        ...
        __host__ __device__ SampledSpectrum f(Vector3f wo, Vector3f wi, TransportMode mode) const {
            ...
            if (mode == TransportMode::Radiance) f *= Sqr(eta);
    ...
    __host__ __device__ inline SampledSpectrum BxDF::f(Vector3f wo, Vector3f wi, TransportMode mode) const {
        auto f = [&](auto ptr) -> SampledSpectrum { return ptr->f(wo, wi, mode); };
        return Dispatch(f);
    }

    __host__ __device__ inline pstd::optional<BSDFSample> BxDF::Sample_f(Vector3f wo, Float uc, Point2f u, TransportMode mode, BxDFReflTransFlags sampleFlags) const {
        auto sample_f = [&](auto ptr) -> pstd::optional<BSDFSample> { return ptr->Sample_f(wo, uc, u, mode, sampleFlags); };
        return Dispatch(sample_f);
    }

    __host__ __device__ inline Float BxDF::PDF(Vector3f wo, Vector3f wi, TransportMode mode, BxDFReflTransFlags sampleFlags) const {
        auto pdf = [&](auto ptr) { return ptr->PDF(wo, wi, mode, sampleFlags); };
        return Dispatch(pdf);
    }

    __host__ __device__ inline BxDFFlags BxDF::Flags() const {
        auto flags = [&](auto ptr) { return ptr->Flags(); };
        return Dispatch(flags);
    }

    __host__ __device__ inline void BxDF::Regularize() {
...
#endif // SPECTRA_PATHTRACER_CORE_BXDFS_H
