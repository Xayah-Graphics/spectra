...
namespace spectra {
    ...
    public:
        ...
        __host__ __device__ Point3f CameraFromRender(Point3f p, Float time) const {
            return renderFromCamera.ApplyInverse(p, time);
        }

        __host__ __device__ Point3f RenderFromWorld(Point3f p) const {
            return worldFromRender.ApplyInverse(p);
        }

        __host__ __device__ Transform RenderFromWorld() const {
            return Inverse(worldFromRender);
        }

        __host__ __device__ Transform CameraFromRender(Float time) const {
            return Inverse(renderFromCamera.Interpolate(time));
        }

        __host__ __device__ Transform CameraFromWorld(Float time) const {
            return Inverse(worldFromRender * renderFromCamera.Interpolate(time));
        }

        __host__ __device__ bool CameraFromRenderHasScale() const {
            return renderFromCamera.HasScale();
        }

        __host__ __device__ Vector3f RenderFromCamera(Vector3f v, Float time) const {
            return renderFromCamera(v, time);
        }

        __host__ __device__ Normal3f RenderFromCamera(Normal3f n, Float time) const {
            return renderFromCamera(n, time);
        }

        __host__ __device__ Ray RenderFromCamera(const Ray& r) const {
            return renderFromCamera(r);
        }

        __host__ __device__ RayDifferential RenderFromCamera(const RayDifferential& r) const {
            return renderFromCamera(r);
        }

        __host__ __device__ Vector3f CameraFromRender(Vector3f v, Float time) const {
            return renderFromCamera.ApplyInverse(v, time);
        }

        __host__ __device__ Normal3f CameraFromRender(Normal3f v, Float time) const {
            return renderFromCamera.ApplyInverse(v, time);
        }

        __host__ __device__ const AnimatedTransform& RenderFromCamera() const {
            return renderFromCamera;
        }

        __host__ __device__ const Transform& WorldFromRender() const {
    ...
    struct CameraWiSample {
        ...
        __host__ __device__ CameraWiSample(const SampledSpectrum& Wi, const Vector3f& wi, Float pdf, Point2f pRaster, const Interaction& pRef, const Interaction& pLens) : Wi(Wi), wi(wi), pdf(pdf), pRaster(pRaster), pRef(pRef), pLens(pLens) {}
    ...
    protected:
        ...
        __host__ __device__ static pstd::optional<CameraRayDifferential> GenerateRayDifferential(Camera camera, CameraSample sample, SampledWavelengths& lambda);
        ...
        __host__ __device__ Normal3f RenderFromCamera(Normal3f v, Float time) const {
            return cameraTransform.RenderFromCamera(v, time);
        }
    ...
    public:
        ...
        ProjectiveCamera(CameraBaseParameters baseParameters, const Transform& screenFromCamera, Bounds2f screenWindow, Float lensRadius, Float focalDistance) : CameraBase(baseParameters), screenFromCamera(screenFromCamera), lensRadius(lensRadius), focalDistance(focalDistance) {
    ...
    public:
        ...
        OrthographicCamera(CameraBaseParameters baseParameters, Bounds2f screenWindow, Float lensRadius, Float focalDist) : ProjectiveCamera(baseParameters, Orthographic(0, 1), screenWindow, lensRadius, focalDist) {
        ...
        __host__ __device__ SampledSpectrum We(const Ray& ray, SampledWavelengths& lambda, Point2f* pRaster2 = nullptr) const {
    ...
    public:
        ...
        PerspectiveCamera(CameraBaseParameters baseParameters, Float fov, Bounds2f screenWindow, Float lensRadius, Float focalDist) : ProjectiveCamera(baseParameters, Perspective(fov, 1e-2f, 1000.f), screenWindow, lensRadius, focalDist) {
    ...
    public:
        ...
        SphericalCamera(CameraBaseParameters baseParameters, Mapping mapping) : CameraBase(baseParameters), mapping(mapping) {
    ...
    private:
        ...
        __host__ __device__ static bool IntersectSphericalElement(Float radius, Float zCenter, const Ray& ray, Float* t, Normal3f* n) {
    ...
    __host__ __device__ inline const CameraTransform& Camera::GetCameraTransform() const {
        auto gtc = [&](auto ptr) -> const CameraTransform& { return ptr->GetCameraTransform(); };
        return Dispatch(gtc);
    }

    __host__ __device__ inline void Camera::Approximate_dp_dxy(Point3f p, Normal3f n, Float time, int samplesPerPixel, Vector3f* dpdx, Vector3f* dpdy) const {
