...
#include <pathtracer/core/lights.cuh> // LightBounds. Should that live elsewhere?
...
namespace spectra {
    ...
    public:
        ...
        __host__ __device__ CompactLightBounds(const LightBounds& lb, const Bounds3f& allb) : w(Normalize(lb.w)), phi(lb.phi), qCosTheta_o(QuantizeCos(lb.cosTheta_o)), qCosTheta_e(QuantizeCos(lb.cosTheta_e)), twoSided(lb.twoSided) {
        ...
        __host__ __device__ bool TwoSided() const {
            return twoSided;
        }

        __host__ __device__ Float CosTheta_o() const {
            return 2 * (qCosTheta_o / 32767.f) - 1;
        }

        __host__ __device__ Float CosTheta_e() const {
            return 2 * (qCosTheta_e / 32767.f) - 1;
        }

        __host__ __device__ Bounds3f Bounds(const Bounds3f& allb) const {
        ...
        __host__ __device__ Float Importance(Point3f p, Normal3f n, const Bounds3f& allb) const {
    ...
    private:
        ...
        __host__ __device__ static unsigned int QuantizeCos(Float c) {
            CHECK(c >= -1 && c <= 1);
            return pstd::floor(32767.f * ((c + 1) / 2));
        }

        __host__ __device__ static Float QuantizeBounds(Float c, Float min, Float max) {
            CHECK(c >= min && c <= max);
            if (min == max) return 0;
    ...
    struct alignas(32) LightBVHNode {
        ...
        __host__ __device__ LightBVHNode(const CompactLightBounds& lightBounds, unsigned int childOrLightIndex, unsigned int isLeaf) : lightBounds(lightBounds), childOrLightIndex(childOrLightIndex), isLeaf(isLeaf) {}

        __host__ __device__ static LightBVHNode MakeLeaf(unsigned int lightIndex, const CompactLightBounds& cb) {
            return LightBVHNode(cb, lightIndex, 1);
        }

        __host__ __device__ static LightBVHNode MakeInterior(unsigned int child1Index, const CompactLightBounds& cb) {
    ...
    __host__ __device__ inline pstd::optional<SampledLight> LightSampler::Sample(const LightSampleContext& ctx, Float u) const {
        auto s = [&](auto ptr) { return ptr->Sample(ctx, u); };
        return Dispatch(s);
    }

    __host__ __device__ inline Float LightSampler::PMF(const LightSampleContext& ctx, Light light) const {
        auto pdf = [&](auto ptr) { return ptr->PMF(ctx, light); };
        return Dispatch(pdf);
    }

    __host__ __device__ inline pstd::optional<SampledLight> LightSampler::Sample(Float u) const {
        auto sample = [&](auto ptr) { return ptr->Sample(u); };
        return Dispatch(sample);
    }

    __host__ __device__ inline Float LightSampler::PMF(Light light) const {
