...
namespace {
    ...
    void write_png_rgba8(const std::filesystem::path& path, const vk::Extent2D extent, const std::vector<std::uint8_t>& pixels) {
        ...
        if (error != 0u) throw std::runtime_error(std::format("Failed to encode Spectra pathtracer screenshot: {}", lodepng_error_text(error)));
...
namespace spectra::pathtracer {
    ...
    void RenderScene(const scene::Scene::ResolvedScene& scene, OfflineRenderRequest request) {
        ...
        if (request.cuda_device.has_value()) runtime_config.cuda_device = *request.cuda_device;

        RenderConfig render_config{};
        render_config.rendering_space = RenderingSpace::CameraWorld;
        render_config.output_file = std::move(request.output_file);
        render_config.seed = request.seed;
        render_config.quiet = request.quiet;
        if (request.pixel_samples.has_value()) render_config.pixel_samples = *request.pixel_samples;
...
namespace {
    ...
    void create_pipeline_viewport_descriptors(spectra::pathtracer::RenderPipeline& pipeline) {
        for (spectra::pathtracer::RenderPipeline::FrameResource& frame : pipeline.frames) {
            ...
            frame.imgui_descriptor = ImGui_ImplVulkan_AddTexture(*frame.image_view, static_cast<VkImageLayout>(vk::ImageLayout::eShaderReadOnlyOptimal));
...
namespace spectra::pathtracer {
    class Renderer::Impl {
    public:
        Impl(std::shared_ptr<scene::Scene> source_scene, std::shared_ptr<scene::CameraWorkspace> camera_workspace);
        ~Impl() noexcept;
    ...
    private:
        ...
        void draw_workspace_tab();
        void draw_render_session_panel();
        ...
        void commit_camera_state(scene::ViewportCamera state);
        void reset_camera();
        ...
        [[nodiscard]] std::string pipeline_status() const;
        ...
        std::shared_ptr<scene::Scene> source_scene{};
        ...
        struct {
            ...
            float sidebar_split_ratio{0.52f};
        } ui;
        ...
        struct {
            ...
            scene::ViewportCamera state = scene::viewport_camera_from_look_at(
    ...
    Renderer::Renderer(std::shared_ptr<scene::Scene> source_scene, std::shared_ptr<scene::CameraWorkspace> camera_workspace) : impl(std::make_unique<Impl>(std::move(source_scene), std::move(camera_workspace))) {}
    ...
    Renderer::Impl::Impl(std::shared_ptr<scene::Scene> source_scene, std::shared_ptr<scene::CameraWorkspace> camera_workspace) : source_scene(std::move(source_scene)), camera_workspace(std::move(camera_workspace)) {
    ...
    std::optional<scene::ViewportNavigationTarget> Renderer::Impl::explicit_viewport_navigation_target() const {
        return this->source_scene->navigation_target();
    }
    ...
    void Renderer::Impl::apply_camera_snapshot(const scene::CameraSnapshot& snapshot) {
        ...
        this->camera.input_enabled      = false;
        this->camera.moving_from_camera = moving_from_camera_from_viewport_camera(this->camera.camera_from_world, this->camera.state);
        this->camera.initialized = true;
        this->camera.observed_revision = snapshot.revision;
    }

    void Renderer::Impl::commit_camera_state(scene::ViewportCamera state) {
    ...
    [[nodiscard]] std::string Renderer::Impl::pipeline_status() const {
        const std::array<int, 2> sample_range = this->sample_range();
        if (!this->scene_snapshot.has_value()) return this->scene_status_state;
        if (this->render_resolution_sync.rebuilding) return "Rebuilding";
        if (!this->render_resolution_sync.pipeline_created) return this->render_resolution_sync.candidate_known ? "Pending Resolution" : "Waiting for Viewport";
        if (this->render_pipeline == nullptr) return "Unavailable";
        if (this->sample_config_rebuild_requested) return "Rebuild Pending";
        return sample_range[0] >= sample_range[1] ? "Completed" : "Sampling";
    }
    ...
    bool Renderer::Impl::process_camera_input() {
        ...
        scene::ViewportCamera state = this->camera.state;
...
namespace spectra::pathtracer {
    ...
    void Renderer::Impl::draw_workspace_tab() {
        const ImVec2 available = ImGui::GetContentRegionAvail();
        if (available.y <= 1.0f) return;

        constexpr float splitter_height = 6.0f;
        constexpr float minimum_panel_height = 150.0f;
        const float usable_height = std::max(1.0f, available.y - splitter_height);
        float session_height = usable_height * this->ui.sidebar_split_ratio;
    ...
    void Renderer::Impl::draw_render_session_panel() {
        const std::string status = this->pipeline_status();
        ...
        else render_resolution = this->ui.viewport_framebuffer_size;

        const std::array<int, 2> samples = this->sample_range();
        ...
        draw_statistics_row("State", status);
    ...
    void Renderer::Impl::draw_inspector_panel() {
        ...
        if (this->camera.initialized) {
            draw_statistics_row("Eye", std::format("{:.3f}, {:.3f}, {:.3f}", this->camera.state.pose.position.x, this->camera.state.pose.position.y, this->camera.state.pose.position.z));
            draw_statistics_row("Focus", std::format("{:.3f}, {:.3f}, {:.3f}", this->camera.state.focus.x, this->camera.state.focus.y, this->camera.state.focus.z));
            draw_statistics_row("Navigation Up", std::format("{:.3f}, {:.3f}, {:.3f}", this->camera.state.navigation_up.x, this->camera.state.navigation_up.y, this->camera.state.navigation_up.z));
            draw_statistics_row("FOV", std::format("{:.3f}", this->camera.state.projection.vertical_fov_degrees));
        } else {
    ...
    void Renderer::Impl::draw_viewport_overlays() {
        ...
        if (image_size.y >= 90.0f) {
            ...
            const std::string hud = fitting_overlay_text({
                std::format("{} | {} | {} | {} | render {}", scene_title, status, sample_text, progress_text, render_resolution),
                std::format("{} | {} | {} | render {}", status, sample_text, progress_text, render_resolution),
                std::format("{} | render {}", status, render_resolution),
                status,
            }, hud_text_width);
            if (!hud.empty()) {
