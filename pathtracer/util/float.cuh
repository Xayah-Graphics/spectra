#ifndef SPECTRA_PATHTRACER_UTIL_FLOAT_H
#define SPECTRA_PATHTRACER_UTIL_FLOAT_H
...
#define MachineEpsilon std::numeric_limits<Float>::epsilon() * 0.5f
    ...
    template <typename T>
    __host__ __device__ typename std::enable_if_t<std::is_floating_point_v<T>, bool> IsNaN(T v) {
#if defined(__CUDA_ARCH__)
        return isnan(v);
#else
        return std::isnan(v);
    ...
    template <typename T>
    __host__ __device__ typename std::enable_if_t<std::is_integral_v<T>, bool> IsNaN(T v) {
        return false;
    }

    template <typename T>
    __host__ __device__ typename std::enable_if_t<std::is_floating_point_v<T>, bool> IsInf(T v) {
#if defined(__CUDA_ARCH__)
        return isinf(v);
#else
        return std::isinf(v);
    ...
    template <typename T>
    __host__ __device__ typename std::enable_if_t<std::is_integral_v<T>, bool> IsInf(T v) {
        return false;
    }

    template <typename T>
    __host__ __device__ typename std::enable_if_t<std::is_floating_point_v<T>, bool> IsFinite(T v) {
#if defined(__CUDA_ARCH__)
        return isfinite(v);
#else
    ...
    template <typename T>
    __host__ __device__ typename std::enable_if_t<std::is_integral_v<T>, bool> IsFinite(T v) {
        return true;
    }
    ...
    __host__ __device__ inline uint32_t FloatToBits(float f) {
#if defined(__CUDA_ARCH__)
    ...
    __host__ __device__ inline float BitsToFloat(uint32_t ui) {
#if defined(__CUDA_ARCH__)
    ...
    __host__ __device__ inline int Exponent(float v) {
        return (FloatToBits(v) >> 23) - 127;
    }

    __host__ __device__ inline int Significand(float v) {
        return FloatToBits(v) & ((1 << 23) - 1);
    }

    __host__ __device__ inline uint32_t SignBit(float v) {
        return FloatToBits(v) & 0x80000000;
    }

    __host__ __device__ inline uint64_t FloatToBits(double f) {
#if defined(__CUDA_ARCH__)
    ...
    __host__ __device__ inline double BitsToFloat(uint64_t ui) {
#if defined(__CUDA_ARCH__)
    ...
    __host__ __device__ inline float NextFloatUp(float v) {
    ...
    __host__ __device__ inline float NextFloatDown(float v) {
    ...
    inline __host__ __device__ Float SubRoundUp(Float a, Float b) {
        return AddRoundUp(a, -b);
    }

    inline __host__ __device__ Float SubRoundDown(Float a, Float b) {
    ...
    __host__ __device__ inline double NextFloatUp(double v) {
        if (IsInf(v) && v > 0.) return v;
        if (v == -0.f) v = 0.f;
    ...
    __host__ __device__ inline double NextFloatDown(double v) {
        if (IsInf(v) && v < 0.) return v;
        if (v == 0.f) v = -0.f;
        uint64_t ui = FloatToBits(v);
        if (v > 0.)
    ...
    __host__ __device__ inline int Exponent(double d) {
        return (FloatToBits(d) >> 52) - 1023;
    }

    __host__ __device__ inline uint64_t Significand(double d) {
        return FloatToBits(d) & ((1ull << 52) - 1);
    }

    __host__ __device__ inline uint64_t SignBit(double v) {
        return FloatToBits(v) & 0x8000000000000000;
    }

    __host__ __device__ inline double FlipSign(double a, double b) {
    ...
    public:
        ...
        __host__ __device__ static Half FromBits(uint16_t v) {
            return Half(v);
        }

        __host__ __device__ explicit Half(float ff) {
#if defined(__CUDA_ARCH__)
        ...
        __host__ __device__ explicit Half(double d) : Half(float(d)) {}
        ...
        __host__ __device__ bool operator==(const Half& v) const {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 530
            return __ushort_as_half(h) == __ushort_as_half(v.h);
#else
            if (Bits() == v.Bits()) return true;
            return ((Bits() == HalfNegativeZero && v.Bits() == HalfPositiveZero) || (Bits() == HalfPositiveZero && v.Bits() == HalfNegativeZero));
#endif
        }
        ...
        __host__ __device__ Half operator-() const {
        ...
        __host__ __device__ int Sign() {
            return (h >> 15) ? -1 : 1;
        }

        __host__ __device__ bool IsInf() {
            return h == HalfPositiveInfinity || h == HalfNegativeInfinity;
        }

        __host__ __device__ bool IsNaN() {
            return ((h & HalfExponentMask) == HalfExponentMask && (h & HalfSignificandMask) != 0);
        }

        __host__ __device__ Half NextUp() {
        ...
        __host__ __device__ Half NextDown() {
            if (IsInf() && Sign() == -1) return *this;
    ...
    private:
        __host__ __device__ explicit Half(uint16_t h) : h(h) {}

        uint16_t h;
    };
} // namespace spectra

#endif // SPECTRA_PATHTRACER_UTIL_FLOAT_H
