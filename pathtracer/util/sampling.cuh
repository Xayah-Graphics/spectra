#ifndef SPECTRA_PATHTRACER_UTIL_SAMPLING_H
#define SPECTRA_PATHTRACER_UTIL_SAMPLING_H
...
namespace spectra {
    ...
    __host__ __device__ inline Float InvertLinearSample(Float x, Float a, Float b);

    __host__ __device__ pstd::array<Float, 3> SampleSphericalTriangle(const pstd::array<Point3f, 3>& v, Point3f p, Point2f u, Float* pdf = nullptr);

    __host__ __device__ Point2f InvertSphericalTriangleSample(const pstd::array<Point3f, 3>& v, Point3f p, Vector3f w);

    __host__ __device__ Point3f SampleSphericalRectangle(Point3f p, Point3f v00, Vector3f eu, Vector3f ev, Point2f u, Float* pdf = nullptr);

    __host__ __device__ Point2f InvertSphericalRectangleSample(Point3f pRef, Point3f v00, Vector3f eu, Vector3f ev, Point3f pRect);

    __host__ __device__ Vector3f SampleHenyeyGreenstein(Vector3f wo, Float g, Point2f u, Float* pdf = nullptr);

    __host__ __device__ Float SampleCatmullRom(pstd::span<const Float> nodes, pstd::span<const Float> f, pstd::span<const Float> cdf, Float sample, Float* fval = nullptr, Float* pdf = nullptr);

    __host__ __device__ Float SampleCatmullRom2D(pstd::span<const Float> nodes1, pstd::span<const Float> nodes2, pstd::span<const Float> values, pstd::span<const Float> cdf, Float alpha, Float sample, Float* fval = nullptr, Float* pdf = nullptr);

    // Sampling Inline Functions
    __host__ __device__ inline Float BalanceHeuristic(int nf, Float fPdf, int ng, Float gPdf) {
    ...
    __host__ __device__ inline int SampleDiscrete(pstd::span<const Float> weights, Float u, Float* pmf, Float* uRemapped) {
        ...
        if (uRemapped) *uRemapped = std::min((up - sum) / weights[offset], OneMinusEpsilon);
    ...
    __host__ __device__ inline Point2f InvertBilinearSample(Point2f p, pstd::span<const Float> w) {
        return {InvertLinearSample(p.x, Lerp(p.y, w[0], w[2]), Lerp(p.y, w[1], w[3])), InvertLinearSample(p.y, w[0] + w[1], w[2] + w[3])};
    }
    ...
    __host__ __device__ inline Float SampleVisibleWavelengths(Float u) {
        return 538 - 138.888889f * std::atanh(0.85691062f - 1.82750197f * u);
    }

    __host__ __device__ inline pstd::array<Float, 3> SampleUniformTriangle(Point2f u) {
        Float b0, b1;
        if (u[0] < u[1]) {
            b0 = u[0] / 2;
            b1 = u[1] - b0;
    ...
    __host__ __device__ inline Point2f InvertUniformTriangleSample(const pstd::array<Float, 3>& b) {
        if (b[0] > b[1]) {
    ...
    __host__ __device__ inline Float SampleTent(Float u, Float r) {
    ...
    __host__ __device__ inline Float TentPDF(Float x, Float r) {
        if (std::abs(x) >= r) return 0;
        return 1 / r - std::abs(x) / Sqr(r);
    }

    __host__ __device__ inline Float InvertTentSample(Float x, Float r) {
        if (x <= 0)
    ...
    __host__ __device__ inline Float NormalPDF(Float x, Float mu = 0, Float sigma = 1) {
        return Gaussian(x, mu, sigma);
    }

    __host__ __device__ inline Float SampleNormal(Float u, Float mu = 0, Float sigma = 1) {
        return mu + Sqrt2 * sigma * ErfInv(2 * u - 1);
    }

    __host__ __device__ inline Float InvertNormalSample(Float x, Float mu = 0, Float sigma = 1) {
        return 0.5f * (1 + std::erf((x - mu) / (sigma * Sqrt2)));
    }

    __host__ __device__ inline Point2f SampleTwoNormal(Point2f u, Float mu = 0, Float sigma = 1) {
        Float r2 = -2 * std::log1p(-u[0]); // log(1-u[0]), robustly.
        return {mu + sigma * std::sqrt(r2) * std::cos(2 * Pi * u[1]), mu + sigma * std::sqrt(r2) * std::sin(2 * Pi * u[1])};
    }

    __host__ __device__ inline Float LogisticPDF(Float x, Float s) {
        x = std::abs(x);
        return std::exp(-x / s) / (s * Sqr(1 + std::exp(-x / s)));
    }

    __host__ __device__ inline Float SampleLogistic(Float u, Float s) {
        return -s * std::log(1 / u - 1);
    }

    __host__ __device__ inline Float InvertLogisticSample(Float x, Float s) {
        return 1 / (1 + std::exp(-x / s));
    }

    __host__ __device__ inline Float TrimmedLogisticPDF(Float x, Float s, Float a, Float b) {
        if (x < a || x > b) return 0;
    ...
    __host__ __device__ inline Float InvertSmoothStepSample(Float x, Float a, Float b) {
        Float t = (x - a) / (b - a);
        auto P  = [&](Float x) { return 2 * Pow<3>(t) - Pow<4>(t); };
        return (P(x) - P(a)) / (P(b) - P(a));
    }

    __host__ __device__ inline Point2f SampleUniformDiskPolar(Point2f u) {
        Float r     = std::sqrt(u[0]);
        Float theta = 2 * Pi * u[1];
        return {r * std::cos(theta), r * std::sin(theta)};
    }

    __host__ __device__ inline Point2f InvertUniformDiskPolarSample(Point2f p) {
        Float phi = std::atan2(p.y, p.x);
        if (phi < 0) phi += 2 * Pi;
        return Point2f(Sqr(p.x) + Sqr(p.y), phi / (2 * Pi));
    }

    __host__ __device__ inline Point2f SampleUniformDiskConcentric(Point2f u) {
    ...
    __host__ __device__ inline Point2f InvertUniformDiskConcentricSample(Point2f p) {
    ...
    __host__ __device__ inline Vector3f SampleUniformHemisphere(Point2f u) {
        Float z   = u[0];
    ...
    __host__ __device__ inline Point2f InvertUniformHemisphereSample(Vector3f w) {
        Float phi = std::atan2(w.y, w.x);
        if (phi < 0) phi += 2 * Pi;
        return Point2f(w.z, phi / (2 * Pi));
    }

    __host__ __device__ inline Vector3f SampleUniformSphere(Point2f u) {
        Float z   = 1 - 2 * u[0];
        Float r   = SafeSqrt(1 - Sqr(z));
        Float phi = 2 * Pi * u[1];
        return {r * std::cos(phi), r * std::sin(phi), z};
    ...
    __host__ __device__ inline Point2f InvertUniformSphereSample(Vector3f w) {
        Float phi = std::atan2(w.y, w.x);
        if (phi < 0) phi += 2 * Pi;
        return Point2f((1 - w.z) / 2, phi / (2 * Pi));
    }

    __host__ __device__ inline Vector3f SampleCosineHemisphere(Point2f u) {
    ...
    __host__ __device__ inline Point2f InvertCosineHemisphereSample(Vector3f w) {
        return InvertUniformDiskConcentricSample({w.x, w.y});
    }

    __host__ __device__ inline Float UniformConePDF(Float cosThetaMax) {
        return 1 / (2 * Pi * (1 - cosThetaMax));
    }

    __host__ __device__ inline Vector3f SampleUniformCone(Point2f u, Float cosThetaMax) {
    ...
    __host__ __device__ inline Point2f InvertUniformConeSample(Vector3f w, Float cosThetaMax) {
        Float cosTheta = w.z;
    ...
    __host__ __device__ inline Float SampleTrimmedExponential(Float u, Float c, Float xMax) {
        return std::log(1 - u * (1 - std::exp(-c * xMax))) / -c;
    }

    __host__ __device__ inline Float TrimmedExponentialPDF(Float x, Float c, Float xMax) {
    ...
    __host__ __device__ inline Vector3f SampleUniformHemisphereConcentric(Point2f u) {
        ...
        return Vector3f(std::cos(theta) * r * std::sqrt(2 - r * r), std::sin(theta) * r * std::sqrt(2 - r * r), 1 - r * r);
    }
    ...
    public:
        ...
        __host__ __device__ void Add(Float x) {
            ++n;
            Float delta = x - mean;
            mean += delta / n;
            Float delta2 = x - mean;
        ...
        __host__ __device__ Float Mean() const {
            return mean;
        }

        __host__ __device__ Float Variance() const {
            return (n > 1) ? S / (n - 1) : 0;
        }

        __host__ __device__ int64_t Count() const {
            return n;
        }

        __host__ __device__ Float RelativeVariance() const {
            return (n < 1 || mean == 0) ? 0 : Variance() / Mean();
        }

        __host__ __device__ void Merge(const VarianceEstimator& ve) {
            if (ve.n == 0) return;
    ...
    public:
        ...
        __host__ __device__ WeightedReservoirSampler(uint64_t rngSeed) : rng(rngSeed) {}

        __host__ __device__ void Seed(uint64_t seed) {
            rng.SetSequence(seed);
        }

        __host__ __device__ bool Add(const T& sample, Float weight) {
            weightSum += weight;
        ...
        template <typename F>
        __host__ __device__ bool Add(F func, Float weight) {
        ...
        __host__ __device__ void Copy(const WeightedReservoirSampler& wrs) {
            weightSum       = wrs.weightSum;
            reservoir       = wrs.reservoir;
            reservoirWeight = wrs.reservoirWeight;
        }

        __host__ __device__ int HasSample() const {
            return weightSum > 0;
        }

        __host__ __device__ const T& GetSample() const {
            return reservoir;
        }

        __host__ __device__ Float SampleProbability() const {
            return reservoirWeight / weightSum;
        }

        __host__ __device__ Float WeightSum() const {
            return weightSum;
        }
    ...
    public:
        ...
        __host__ __device__ size_t BytesUsed() const {
        ...
        PiecewiseConstant1D(pstd::span<const Float> f, Allocator alloc = {}) : PiecewiseConstant1D(f, 0., 1., alloc) {}
        ...
        __host__ __device__ Float Sample(Float u, Float* pdf = nullptr, int* offset = nullptr) const {
        ...
        __host__ __device__ pstd::optional<Float> Invert(Float x) const {
    ...
    public:
        ...
        PiecewiseConstant2D(pstd::span<const Float> data, int nx, int ny, Allocator alloc = {}) : PiecewiseConstant2D(data, nx, ny, Bounds2f(Point2f(0, 0), Point2f(1, 1)), alloc) {}

        explicit PiecewiseConstant2D(const Array2D<Float>& data, Allocator alloc = {}) : PiecewiseConstant2D(pstd::span<const Float>(data.begin(), static_cast<size_t>(data.size())), data.XSize(), data.YSize(), alloc) {}

        PiecewiseConstant2D(const Array2D<Float>& data, Bounds2f domain, Allocator alloc = {}) : PiecewiseConstant2D(pstd::span<const Float>(data.begin(), static_cast<size_t>(data.size())), data.XSize(), data.YSize(), domain, alloc) {}

        __host__ __device__ size_t BytesUsed() const {
            return pConditionalV.size() * (pConditionalV[0].BytesUsed() + sizeof(pConditionalV[0])) + pMarginal.BytesUsed();
        }

        __host__ __device__ Bounds2f Domain() const {
            return domain;
        }

        __host__ __device__ Point2i Resolution() const {
            return {int(pConditionalV[0].size()), int(pMarginal.size())};
        }


        PiecewiseConstant2D(pstd::span<const Float> func, int nu, int nv, Bounds2f domain, Allocator alloc = {}) : domain(domain), pConditionalV(alloc), pMarginal(alloc) {
            ...
            for (int v = 0; v < nv; ++v) marginalFunc.push_back(pConditionalV[v].Integral());
        ...
        __host__ __device__ Point2f Sample(Point2f u, Float* pdf = nullptr, Point2i* offset = nullptr) const {
            Float pdfs[2];
            Point2i uv;
        ...
        __host__ __device__ Float PDF(Point2f pr) const {
        ...
        __host__ __device__ pstd::optional<Point2f> Invert(Point2f p) const {
    ...
    public:
        ...
        __host__ __device__ int Sample(Float u, Float* pmf = nullptr, Float* uRemapped = nullptr) const;

        __host__ __device__ size_t size() const {
            return bins.size();
        }

        __host__ __device__ Float PMF(int index) const {
            return bins[index].p;
        }
    ...
    public:
        ...
        SummedAreaTable(const Array2D<Float>& values, Allocator alloc = {}) : sum(values.XSize(), values.YSize(), alloc) {
            sum(0, 0) = values(0, 0);
            // Compute sums along first row and column
            for (int x = 1; x < sum.XSize(); ++x) sum(x, 0) = values(x, 0) + sum(x - 1, 0);
            for (int y = 1; y < sum.YSize(); ++y) sum(0, y) = values(0, y) + sum(0, y - 1);

            // Compute sums for the remainder of the entries
            for (int y = 1; y < sum.YSize(); ++y)
                for (int x = 1; x < sum.XSize(); ++x) sum(x, y) = (values(x, y) + sum(x - 1, y) + sum(x, y - 1) - sum(x - 1, y - 1));
        }

        __host__ __device__ Float Integral(Bounds2f extent) const {
            double s = (((double) Lookup(extent.pMax.x, extent.pMax.y) - (double) Lookup(extent.pMin.x, extent.pMax.y)) + ((double) Lookup(extent.pMin.x, extent.pMin.y) - (double) Lookup(extent.pMax.x, extent.pMin.y)));
    ...
    private:
        ...
        __host__ __device__ Float LookupInt(int x, int y) const {
    ...
    public:
        ...
        __host__ __device__ pstd::optional<Point2f> Sample(Point2f u, Bounds2f b, Float* pdf) const {
            ...
            int nx = func.XSize();
            Bounds2f bCond(Point2f(pstd::floor(p.x * nx) / nx, b.pMin.y), Point2f(pstd::ceil(p.x * nx) / nx, b.pMax.y));
            if (bCond.pMin.x == bCond.pMax.x) bCond.pMax.x += 1.f / nx;
            if (sat.Integral(bCond) == 0) return {};
        ...
        __host__ __device__ Float PDF(Point2f p, const Bounds2f& b) const {
            Float funcInt = sat.Integral(b);
            if (funcInt == 0) return 0;
    ...
    private:
        ...
        __host__ __device__ Float Eval(Point2f p) const {
            Point2i pi(std::min<int>(p[0] * func.XSize(), func.XSize() - 1), std::min<int>(p[1] * func.YSize(), func.YSize() - 1));
            return func[pi];
        }
    ...
    public:
        PiecewiseLinear2D(Allocator alloc) : m_param_values(alloc), m_data(alloc), m_marginal_cdf(alloc), m_conditional_cdf(alloc) {
            for (int i = 0; i < ArraySize; ++i) m_param_values.emplace_back(alloc);
        }
         ...
         * eval() is used).
         */
        PiecewiseLinear2D(Allocator alloc, const float* data, int xSize, int ySize, pstd::array<int, Dimension> param_res = {}, pstd::array<const float*, Dimension> param_values = {}, bool normalize = true, bool build_cdf = true) : m_size(xSize, ySize), m_patch_size(1.f / (xSize - 1), 1.f / (ySize - 1)), m_inv_patch_size(m_size - Vector2i(1, 1)), m_param_values(alloc), m_data(alloc), m_marginal_cdf(alloc), m_conditional_cdf(alloc) {
            if (build_cdf && !normalize) SPECTRA_FATAL("PiecewiseLinear2D: build_cdf implies normalize=true");

            /* Keep track of the dependence on additional parameters (optional) */
            uint32_t slices = 1;
            for (int i = 0; i < ArraySize; ++i) m_param_values.emplace_back(alloc);
            for (int i = (int) Dimension - 1; i >= 0; --i) {
                if (param_res[i] < 1) SPECTRA_FATAL("PiecewiseLinear2D(): parameter resolution must be >= 1!");

                m_param_size[i]   = param_res[i];
                m_param_values[i] = FloatStorage(param_res[i]);
                memcpy(m_param_values[i].data(), param_values[i], sizeof(float) * param_res[i]);
            ...
            if (build_cdf) {
                ...
                float *marginal_cdf = m_marginal_cdf.data(), *conditional_cdf = m_conditional_cdf.data(), *data_out = m_data.data();
                ...
                for (uint32_t slice = 0; slice < slices; ++slice) {
                    ...
                    for (int y = 0; y < m_size.y - 1; ++y) {
                        sum += .5 * ((double) conditional_cdf[(y + 1) * xSize - 1] + (double) conditional_cdf[(y + 2) * xSize - 1]);
                        marginal_cdf[y + 1] = (float) sum;
                    }

                    /* Normalize CDFs and PDF (if requested) */
                    float normalization = 1.f / marginal_cdf[m_size.y - 1];
                    for (size_t i = 0; i < n_values; ++i) conditional_cdf[i] *= normalization;
                    for (size_t i = 0; i < m_size.y; ++i) marginal_cdf[i] *= normalization;
                    for (size_t i = 0; i < n_values; ++i) data_out[i] = data[i] * normalization;
                ...
                for (uint32_t slice = 0; slice < slices; ++slice) {
                    ...
                    if (normalize) {
                        ...
                        for (int y = 0; y < m_size.y - 1; ++y) {
                            size_t i = y * xSize;
                            for (int x = 0; x < m_size.x - 1; ++x, ++i) {
                                float v00 = data[i], v10 = data[i + 1], v01 = data[i + xSize], v11 = data[i + 1 + xSize], avg = .25f * (v00 + v10 + v01 + v11);
                                sum += (double) avg;
                            }
                        }
                    ...
                    for (uint32_t k = 0; k < n_values; ++k) data_out[k] = data[k] * normalization;
        ...
        __host__ __device__ PLSample Sample(Point2f sample, Ts... params) const {
            static_assert((std::is_arithmetic_v<Ts> && ...), "Additional parameters must be numeric values");
            static_assert(sizeof...(Ts) == Dimension, "Incorrect number of additional parameters passed");
            ...
            for (size_t dim = 0; dim < Dimension; ++dim) {
                ...
                uint32_t param_index = FindInterval(m_param_size[dim], [&](uint32_t idx) { return m_param_values[dim].data()[idx] <= param[dim]; });

                Float p0 = m_param_values[dim][param_index], p1 = m_param_values[dim][param_index + 1];
            ...
            auto fetch_marginal = [&](uint32_t idx) -> float { return lookup<Dimension>(m_marginal_cdf.data(), offset + idx, m_size.y, param_weight); };

            uint32_t row = FindInterval(m_size.y, [&](uint32_t idx) { return fetch_marginal(idx) < sample.y; });
            ...
            Float r0 = lookup<Dimension>(m_conditional_cdf.data(), offset + m_size.x - 1, slice_size, param_weight), r1 = lookup<Dimension>(m_conditional_cdf.data(), offset + (m_size.x * 2 - 1), slice_size, param_weight);

            bool is_const = std::abs(r0 - r1) < 1e-4f * (r0 + r1);
            sample.y      = is_const ? (2.f * sample.y) : (r0 - SafeSqrt(r0 * r0 - 2.f * sample.y * (r0 - r1)));
            ...
            auto fetch_conditional = [&](uint32_t idx) -> float {
                float v0 = lookup<Dimension>(m_conditional_cdf.data(), offset + idx, slice_size, param_weight), v1 = lookup<Dimension>(m_conditional_cdf.data() + m_size.x, offset + idx, slice_size, param_weight);

                return (1.f - sample.y) * v0 + sample.y * v1;
            };

            uint32_t col = FindInterval(m_size.x, [&](uint32_t idx) { return fetch_conditional(idx) < sample.x; });

            sample.x -= fetch_conditional(col);

            offset += col;

            Float v00 = lookup<Dimension>(m_data.data(), offset, slice_size, param_weight), v10 = lookup<Dimension>(m_data.data() + 1, offset, slice_size, param_weight), v01 = lookup<Dimension>(m_data.data() + m_size.x, offset, slice_size, param_weight), v11 = lookup<Dimension>(m_data.data() + m_size.x + 1, offset, slice_size, param_weight), c0 = FMA((1.f - sample.y), v00, sample.y * v01), c1 = FMA((1.f - sample.y), v10, sample.y * v11);

            is_const = std::abs(c0 - c1) < 1e-4f * (c0 + c1);
            sample.x = is_const ? (2.f * sample.x) : (c0 - SafeSqrt(c0 * c0 - 2.f * sample.x * (c0 - c1)));
            sample.x /= is_const ? (c0 + c1) : (c0 - c1);

            return {Point2f((col + sample.x) * m_patch_size.x, (row + sample.y) * m_patch_size.y), ((1.f - sample.x) * c0 + sample.x * c1) * HProd(m_inv_patch_size)};
        }

        /// Inverse of the mapping implemented in \c Sample()
        template <typename... Ts>
        __host__ __device__ PLSample Invert(Point2f sample, Ts... params) const {
            static_assert((std::is_arithmetic_v<Ts> && ...), "Additional parameters must be numeric values");
            static_assert(sizeof...(Ts) == Dimension, "Incorrect number of additional parameters passed");
            pstd::array<Float, Dimension> param = {params...};
            ...
            for (size_t dim = 0; dim < Dimension; ++dim) {
                ...
                uint32_t param_index = FindInterval(m_param_size[dim], [&](uint32_t idx) { return m_param_values[dim][idx] <= param[dim]; });
            ...
            Float v00 = lookup<Dimension>(m_data.data(), offset, slice_size, param_weight), v10 = lookup<Dimension>(m_data.data() + 1, offset, slice_size, param_weight), v01 = lookup<Dimension>(m_data.data() + m_size.x, offset, slice_size, param_weight), v11 = lookup<Dimension>(m_data.data() + m_size.x + 1, offset, slice_size, param_weight);

            Vector2f w1 = Vector2f(sample), w0 = Vector2f(1, 1) - w1;

            Float c0 = FMA(w0.y, v00, w1.y * v01), c1 = FMA(w0.y, v10, w1.y * v11), pdf = FMA(w0.x, c0, w1.x * c1);

            sample.x *= c0 + .5f * sample.x * (c1 - c0);

            Float v0 = lookup<Dimension>(m_conditional_cdf.data(), offset, slice_size, param_weight), v1 = lookup<Dimension>(m_conditional_cdf.data() + m_size.x, offset, slice_size, param_weight);
            ...
            sample.y += lookup<Dimension>(m_marginal_cdf.data(), offset, m_size.y, param_weight);
         ...
         */
        template <typename... Ts>
        __host__ __device__ float Evaluate(Point2f pos, Ts... params) const {
            static_assert((std::is_arithmetic_v<Ts> && ...), "Additional parameters must be numeric values");
            static_assert(sizeof...(Ts) == Dimension, "Incorrect number of additional parameters passed");
            pstd::array<Float, Dimension> param = {params...};
            ...
            Float v00 = lookup<Dimension>(m_data.data(), index, size, param_weight), v10 = lookup<Dimension>(m_data.data() + 1, index, size, param_weight), v01 = lookup<Dimension>(m_data.data() + m_size.x, index, size, param_weight), v11 = lookup<Dimension>(m_data.data() + m_size.x + 1, index, size, param_weight);

            return FMA(w0.y, FMA(w0.x, v00, w1.x * v10), w1.y * FMA(w0.x, v01, w1.x * v11)) * HProd(m_inv_patch_size);
        }

        __host__ __device__ size_t BytesUsed() const {
            size_t sum = 4 * (m_data.capacity() + m_marginal_cdf.capacity() + m_conditional_cdf.capacity());
            for (int i = 0; i < ArraySize; ++i) sum += m_param_values[i].capacity();
            return sum;
        }
    ...
    private:
        ...
        __host__ __device__ Float lookup(const float* data, uint32_t i0, uint32_t size, const float* param_weight) const {
            ...
            Float w0 = param_weight[2 * Dim - 2], w1 = param_weight[2 * Dim - 1], v0 = lookup<Dim - 1>(data, i0, size, param_weight), v1 = lookup<Dim - 1>(data, i1, size, param_weight);

            return FMA(v0, w0, v1 * w1);
        }

        template <size_t Dim, std::enable_if_t<Dim == 0, int> = 0>
        __host__ __device__ Float lookup(const float* data, uint32_t index, uint32_t, const float*) const {
            return data[index];
        }
...
#endif // SPECTRA_PATHTRACER_UTIL_SAMPLING_H
