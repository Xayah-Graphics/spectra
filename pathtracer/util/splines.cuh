#ifndef SPECTRA_PATHTRACER_UTIL_SPLINES_H
#define SPECTRA_PATHTRACER_UTIL_SPLINES_H
...
namespace spectra {
    ...
    template <typename P>
    __host__ __device__ P BlossomCubicBezier(pstd::span<const P> p, Float u0, Float u1, Float u2) {
    ...
    template <typename P>
    __host__ __device__ P EvaluateCubicBezier(pstd::span<const P> cp, Float u) {
        return BlossomCubicBezier(cp, u, u, u);
    }

    __host__ __device__ inline Point3f EvaluateCubicBezier(pstd::span<const Point3f> cp, Float u, Vector3f* deriv) {
    ...
    __host__ __device__ inline pstd::array<Point3f, 7> SubdivideCubicBezier(pstd::span<const Point3f> cp) {
        return {cp[0], (cp[0] + cp[1]) / 2, (cp[0] + 2 * cp[1] + cp[2]) / 4, (cp[0] + 3 * cp[1] + 3 * cp[2] + cp[3]) / 8, (cp[1] + 2 * cp[2] + cp[3]) / 4, (cp[2] + cp[3]) / 2, cp[3]};
    }

    __host__ __device__ inline pstd::array<Point3f, 4> CubicBezierControlPoints(pstd::span<const Point3f> cp, Float uMin, Float uMax) {
        return {BlossomCubicBezier(cp, uMin, uMin, uMin), BlossomCubicBezier(cp, uMin, uMin, uMax), BlossomCubicBezier(cp, uMin, uMax, uMax), BlossomCubicBezier(cp, uMax, uMax, uMax)};
    }

    __host__ __device__ inline Bounds3f BoundCubicBezier(pstd::span<const Point3f> cp) {
        return Union(Bounds3f(cp[0], cp[1]), Bounds3f(cp[2], cp[3]));
    }

    __host__ __device__ inline Bounds3f BoundCubicBezier(pstd::span<const Point3f> cp, Float uMin, Float uMax) {
        if (uMin == 0 && uMax == 1) return BoundCubicBezier(cp);
        auto cpSeg = CubicBezierControlPoints(cp, uMin, uMax);
        return BoundCubicBezier(pstd::span<const Point3f>(cpSeg));
    }

    __host__ __device__ inline pstd::array<Point3f, 4> ElevateQuadraticBezierToCubic(pstd::span<const Point3f> cp) {
        return {cp[0], Lerp(2.f / 3.f, cp[0], cp[1]), Lerp(1.f / 3.f, cp[1], cp[2]), cp[2]};
    }

    __host__ __device__ inline pstd::array<Point3f, 3> QuadraticBSplineToBezier(pstd::span<const Point3f> cp) {
    ...
    __host__ __device__ inline pstd::array<Point3f, 4> CubicBSplineToBezier(pstd::span<const Point3f> cp) {
...
#endif // SPECTRA_PATHTRACER_UTIL_SPLINES_H
