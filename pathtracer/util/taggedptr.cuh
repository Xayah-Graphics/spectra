...
namespace spectra {
    ...
    public:
        ...
        __host__ __device__ TaggedPointer(std::nullptr_t np) {}

        __host__ __device__ TaggedPointer(const TaggedPointer& t) {
            bits = t.bits;
        }
        __host__ __device__ TaggedPointer& operator=(const TaggedPointer& t) {
            bits = t.bits;
            return *this;
        }

        template <typename T>
        __host__ __device__ static constexpr unsigned int TypeIndex() {
        ...
        __host__ __device__ unsigned int Tag() const {
        ...
        __host__ __device__ static constexpr unsigned int MaxTag() {
            return sizeof...(Ts);
        }
        __host__ __device__ static constexpr unsigned int NumTags() {
            return MaxTag() + 1;
        }
        ...
        __host__ __device__ bool operator<(const TaggedPointer& tp) const {
            return bits < tp.bits;
        }
        ...
        template <typename T>
        __host__ __device__ T* CastOrNullptr() {
            if (Is<T>())
                return reinterpret_cast<T*>(ptr());
            else
                return nullptr;
        ...
        template <typename T>
        __host__ __device__ const T* CastOrNullptr() const {
            if (Is<T>())
                return reinterpret_cast<const T*>(ptr());
            else
                return nullptr;
        ...
        __host__ __device__ void* ptr() {
            return reinterpret_cast<void*>(bits & ptrMask);
        }

        __host__ __device__ const void* ptr() const {
