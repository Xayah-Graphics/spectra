...
namespace {
    ...
    void print_usage(const std::string_view message) {
        if (!message.empty()) std::cerr << "xayah_bouncing_ball_cli: " << message << "\n\n";
        std::cerr << "usage: xayah_bouncing_ball_cli [--frames <n>] [--dt <seconds>] [--quiet]\n";
    }
...
int main(const int argc, char* argv[]) {
    try {
        ...
        const std::array<float, 3>& position = solver.current_position();
        std::cout << std::fixed << std::setprecision(6);
        std::cout << "bouncing_ball frames=" << options.frames
                  << " dt=" << options.delta_seconds
                  << " position=(" << position[0] << ", " << position[1] << ", " << position[2] << ")"
                  << " vertices=" << solver.mesh_vertices().size()
                  << " indices=" << solver.mesh_indices().size() << "\n";
        return 0;
