...
namespace xayah::projects::cloth::cuda {
    ...
    void launch_solve_distance_constraints(cudaStream_t cuda_stream, unsigned grid, unsigned block, std::uint32_t constraint_kind, std::uint32_t constraint_color, float* position_x, float* position_y, float* position_z, const float* inverse_mass, float* lambda, int* constraint_error_flag, std::uint32_t constraint_count, std::uint32_t columns, std::uint32_t rows, float rest_length, float compliance, float substep_seconds);
    void launch_solve_sphere_collision(cudaStream_t cuda_stream, unsigned grid, unsigned block, float* position_x, float* position_y, float* position_z, const float* inverse_mass, int* constraint_error_flag, std::uint32_t vertex_count, float center_x, float center_y, float center_z, float radius);
    void launch_update_velocities(cudaStream_t cuda_stream, unsigned grid, unsigned block, const float* position_x, const float* position_y, const float* position_z, const float* previous_x, const float* previous_y, const float* previous_z, float* velocity_x, float* velocity_y, float* velocity_z, const float* inverse_mass, std::uint32_t vertex_count, float substep_seconds);
} // namespace xayah::projects::cloth::cuda
