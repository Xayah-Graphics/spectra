...
namespace {
    ...
    void print_usage(const std::string_view message) {
        if (!message.empty()) std::cerr << "xayah_cloth_cli: " << message << "\n\n";
        std::cerr << "usage: xayah_cloth_cli [--frames <n>] [--dt <seconds>] [--quiet]\n";
    }
...
int main(const int argc, char* argv[]) {
    try {
        ...
        std::cout << "cloth frames=" << options.frames
                  << " dt=" << options.delta_seconds
                  << " vertices=" << vertices.size()
                  << " indices=" << solver.mesh_indices().size()
                  << " y_range=(" << minimum_y << ", " << maximum_y << ")\n";
        return 0;
