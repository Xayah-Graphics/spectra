...
namespace {
    ...
    void print_usage(const std::string_view message) {
        if (!message.empty()) std::cerr << "xayah_pyro_cli: " << message << "\n\n";
        std::cerr << "usage: xayah_pyro_cli [--frames <n>] [--dt <seconds>] [--quiet]\n";
    }
    ...
    [[nodiscard]] CliOptions parse_cli(const std::vector<std::string>& arguments) {
        ...
        if (options.frames > static_cast<std::uint32_t>(std::numeric_limits<int>::max())) throw UsageError("--frames exceeds Pyro frame index range");
        return options;
    }
} // namespace
...
int main(const int argc, char* argv[]) {
    try {
        ...
        std::cout << "pyro frames=" << options.frames
                  << " dt=" << options.delta_seconds
                  << " resolution=(" << frame.resolution[0] << ", " << frame.resolution[1] << ", " << frame.resolution[2] << ")"
