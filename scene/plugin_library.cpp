...
namespace spectra::scene {
    ...
    PluginLibrary::PluginLibrary(std::filesystem::path plugin_path) : state(std::make_unique<State>(make_plugin_inspect_request_storage(std::move(plugin_path)))) {}

    PluginLibrary::PluginLibrary(std::filesystem::path plugin_path, std::vector<ControlOption> options, std::shared_ptr<HostServices> host) : state(std::make_unique<State>(make_plugin_open_request_storage(std::move(plugin_path), std::move(options), std::move(host)))) {}

    PluginLibrary::~PluginLibrary() noexcept = default;

    ScenePluginInfo PluginLibrary::info() const {
