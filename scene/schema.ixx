export module spectra.scene.schema;
...
namespace spectra::scene {
    ...
    export struct Float3 {
        ...
        [[nodiscard]] constexpr Float3 operator+(const Float3 right) const noexcept { return {this->x + right.x, this->y + right.y, this->z + right.z}; }
        [[nodiscard]] constexpr Float3 operator-(const Float3 right) const noexcept { return {this->x - right.x, this->y - right.y, this->z - right.z}; }
        [[nodiscard]] constexpr Float3 operator-() const noexcept { return {-this->x, -this->y, -this->z}; }
        [[nodiscard]] constexpr Float3 operator*(const float scale) const noexcept { return {this->x * scale, this->y * scale, this->z * scale}; }
        [[nodiscard]] constexpr Float3 operator/(const float scale) const noexcept { return {this->x / scale, this->y / scale, this->z / scale}; }
        [[nodiscard]] constexpr float dot(const Float3 right) const noexcept { return this->x * right.x + this->y * right.y + this->z * right.z; }
        [[nodiscard]] constexpr Float3 cross(const Float3 right) const noexcept { return {this->y * right.z - this->z * right.y, this->z * right.x - this->x * right.z, this->x * right.y - this->y * right.x}; }
    ...
    export enum class GeometryUpdateMode : std::uint8_t {
        Static,
        Deformable,
        TopologyChanging,
    };
    ...
    export struct TriangleMeshGeometry {
        GeometryUpdateMode update_mode{GeometryUpdateMode::Static};
