module spectra.scene_workspace.controller;
...
namespace spectra::scene_workspace {
    ...
    Entry::Entry(std::string id, std::string title, const EntryKind kind, std::move_only_function<std::shared_ptr<scene::Scene>()> create_static_scene, std::move_only_function<std::unique_ptr<dynamic_scene::SourceInstance>()> create_dynamic_source) : id(std::move(id)), title(std::move(title)), kind(kind), create_static_scene(std::move(create_static_scene)), create_dynamic_source(std::move(create_dynamic_source)) {}

    std::size_t Registry::upsert_static_scene(std::string id, std::string title, std::move_only_function<std::shared_ptr<scene::Scene>()> create_scene) {
        ...
        if (std::optional<std::size_t> existing_index = this->find_entry_index(id)) {
            Entry& entry = this->entries.at(*existing_index);
            if (entry.kind != EntryKind::Static) throw std::runtime_error("Cannot replace dynamic scene source with a static scene: " + id);
            entry.title = std::move(title);
            entry.create_static_scene = std::move(create_scene);
            return *existing_index;
        }
        this->entries.push_back(Entry{std::move(id), std::move(title), EntryKind::Static, std::move(create_scene), std::move_only_function<std::unique_ptr<dynamic_scene::SourceInstance>()>{}});
        return this->entries.size() - 1u;
    }

    std::size_t Registry::upsert_dynamic_source(std::string id, std::string title, std::move_only_function<std::unique_ptr<dynamic_scene::SourceInstance>()> create_source) {
        ...
        if (std::optional<std::size_t> existing_index = this->find_entry_index(id)) {
            Entry& entry = this->entries.at(*existing_index);
            if (entry.kind != EntryKind::Dynamic) throw std::runtime_error("Cannot replace static scene with a dynamic scene source: " + id);
            entry.title = std::move(title);
            entry.create_dynamic_source = std::move(create_source);
            return *existing_index;
        }
        this->entries.push_back(Entry{std::move(id), std::move(title), EntryKind::Dynamic, std::move_only_function<std::shared_ptr<scene::Scene>()>{}, std::move(create_source)});
        return this->entries.size() - 1u;
    }

    std::unique_ptr<dynamic_scene::SourceInstance> Registry::create_dynamic_source(const std::size_t index) {
        ...
        Entry& source = this->entries.at(index);
        if (source.kind != EntryKind::Dynamic) throw std::runtime_error("Scene registry entry is not dynamic");
    ...
    std::shared_ptr<scene::Scene> Registry::create_static_scene(const std::size_t index) {
        if (this->entries.empty()) throw std::runtime_error("Scene registry is empty");
        if (index >= this->entries.size()) throw std::runtime_error("Scene registry index is out of range");
        Entry& source = this->entries.at(index);
        if (source.kind != EntryKind::Static) throw std::runtime_error("Scene registry entry is not static");
    ...
    std::optional<std::size_t> Registry::find_entry_index(const std::string_view id) const {
        for (std::size_t index = 0; index < this->entries.size(); ++index)
            if (this->entries.at(index).id == id) return index;
        return std::nullopt;
    }

    const Entry& Registry::entry(const std::size_t index) const {
        if (index >= this->entries.size()) throw std::runtime_error("Scene registry index is out of range");
    ...
    Controller::Controller(Registry registry, std::shared_ptr<scene::Scene> empty_workspace, std::shared_ptr<dynamic_scene::HostServices> host) : registry(std::move(registry)), empty_workspace(std::move(empty_workspace)), host(std::move(host)) {
        if (this->empty_workspace == nullptr) throw std::runtime_error("Scene controller requires an empty Untitled workspace");
        if (this->host == nullptr) throw std::runtime_error("Scene controller requires dynamic scene host services");
        this->slots.resize(this->registry.size());
    }

    std::shared_ptr<scene::Scene> Controller::active_workspace() {
    ...
    const Entry& Controller::entry(const std::size_t index) const {
        return this->registry.entry(index);
    }

    std::size_t Controller::size() const {
        return this->registry.size();
    }

    bool Controller::has_selected_entry() const {
        return this->selected_entry_index.has_value();
    }

    std::size_t Controller::selected_index() const {
    ...
    bool Controller::pending_switch() const {
    ...
    bool Controller::has_activation_error() const {
        return !this->activation_error_message.empty();
    }

    const std::string& Controller::activation_error() const {
        return this->activation_error_message;
    }

    bool Controller::has_active_dynamic_scene_controls() {
        if (!this->selected_entry_index.has_value()) return false;
        const Entry& source = this->registry.entry(*this->selected_entry_index);
        if (source.kind != EntryKind::Dynamic) return false;
        SceneSlot& slot = this->ensure_slot(*this->selected_entry_index);
        return slot.source != nullptr;
    }

    std::shared_ptr<dynamic_scene::HostServices> Controller::dynamic_host() const {
        return this->host;
    }

    bool Controller::active_scene_timeline_enabled() {
        if (!this->selected_entry_index.has_value()) return false;
        SceneSlot& slot = this->ensure_slot(*this->selected_entry_index);
    ...
    void Controller::clear_activation_error() {
        this->activation_error_message.clear();
    }

    void Controller::request_activate(const std::size_t index) {
    ...
    bool Controller::activate_static_scene(std::string id, std::string title, std::move_only_function<std::shared_ptr<scene::Scene>()> load_scene) {
        try {
    ...
    bool Controller::activate_dynamic_scene(std::string id, std::string title, std::move_only_function<std::unique_ptr<dynamic_scene::SourceInstance>()> create_source) {
        try {
    ...
    bool Controller::apply_pending_scene() {
        ...
        try {
            ...
            const Entry& source = this->registry.entry(next_index);
    ...
    void Controller::toggle_active_scene_timeline_playback() {
    ...
    void Controller::request_active_scene_timeline_reset() {
    ...
    void Controller::update_active_scene(const double delta_seconds) {
        if (!this->selected_entry_index.has_value()) return;
        const Entry& source = this->registry.entry(*this->selected_entry_index);
        if (source.kind == EntryKind::Static) return;
        SceneSlot& slot = this->ensure_slot(*this->selected_entry_index);
        ...
        slot.source->update(dynamic_scene::UpdateInfo{
    ...
    void Controller::execute_active_dynamic_scene_control_action(const std::string_view action_id, const std::span<const dynamic_scene::Option> options) {
        ...
        const Entry& source = this->registry.entry(*this->selected_entry_index);
        if (source.kind != EntryKind::Dynamic) throw std::runtime_error("Active scene is not a dynamic scene controls");
        SceneSlot& slot = this->ensure_slot(*this->selected_entry_index);
    ...
    void Controller::update_active_dynamic_scene_control_setting(const std::string_view key, const std::string_view value) {
    ...
    void Controller::commit_dynamic_scene_revision(SceneSlot& slot, const std::string_view context) {
    ...
    void Controller::sync_slot_count() {
    ...
    void Controller::set_static_slot(const std::size_t index, std::shared_ptr<scene::Scene> scene) {
    ...
    void Controller::set_dynamic_slot(const std::size_t index, std::unique_ptr<dynamic_scene::SourceInstance> source, std::shared_ptr<scene::Scene> workspace) {
    ...
    void Controller::release_selected_dynamic_slot() {
        if (!this->selected_entry_index.has_value()) return;
        this->sync_slot_count();
        ...
        const Entry& source = this->registry.entry(*this->selected_entry_index);
        if (source.kind != EntryKind::Dynamic) return;
    ...
    Controller::SceneSlot& Controller::ensure_slot(const std::size_t index) {
        this->sync_slot_count();
        if (index >= this->slots.size()) throw std::runtime_error("Scene slot index is out of range");
        SceneSlot& slot = this->slots.at(index);
        const Entry& source = this->registry.entry(index);
        if (slot.workspace != nullptr) {
            if (source.kind == EntryKind::Dynamic && slot.source == nullptr) throw std::runtime_error("Dynamic scene slot has a workspace but no source instance");
            return slot;
        }
        if (source.kind == EntryKind::Static) {
    ...
    dynamic_scene::SourceInstance& Controller::active_dynamic_scene_control_source() {
        if (!this->selected_entry_index.has_value()) throw std::runtime_error("No active dynamic scene controls");
        const Entry& source = this->registry.entry(*this->selected_entry_index);
        if (source.kind != EntryKind::Dynamic) throw std::runtime_error("Active scene is not a dynamic scene controls");
        SceneSlot& slot = this->ensure_slot(*this->selected_entry_index);
    ...
    scene::Scene::Document Controller::create_dynamic_slot(const std::size_t index, SceneSlot* slot) {
    ...
    void Controller::reset_dynamic_scene(SceneSlot& slot, scene::Scene::Timeline timeline) {
        slot.frame_accumulator_seconds = 0.0;
        slot.stream_time_seconds = 0.0;
        slot.stream_frame_index = 0;
        slot.source->reset();
