...
module spectra.render.display.neural_field;
...
namespace spectra::render {
    NeuralFieldPass::NeuralFieldPass(runtime::VulkanRuntime& runtime, GpuScene& gpu_scene, std::filesystem::path shader_directory) : context{runtime, gpu_scene, std::move(shader_directory)} {
        if (!this->context.runtime.device.neural_field_supported) return;
        const std::vector<std::uint32_t> code = runtime::load_spirv(this->context.shader_directory / "neural_field_render.spv");
        this->render_shader                   = vk::raii::ShaderEXT{this->context.runtime.device.logical, vk::ShaderCreateInfoEXT{vk::ShaderCreateFlagBitsEXT::eDescriptorHeap, vk::ShaderStageFlagBits::eCompute, {}, vk::ShaderCodeTypeEXT::eSpirv, code.size() * sizeof(std::uint32_t), code.data(), "render_neural_field"}};
    }

    NeuralFieldPass::~NeuralFieldPass() {
        this->context.runtime.frames.defer_destruction([render = std::move(this->render_shader), model = std::move(this->model)]() mutable {});
    }

    bool NeuralFieldPass::has_visible(const scene::ResolvedSceneView scene) const noexcept {
    ...
    void NeuralFieldPass::record(const vk::raii::CommandBuffer& command_buffer, const ColorTarget target, DepthBufferView depth, const scene::ResolvedSceneView scene, const scene::Camera& camera) {
        if (!this->context.runtime.device.neural_field_supported) throw std::runtime_error("HashGridRadianceField requires VK_NV_cooperative_vector and shader float16 support");
        ...
        float far_plane{};
        std::visit(
            [&near_plane, &far_plane](const auto& data) {
    ...
    void NeuralFieldPass::synchronize_model(const vk::raii::CommandBuffer& command_buffer, const GpuNeuralField& source) {
        ...
        const auto create_matrix    = [this](const runtime::GpuBuffer& source_buffer, const std::uint32_t rows, const std::uint32_t columns) {
            ...
            if (this->context.runtime.device.logical.convertCooperativeVectorMatrixNV(query) != vk::Result::eSuccess) throw std::runtime_error("Vulkan failed to query cooperative vector matrix size");
        ...
        const std::array<const runtime::GpuBuffer*, 5> sources{&source.density_input.buffer, &source.density_output.buffer, &source.rgb_input.buffer, &source.rgb_hidden.buffer, &source.rgb_output.buffer};
