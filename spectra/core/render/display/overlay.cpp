module spectra.render.display.overlay;
...
namespace spectra::render {
    OverlayPass::OverlayPass(runtime::VulkanRuntime& runtime, GpuScene& gpu_scene, std::filesystem::path shader_directory) : context{runtime, gpu_scene, std::move(shader_directory)} {
        this->load_shaders();
    }

    OverlayPass::~OverlayPass() {
        this->context.runtime.frames.defer_destruction([mask_shaders = std::move(this->overlay.mask_shaders), axes_shaders = std::move(this->overlay.axes_shaders), outline_shaders = std::move(this->overlay.outline_shaders), mask = std::move(this->overlay.mask), depth = std::move(this->overlay.depth)]() mutable {});
    }

    void OverlayPass::record(const vk::raii::CommandBuffer& command_buffer, ColorTarget target, const scene::Camera& camera, const OverlayRequest& request) {
        ...
        for (const scene::InstanceId instance : request.selected_instances) collect(instance, selected_indices);
        if (request.active_instance) collect(*request.active_instance, active_indices);
        if (request.hovered_instance) collect(*request.hovered_instance, hovered_indices);
        const vk::Extent2D extent = target.image.extent;
        this->record_impl(command_buffer, *target.image.image, *target.image.view, target.layout, extent, vk::Rect2D{{0, 0}, extent}, camera, selected_indices, active_indices, hovered_indices, request.axes_plane, request.axes_visible, request.outline_visible);
        target.layout = vk::ImageLayout::eShaderReadOnlyOptimal;
    }


    void OverlayPass::configure_mask_render_state(const vk::raii::CommandBuffer& command_buffer, const vk::Rect2D render_region, const vk::CompareOp depth_compare, const bool depth_write) {
    ...
    void OverlayPass::load_shaders() {
        ...
        const std::vector<std::uint32_t> mask_mesh_code     = runtime::load_spirv(this->context.shader_directory / "overlay_mesh.spv");
        const std::vector<std::uint32_t> mask_fragment_code = runtime::load_spirv(this->context.shader_directory / "overlay_mask.spv");
        ...
        this->overlay.mask_shaders = vk::raii::ShaderEXTs{this->context.runtime.device.logical, mask_create_infos};

        const std::vector<std::uint32_t> axes_vertex_code   = runtime::load_spirv(this->context.shader_directory / "overlay_axes_vertex.spv");
        const std::vector<std::uint32_t> axes_fragment_code = runtime::load_spirv(this->context.shader_directory / "overlay_axes_fragment.spv");
        ...
        this->overlay.axes_shaders = vk::raii::ShaderEXTs{this->context.runtime.device.logical, axes_create_infos};

        const std::vector<std::uint32_t> outline_vertex_code   = runtime::load_spirv(this->context.shader_directory / "overlay_vertex.spv");
        const std::vector<std::uint32_t> outline_fragment_code = runtime::load_spirv(this->context.shader_directory / "overlay_outline.spv");
        ...
        this->overlay.outline_shaders = vk::raii::ShaderEXTs{this->context.runtime.device.logical, outline_create_infos};
    ...
    void OverlayPass::create_overlay_images(const vk::Extent2D extent) {
        runtime::GpuImage next_mask  = this->context.runtime.resources.create_image_2d(extent, vk::Format::eR8G8B8A8Unorm, vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eSampled);
        runtime::GpuImage next_depth = this->context.runtime.resources.create_image_2d(extent, vk::Format::eD32Sfloat, vk::ImageUsageFlagBits::eDepthStencilAttachment, vk::ImageAspectFlagBits::eDepth);
        ...
        } else this->context.runtime.resources.write_sampled_image_descriptor(this->overlay.mask_descriptor, next_mask, vk::ImageLayout::eShaderReadOnlyOptimal);
    ...
    void OverlayPass::record_impl(const vk::raii::CommandBuffer& command_buffer, const vk::Image target_image, const vk::ImageView target_view, const vk::ImageLayout target_layout, const vk::Extent2D extent, const vk::Rect2D render_region, const scene::Camera& camera, const std::span<const std::uint32_t> selected_instances, const std::span<const std::uint32_t> active_instances, const std::span<const std::uint32_t> hovered_instances, const AxesPlane axes_plane, const bool axes_visible, const bool outline_visible) {
        ...
        if (axes_visible) {
            ...
            const AxesPushData axes_push{
                ...
                {std::to_underlying(axes_plane), render_region.extent.width, render_region.extent.height, 0},
            };
