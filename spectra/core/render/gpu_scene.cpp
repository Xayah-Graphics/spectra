...
namespace spectra::render {
    namespace {
        struct OutputInstanceBinding {
            std::array<std::uint32_t, 4> metadata{};
            math::Transform primitive_from_instance{};
        };

        static_assert(sizeof(OutputInstanceBinding) == 80);
        ...
        [[nodiscard]] runtime::GpuBuffer upload_buffer(runtime::VulkanRuntime& runtime, const vk::raii::CommandBuffer& command_buffer, const std::span<const Element> elements, const vk::BufferUsageFlags usage, const std::size_t element_capacity = 0) {
            runtime::GpuBuffer destination = runtime.resources.create_buffer(std::max({elements.size(), element_capacity, std::size_t{1}}) * sizeof(Element), usage | vk::BufferUsageFlagBits::eShaderDeviceAddress | vk::BufferUsageFlagBits::eTransferDst, vk::MemoryPropertyFlagBits::eDeviceLocal, false);
            if (elements.empty()) command_buffer.fillBuffer(*destination.buffer, 0, destination.size, 0u);
            else {
        ...
        GpuTextureImage upload_texture_image(runtime::VulkanRuntime& runtime, const scene::ImageTexture& data, const vk::Format format, const vk::PipelineStageFlags2 destination_stages, const vk::raii::CommandBuffer& command_buffer) {
            ...
            } else throw std::runtime_error("GPU Texture Image format must be RGBA16F or RGBA32F");
    ...
    GpuScene::GpuScene(runtime::VulkanRuntime& runtime, std::filesystem::path shader_directory) noexcept : context{runtime, std::move(shader_directory)} {}

    GpuScene::~GpuScene() {
        this->destroy();
    }

    void GpuScene::initialize(const scene::Scene& evaluated_scene, const std::span<const simulation::MeshOutputBinding> mesh_bindings, const std::span<const simulation::SphereSetOutputBinding> sphere_set_bindings) {
    ...
    void GpuScene::destroy() noexcept {
        ...
        this->resources.output_instance_bindings_descriptor         = {};
        ...
        this->resources.output_changes                = GpuSceneChange::None;
        this->resources.output_revision               = 0;
        this->resources.output_structure_revision     = 0;
    ...
    GpuSceneView GpuScene::view() const noexcept {
        return {
            ...
            this->context.runtime.device.ray_tracing_supported && !this->resources.volumes.empty() ? &this->resources.volume_region_geometry : nullptr,
            ...
            this->resources.output_revision,
            this->resources.output_structure_revision,
        };
    }
    ...
    GpuSceneUpdate GpuScene::apply(const simulation::SimulationFrame& frame, const scene::ResolvedSceneView scene, const vk::raii::CommandBuffer& command_buffer, const std::uint32_t frame_slot_index) {
        this->resources.resource_binding_changes          = scene::SceneChange::None;
        this->resources.output_changes                    = GpuSceneChange::None;
        this->resources.recording_frame_slot              = frame_slot_index;
        ...
        for (const simulation::GpuSceneUpdate& update : frame.scene_updates) {
            if (const auto* mesh = std::get_if<simulation::GpuTriangleMeshUpdate>(&update.data)) external_geometries.push_back(mesh->geometry_id);
            else if (const auto* spheres = std::get_if<simulation::GpuSphereSetUpdate>(&update.data)) external_sphere_sets.push_back(spheres->sphere_set_id);
            else if (const auto* field = std::get_if<simulation::GpuFieldUpdate>(&update.data)) external_volumes.push_back(field->volume_id);
        }
        ...
        for (const simulation::GpuSceneUpdate& update : frame.scene_updates) {
            if (const auto* mesh = std::get_if<simulation::GpuTriangleMeshUpdate>(&update.data)) {
                this->synchronize_external_geometry(mesh->geometry_id, mesh->positions.buffer, mesh->normals ? mesh->normals->buffer : nullptr, mesh->tangents ? mesh->tangents->buffer : nullptr, command_buffer);
                this->resources.output_changes = this->resources.output_changes | GpuSceneChange::Geometry;
                continue;
            }
            if (const auto* spheres = std::get_if<simulation::GpuSphereSetUpdate>(&update.data)) {
            ...
            if (const auto* particles = std::get_if<simulation::GpuParticleSetUpdate>(&update.data)) {
                this->synchronize_external_particle_set(*particles);
                this->resources.output_changes = this->resources.output_changes | GpuSceneChange::Particle;
                continue;
            }
            const auto* field = std::get_if<simulation::GpuFieldUpdate>(&update.data);
            if (field) {
                this->synchronize_external_volume(field->volume_id, field->fields, command_buffer);
                this->resources.output_changes = this->resources.output_changes | GpuSceneChange::Volume;
                continue;
            }
            const auto* neural_field = std::get_if<simulation::GpuHashGridRadianceFieldUpdate>(&update.data);
            if (!neural_field) continue;
            this->synchronize_external_neural_field(*neural_field, command_buffer);
            this->resources.output_changes = this->resources.output_changes | GpuSceneChange::NeuralField;
        }
        this->end_external_updates(scene, command_buffer);
        bool instance_transforms_updated{};
        for (const simulation::GpuSceneUpdate& update : frame.scene_updates)
            if (const auto* transforms = std::get_if<simulation::GpuInstanceTransformUpdate>(&update.data)) {
        ...
        if (instance_transforms_updated) {
            ...
            this->resources.output_changes = this->resources.output_changes | GpuSceneChange::Transform;
        }
        ...
        if (this->resources.output_changes != GpuSceneChange::None) ++this->resources.output_revision;
        if ((this->resources.output_changes & GpuSceneChange::Structure) != GpuSceneChange::None) ++this->resources.output_structure_revision;
        return {
            std::exchange(this->resources.resource_binding_changes, scene::SceneChange::None),
            std::exchange(this->resources.output_changes, GpuSceneChange::None),
            this->resources.output_revision,
            this->resources.output_structure_revision,
        };
    }

    GpuSceneUpdate GpuScene::synchronize(const scene::ResolvedSceneView scene, const vk::raii::CommandBuffer& command_buffer, const std::uint32_t frame_slot_index) {
        this->resources.resource_binding_changes          = scene::SceneChange::None;
        this->resources.output_changes                    = GpuSceneChange::None;
        this->resources.recording_frame_slot              = frame_slot_index;
        this->resources.scratch_offsets[frame_slot_index] = 0;
    ...
    void GpuScene::initialize_resources(const scene::ResolvedSceneView scene, const std::span<const simulation::MeshOutputBinding> mesh_bindings, const std::span<const simulation::SphereSetOutputBinding> sphere_set_bindings, const vk::raii::CommandBuffer* command_buffer) {
        this->resources.output_changes            = GpuSceneChange::None;
        this->resources.output_revision           = 0;
        this->resources.output_structure_revision = 0;
        ...
        const auto create_shader                  = [this](const std::string_view file, const char* entry) {
            const std::vector<std::uint32_t> code = runtime::load_spirv(this->context.shader_directory / file);
            return vk::raii::ShaderEXT{this->context.runtime.device.logical, vk::ShaderCreateInfoEXT{vk::ShaderCreateFlagBitsEXT::eDescriptorHeap, vk::ShaderStageFlagBits::eCompute, {}, vk::ShaderCodeTypeEXT::eSpirv, code.size() * sizeof(std::uint32_t), code.data(), entry}};
        };
        ...
        this->resources.sphere_unpack_shader           = create_shader("output_sphere_unpack.spv", "unpack_sphere_output");
        this->resources.instance_apply_shader          = create_shader("output_instance_apply.spv", "apply_instance_outputs");
        const auto record                              = [&](const vk::raii::CommandBuffer& target) {
            ...
            for (const scene::SphereSet& spheres : scene.resources.sphere_sets) {
                const auto binding = std::ranges::find(sphere_set_bindings, spheres.id, &simulation::SphereSetOutputBinding::sphere_set_id);
            ...
            for (const scene::ParticleSet& particles : scene.resources.particle_sets) {
                ...
                for (const scene::ParticleField& field : particles.fields) gpu_particles.fields.emplace_back(field.id, field.name, field.unit, scene::field_kind(field), scene::field_vector_space(field));
            }
            ...
            if (this->context.runtime.device.ray_tracing_supported && !scene.resources.volumes.empty()) {
            ...
            const vk::BufferUsageFlags instance_usage                   = vk::BufferUsageFlagBits::eStorageBuffer | (this->context.runtime.device.ray_tracing_supported ? vk::BufferUsageFlagBits::eAccelerationStructureBuildInputReadOnlyKHR : vk::BufferUsageFlags{});
            ...
            if (this->context.runtime.device.ray_tracing_supported) this->resources.top_level_acceleration_structure = this->build_top_level(instances, instance_capacity, target);
            std::vector<math::Transform> primitive_transforms{};
            std::vector<OutputInstanceBinding> instance_bindings{};
            ...
            this->resources.output_instance_bindings            = upload_buffer(this->context.runtime, target, std::span<const OutputInstanceBinding>{instance_bindings}, vk::BufferUsageFlagBits::eStorageBuffer, instance_bindings.size());
            this->resources.primitive_transforms_descriptor     = this->context.runtime.frames.allocate_resource_descriptor();
            this->resources.output_instance_bindings_descriptor = this->context.runtime.frames.allocate_resource_descriptor();
            ...
            this->context.runtime.resources.write_buffer_descriptor(this->resources.output_instance_bindings_descriptor, vk::DescriptorType::eStorageBuffer, this->resources.output_instance_bindings);
            ...
            for (runtime::GpuBuffer& readback : this->resources.instance_bounds_readbacks) readback = this->context.runtime.resources.create_buffer(instance_bounds_size, vk::BufferUsageFlagBits::eTransferDst, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent, true);
            this->update_instance_bounds(scene, target, command_buffer ? this->resources.recording_frame_slot : 0);
        };
        if (command_buffer) record(*command_buffer);
        else this->context.runtime.resources.submit_immediate(record);
        if (!command_buffer) this->retire_frame(0);
        this->resources.instance_bounds_dirty = false;
        this->resources.synchronized_revision = scene.revision;
    }

    void GpuScene::cache_texture_images(const scene::ResolvedSceneView scene, const vk::raii::CommandBuffer& command_buffer) {
        ...
        for (const scene::Texture& texture : scene.resources.textures) {
            ...
            if (cached != previous_indices.end() && previous_images[cached->second].cache_revision == revision) this->resources.texture_images.emplace_back(std::move(previous_images[cached->second]));
            else {
    ...
    GpuGeometry GpuScene::create_geometry(const scene::Geometry& geometry, const vk::raii::CommandBuffer& command_buffer) {
        ...
        const auto binding                  = std::ranges::find(this->resources.mesh_bindings, geometry.id, &simulation::MeshOutputBinding::geometry_id);
        result.update_mode                  = binding == this->resources.mesh_bindings.end() ? GpuMeshUpdateMode::Immutable : GpuMeshUpdateMode::Deformable;
        ...
        result.vertex_count                 = binding == this->resources.mesh_bindings.end() ? static_cast<std::uint32_t>(mesh.positions.size()) : binding->vertex_capacity;
        result.index_count                  = binding != this->resources.mesh_bindings.end() && binding->indices ? binding->index_capacity : static_cast<std::uint32_t>(mesh.indices.size());
        ...
        const vk::BufferUsageFlags position_usage               = vk::BufferUsageFlagBits::eStorageBuffer | (this->context.runtime.device.ray_tracing_supported ? vk::BufferUsageFlagBits::eAccelerationStructureBuildInputReadOnlyKHR : vk::BufferUsageFlags{});
        ...
        if (binding != this->resources.mesh_bindings.end() && binding->indices) command_buffer.copyBuffer(*binding->indices->buffer->buffer, *result.indices.buffer, vk::BufferCopy{0u, 0u, static_cast<vk::DeviceSize>(binding->index_capacity) * sizeof(std::uint32_t)});
        if (binding != this->resources.mesh_bindings.end() && binding->texture_coordinates) {
            command_buffer.copyBuffer(*binding->texture_coordinates->buffer->buffer, *result.texture_coordinates.buffer, vk::BufferCopy{0u, 0u, static_cast<vk::DeviceSize>(binding->vertex_capacity) * sizeof(math::Float2)});
            result.attribute_mask |= gpu_geometry_attribute_texture_coordinate;
        }
        if (binding != this->resources.mesh_bindings.end() && (binding->indices || binding->texture_coordinates)) {
            const vk::MemoryBarrier2 dependency{vk::PipelineStageFlagBits2::eCopy, vk::AccessFlagBits2::eTransferWrite, vk::PipelineStageFlagBits2::eAllCommands, vk::AccessFlagBits2::eShaderStorageRead | (this->context.runtime.device.ray_tracing_supported ? vk::AccessFlagBits2::eAccelerationStructureReadKHR : vk::AccessFlags2{})};
            command_buffer.pipelineBarrier2(vk::DependencyInfo{{}, 1u, &dependency});
        }
        if (result.acceleration_kind == AccelerationGeometryKind::Procedural && this->context.runtime.device.ray_tracing_supported) {
    ...
    GpuSphereSet GpuScene::create_sphere_set(const scene::SphereSet& spheres, const vk::raii::CommandBuffer& command_buffer, const std::uint32_t capacity) {
        ...
        const vk::BufferUsageFlags aabb_usage      = vk::BufferUsageFlagBits::eStorageBuffer | (this->context.runtime.device.ray_tracing_supported ? vk::BufferUsageFlagBits::eAccelerationStructureBuildInputReadOnlyKHR : vk::BufferUsageFlags{});
        ...
        if (this->context.runtime.device.ray_tracing_supported) result.bottom_level_acceleration_structure = this->build_bottom_level(sphere_set_geometry(result), result.sphere_count, GpuMeshUpdateMode::Deformable, command_buffer, result.sphere_capacity);
        return result;
    }
                    ...
                    for (const scene::VolumeField& source : data.fields) {
                        ...
                        std::visit(
                            [&](const auto& field_data) {
                                if constexpr (std::same_as<std::remove_cvref_t<decltype(field_data)>, scene::ScalarVolumeField>) field.buffers.emplace_back(upload_buffer(this->context.runtime, command_buffer, std::span<const float>{field_data.values}, vk::BufferUsageFlagBits::eStorageBuffer, static_cast<std::size_t>(data.resolution.x) * data.resolution.y * data.resolution.z));
                                else if constexpr (std::same_as<std::remove_cvref_t<decltype(field_data)>, scene::VectorVolumeField>) field.buffers.emplace_back(upload_buffer(this->context.runtime, command_buffer, std::span<const math::Float3>{field_data.values}, vk::BufferUsageFlagBits::eStorageBuffer, static_cast<std::size_t>(data.resolution.x) * data.resolution.y * data.resolution.z));
                                else if constexpr (std::same_as<std::remove_cvref_t<decltype(field_data)>, scene::CategoryVolumeField>) field.buffers.emplace_back(upload_buffer(this->context.runtime, command_buffer, std::span<const std::uint32_t>{field_data.values}, vk::BufferUsageFlagBits::eStorageBuffer, static_cast<std::size_t>(data.resolution.x) * data.resolution.y * data.resolution.z));
                                else {
                                    ...
                                    for (std::size_t component = 0; component != 3u; ++component) field.buffers.emplace_back(upload_buffer(this->context.runtime, command_buffer, std::span<const float>{field_data.values[component]}, vk::BufferUsageFlagBits::eStorageBuffer, capacities[component]));
                                }
                            },
                            source.data);
                        for (runtime::GpuBuffer& buffer : field.buffers) {
    ...
    std::vector<vk::AccelerationStructureInstanceKHR> GpuScene::acceleration_structure_instance_data(const scene::ResolvedSceneView scene) {
        ...
        for (std::uint32_t instance_index = 0; instance_index < scene.resources.instances.size(); ++instance_index) {
            ...
            for (std::uint32_t primitive_index = 0; primitive_index < prototype->primitives.size(); ++primitive_index) {
                ...
                if (!this->context.runtime.device.ray_tracing_supported) continue;
    ...
    vk::DeviceAddress GpuScene::acquire_acceleration_scratch(const vk::DeviceSize size) {
        const std::uint32_t frame_slot_index = this->resources.recording_frame_slot;
        const vk::DeviceSize alignment       = this->context.runtime.device.acceleration_structure_properties.minAccelerationStructureScratchOffsetAlignment;
        runtime::GpuBuffer& buffer           = this->resources.frame_scratch[frame_slot_index];
        ...
        if (!available) {
            runtime::GpuBuffer replacement = this->context.runtime.resources.create_buffer(std::max(size + alignment - 1u, buffer.size * 2u), vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eShaderDeviceAddress, vk::MemoryPropertyFlagBits::eDeviceLocal, false);
    ...
    void GpuScene::update_bottom_level(GpuGeometry& gpu_geometry, const scene::Geometry& source_geometry, const vk::raii::CommandBuffer& command_buffer) {
        ...
        const std::array<math::Float3, 1> missing_float3{};
        const std::array<math::Float2, 1> missing_float2{};
        const runtime::GpuUploadSlice position_upload           = this->context.runtime.frames.stage_upload(std::as_bytes(std::span<const math::Float3>{
            triangle_mesh.positions,
        }));
        const runtime::GpuUploadSlice normal_upload             = this->context.runtime.frames.stage_upload(std::as_bytes(triangle_mesh.normals.empty() ? std::span<const math::Float3>{missing_float3} : std::span<const math::Float3>{triangle_mesh.normals}));
        const runtime::GpuUploadSlice tangent_upload            = this->context.runtime.frames.stage_upload(std::as_bytes(triangle_mesh.tangents.empty() ? std::span<const math::Float3>{missing_float3} : std::span<const math::Float3>{triangle_mesh.tangents}));
        const runtime::GpuUploadSlice texture_coordinate_upload = this->context.runtime.frames.stage_upload(std::as_bytes(triangle_mesh.texture_coordinates.empty() ? std::span<const math::Float2>{missing_float2} : std::span<const math::Float2>{triangle_mesh.texture_coordinates}));
        ...
        const std::array<vk::BufferMemoryBarrier2, 4> upload_dependencies{
            vk::BufferMemoryBarrier2{
                vk::PipelineStageFlagBits2::eCopy,
                vk::AccessFlagBits2::eTransferWrite,
                vk::PipelineStageFlagBits2::eAllCommands | (this->context.runtime.device.ray_tracing_supported ? vk::PipelineStageFlagBits2::eAccelerationStructureBuildKHR : vk::PipelineStageFlags2{}),
                vk::AccessFlagBits2::eShaderStorageRead | (this->context.runtime.device.ray_tracing_supported ? vk::AccessFlagBits2::eAccelerationStructureReadKHR : vk::AccessFlags2{}),
                vk::QueueFamilyIgnored,
                vk::QueueFamilyIgnored,
        ...
        if (this->context.runtime.device.ray_tracing_supported) {
            ...
            const vk::AccelerationStructureBuildSizesInfoKHR sizes = this->context.runtime.device.logical.getAccelerationStructureBuildSizesKHR(vk::AccelerationStructureBuildTypeKHR::eDevice, build_info, primitive_count);
    ...
    void GpuScene::generate_missing_attributes(GpuGeometry& geometry, const bool generate_normals, const bool generate_tangents, const vk::raii::CommandBuffer& command_buffer) {
        ...
        command_buffer.dispatch((geometry.vertex_count + 255u) / 256u, 1, 1);
        const vk::MemoryBarrier2 output_dependency{vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderStorageWrite, vk::PipelineStageFlagBits2::eAllCommands | (this->context.runtime.device.ray_tracing_supported ? vk::PipelineStageFlagBits2::eAccelerationStructureBuildKHR : vk::PipelineStageFlags2{}), vk::AccessFlagBits2::eShaderStorageRead | (this->context.runtime.device.ray_tracing_supported ? vk::AccessFlagBits2::eAccelerationStructureReadKHR : vk::AccessFlags2{})};
    ...
    void GpuScene::synchronize_external_geometry(const scene::GeometryId geometry_id, const runtime::GpuBuffer* positions, const runtime::GpuBuffer* normals, const runtime::GpuBuffer* tangents, const vk::raii::CommandBuffer& command_buffer) {
        ...
        if (mesh.update_mode == GpuMeshUpdateMode::Immutable) throw std::runtime_error("Mutable Geometry requires a mutable update mode");
        const vk::DeviceSize position_bytes = static_cast<vk::DeviceSize>(mesh.vertex_count) * sizeof(math::Float3);
        if (positions && mesh.vertex_count != 0) command_buffer.copyBuffer(*positions->buffer, *mesh.positions.buffer, vk::BufferCopy{0, 0, position_bytes});
        if (normals && mesh.vertex_count != 0) command_buffer.copyBuffer(*normals->buffer, *mesh.normals.buffer, vk::BufferCopy{0, 0, position_bytes});
        if (tangents && mesh.vertex_count != 0) command_buffer.copyBuffer(*tangents->buffer, *mesh.tangents.buffer, vk::BufferCopy{0, 0, position_bytes});

        const vk::MemoryBarrier2 upload_dependency{vk::PipelineStageFlagBits2::eCopy, vk::AccessFlagBits2::eTransferWrite, vk::PipelineStageFlagBits2::eAllCommands | vk::PipelineStageFlagBits2::eComputeShader | (this->context.runtime.device.ray_tracing_supported ? vk::PipelineStageFlagBits2::eAccelerationStructureBuildKHR : vk::PipelineStageFlags2{}), vk::AccessFlagBits2::eShaderStorageRead | vk::AccessFlagBits2::eShaderStorageWrite | (this->context.runtime.device.ray_tracing_supported ? vk::AccessFlagBits2::eAccelerationStructureReadKHR : vk::AccessFlags2{})};
        ...
        const bool geometry_changed  = positions;
        const bool generate_normals  = geometry_changed && !normals;
        const bool generate_tangents = (geometry_changed || normals) && !tangents;
        ...
        if (mesh.attribute_mask != previous_attribute_mask) this->resources.output_changes = this->resources.output_changes | GpuSceneChange::Structure;
        ...
        if (!this->context.runtime.device.ray_tracing_supported) return;
        ...
        const vk::AccelerationStructureBuildSizesInfoKHR sizes = this->context.runtime.device.logical.getAccelerationStructureBuildSizesKHR(vk::AccelerationStructureBuildTypeKHR::eDevice, build_info, mesh.acceleration_primitive_count);
        build_info.scratchData                                 = vk::DeviceOrHostAddressKHR{this->acquire_acceleration_scratch(sizes.updateScratchSize)};
    ...
    void GpuScene::update_sphere_set(GpuSphereSet& gpu_spheres, const scene::SphereSet& source_spheres, const vk::raii::CommandBuffer& command_buffer) {
        ...
        const std::array<runtime::GpuBuffer*, 3> destinations{&gpu_spheres.positions, &gpu_spheres.radii, &gpu_spheres.axis_aligned_boxes};
        ...
        const vk::MemoryBarrier2 upload_dependency{vk::PipelineStageFlagBits2::eCopy, vk::AccessFlagBits2::eTransferWrite, vk::PipelineStageFlagBits2::eAllCommands, vk::AccessFlagBits2::eShaderStorageRead | (this->context.runtime.device.ray_tracing_supported ? vk::AccessFlagBits2::eAccelerationStructureReadKHR : vk::AccessFlags2{})};
        command_buffer.pipelineBarrier2(vk::DependencyInfo{{}, 1, &upload_dependency});
        this->update_sphere_set_acceleration(gpu_spheres, command_buffer);
    }
    ...
    void GpuScene::update_sphere_set_acceleration(GpuSphereSet& spheres, const vk::raii::CommandBuffer& command_buffer) {
        ...
        const vk::AccelerationStructureBuildSizesInfoKHR sizes = this->context.runtime.device.logical.getAccelerationStructureBuildSizesKHR(vk::AccelerationStructureBuildTypeKHR::eDevice, build_info, spheres.sphere_capacity);
        build_info.scratchData                                 = vk::DeviceOrHostAddressKHR{this->acquire_acceleration_scratch(sizes.updateScratchSize)};
    ...
    void GpuScene::synchronize_external_sphere_set(const scene::SphereSetId sphere_set_id, const runtime::DescriptorHandle spheres_descriptor, const std::uint32_t sphere_count, const vk::raii::CommandBuffer& command_buffer) {
        ...
        if (sphere_count > spheres.sphere_capacity) {
            ...
            this->resources.output_changes = this->resources.output_changes | GpuSceneChange::Structure;
            rebuild_acceleration           = true;
        } else spheres.sphere_count = sphere_count;
        ...
        const vk::MemoryBarrier2 dependency{vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderStorageWrite, vk::PipelineStageFlagBits2::eAllCommands, vk::AccessFlagBits2::eShaderStorageRead | (this->context.runtime.device.ray_tracing_supported ? vk::AccessFlagBits2::eAccelerationStructureReadKHR : vk::AccessFlags2{})};
        command_buffer.pipelineBarrier2(vk::DependencyInfo{{}, 1, &dependency});
        if (!this->context.runtime.device.ray_tracing_supported) {
            if (acceleration_presence_changed) this->resources.output_changes = this->resources.output_changes | GpuSceneChange::Structure;
        ...
        } else this->update_sphere_set_acceleration(spheres, command_buffer);
    ...
    void GpuScene::synchronize_external_instance_transforms(const simulation::GpuInstanceTransformUpdate& update, const vk::raii::CommandBuffer& command_buffer) {
        ...
        const vk::MemoryBarrier2 transform_dependency{vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderStorageWrite, vk::PipelineStageFlagBits2::eComputeShader | vk::PipelineStageFlagBits2::eVertexShader | vk::PipelineStageFlagBits2::eMeshShaderEXT | vk::PipelineStageFlagBits2::eFragmentShader | (this->context.runtime.device.ray_tracing_supported ? vk::PipelineStageFlagBits2::eRayTracingShaderKHR : vk::PipelineStageFlags2{}), vk::AccessFlagBits2::eShaderStorageRead};
        command_buffer.pipelineBarrier2(vk::DependencyInfo{{}, 1, &transform_dependency});
        this->resources.output_changes        = this->resources.output_changes | GpuSceneChange::Transform;
        this->resources.instance_bounds_dirty = true;
    }

    void GpuScene::synchronize_external_volume(const scene::VolumeId volume_id, const std::span<const simulation::GpuFieldView> fields, const vk::raii::CommandBuffer& command_buffer) {
        GpuVolume& volume = *std::ranges::find(this->resources.volumes, volume_id, &GpuVolume::volume_id);
        for (const simulation::GpuFieldView& source : fields) {
        ...
        const vk::MemoryBarrier2 dependency{vk::PipelineStageFlagBits2::eCopy, vk::AccessFlagBits2::eTransferWrite, vk::PipelineStageFlagBits2::eComputeShader | vk::PipelineStageFlagBits2::eFragmentShader | (this->context.runtime.device.ray_tracing_supported ? vk::PipelineStageFlagBits2::eRayTracingShaderKHR : vk::PipelineStageFlags2{}), vk::AccessFlagBits2::eShaderStorageRead};
        command_buffer.pipelineBarrier2(vk::DependencyInfo{{}, 1, &dependency});
    ...
    void GpuScene::synchronize_external_particle_set(const simulation::GpuParticleSetUpdate& update) {
        ...
        for (const simulation::GpuFieldView& source : update.fields) std::ranges::find(particles.fields, source.field.id, &GpuParticleField::id)->descriptor = source.values.front().descriptor;
    }

    void GpuScene::synchronize_external_neural_field(const simulation::GpuHashGridRadianceFieldUpdate& update, const vk::raii::CommandBuffer& command_buffer) {
        GpuNeuralField& field = *std::ranges::find(this->resources.neural_fields, update.neural_field_id, &GpuNeuralField::neural_field_id);
        const auto copy       = [this, &command_buffer](GpuNeuralBuffer& destination, const simulation::GpuBufferView source) {
    ...
    void GpuScene::update_volumes(const scene::ResolvedSceneView scene, const vk::raii::CommandBuffer& command_buffer) {
    ...
    void GpuScene::update_instance_state(const scene::ResolvedSceneView scene, const vk::raii::CommandBuffer& command_buffer) {
        std::vector<math::Transform> primitive_transforms{};
        std::vector<OutputInstanceBinding> instance_bindings{};
        std::vector<std::uint32_t> acceleration_indices(this->resources.primitives.size(), std::numeric_limits<std::uint32_t>::max());
        for (std::uint32_t acceleration_index = 0; acceleration_index < this->resources.acceleration_entities.size(); ++acceleration_index) {
        ...
        const std::array uploads{
            ...
            this->context.runtime.frames.stage_upload(std::as_bytes(std::span<const OutputInstanceBinding>{instance_bindings})),
        };
        ...
        command_buffer.copyBuffer(uploads[1].buffer, *this->resources.output_instance_bindings.buffer, vk::BufferCopy{uploads[1].offset, 0, uploads[1].size});
    ...
    void GpuScene::update_instance_bounds(const scene::ResolvedSceneView scene, const vk::raii::CommandBuffer& command_buffer, const std::uint32_t frame_slot_index) {
        ...
        command_buffer.pipelineBarrier2(vk::DependencyInfo{{}, 1, &output_dependency});
        runtime::GpuBuffer& readback = this->resources.instance_bounds_readbacks[frame_slot_index];
    ...
    void GpuScene::update_top_level_from_gpu(const std::uint32_t instance_count, const vk::raii::CommandBuffer& command_buffer) {
        ...
        const vk::AccelerationStructureBuildSizesInfoKHR sizes = this->context.runtime.device.logical.getAccelerationStructureBuildSizesKHR(vk::AccelerationStructureBuildTypeKHR::eDevice, build_info, instance_count);
        build_info.scratchData                                 = vk::DeviceOrHostAddressKHR{this->acquire_acceleration_scratch(sizes.updateScratchSize)};
    ...
    void GpuScene::update_top_level(const std::span<const vk::AccelerationStructureInstanceKHR> instances, const vk::raii::CommandBuffer& command_buffer) {
        ...
        if (instances.empty()) command_buffer.pipelineBarrier2(vk::DependencyInfo{{}, 1, &bottom_level_dependency});
        else {
            const runtime::GpuUploadSlice upload = this->context.runtime.frames.stage_upload(std::as_bytes(instances));
    ...
    void GpuScene::synchronize_scene(const scene::ResolvedSceneView scene, const vk::raii::CommandBuffer& command_buffer) {
        ...
        if ((scene.revision.changes & scene::SceneChange::Structure) != scene::SceneChange::None) {
            ...
            const std::vector<simulation::MeshOutputBinding> mesh_bindings = this->resources.mesh_bindings;
            std::vector<simulation::SphereSetOutputBinding> sphere_set_bindings{};
            ...
            const std::uint64_t output_revision    = this->resources.output_revision + 1;
            const std::uint64_t structure_revision = this->resources.output_structure_revision + 1;
            GpuScene next{this->context.runtime, this->context.shader_directory};
            ...
            next.resources.output_changes            = GpuSceneChange::Structure;
            next.resources.output_revision           = output_revision;
            next.resources.output_structure_revision = structure_revision;
            this->destroy();
            std::swap(this->resources, next.resources);
            return;
        }
    ...
    void GpuScene::end_external_updates(const scene::ResolvedSceneView scene, const vk::raii::CommandBuffer& command_buffer) {
    ...
    GpuAccelerationStructure GpuScene::build_bottom_level(const vk::AccelerationStructureGeometryKHR& geometry, const std::uint32_t primitive_count, const GpuMeshUpdateMode update_mode, const vk::raii::CommandBuffer& command_buffer, const std::uint32_t maximum_primitive_count) {
        ...
        const vk::AccelerationStructureBuildSizesInfoKHR sizes = this->context.runtime.device.logical.getAccelerationStructureBuildSizesKHR(vk::AccelerationStructureBuildTypeKHR::eDevice, build_info, build_capacity);
        ...
        result.acceleration_structure = vk::raii::AccelerationStructureKHR{
            this->context.runtime.device.logical,
        ...
        const std::array<const vk::AccelerationStructureBuildRangeInfoKHR*, 1> ranges{&range};
        command_buffer.buildAccelerationStructuresKHR(build_info, ranges);
        result.address = this->context.runtime.device.logical.getAccelerationStructureAddressKHR(vk::AccelerationStructureDeviceAddressInfoKHR{*result.acceleration_structure});
        return result;
    }
    ...
    GpuAccelerationStructure GpuScene::build_top_level(const std::span<const vk::AccelerationStructureInstanceKHR> instances, const std::uint32_t maximum_primitive_count, const vk::raii::CommandBuffer& command_buffer) {
        ...
        const vk::AccelerationStructureBuildSizesInfoKHR sizes = this->context.runtime.device.logical.getAccelerationStructureBuildSizesKHR(vk::AccelerationStructureBuildTypeKHR::eDevice, build_info, maximum_primitive_count);
