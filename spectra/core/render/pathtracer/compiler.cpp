...
module spectra.render.pathtracer.compiler;
...
namespace spectra::render {
        ...
        for (const scene::Geometry& geometry : source.geometries) {
            ...
            std::visit(
                [&copy](const auto& data) {
                    if constexpr (std::same_as<std::remove_cvref_t<decltype(data)>, scene::TriangleMeshGeometry>) copy.data = scene::TriangleMeshGeometry{.source = data.source};
                    else copy.data = data;
                },
                geometry.data);
    ...
    PathCompilationInput capture_path_compilation_input(GpuScene& source, const scene::ResolvedSceneView scene) {
        PathCompilationInput input{};
        const GpuSceneView gpu_scene = source.view();
        input.textures.resize(scene.resources.textures.size());
        for (std::size_t index = 0; index != scene.resources.textures.size(); ++index) {
            ...
            input.textures[index]        = {image.image_descriptor, image.sampler_descriptor};
        }
        input.volumes.reserve(gpu_scene.volumes.size());
        for (const GpuVolume& volume : gpu_scene.volumes) {
            PathCompilationInput::Volume& destination = input.volumes.emplace_back(volume.volume_id, volume.revision, volume.resolution);
        ...
        input.geometries.reserve(gpu_scene.geometries.size());
        for (const GpuGeometry& geometry : gpu_scene.geometries) input.geometries.push_back({geometry.positions_descriptor, geometry.indices_descriptor, geometry.normals_descriptor, geometry.tangents_descriptor, geometry.texture_coordinates_descriptor, geometry.vertex_count, geometry.index_count, geometry.vertex_capacity, geometry.index_capacity, geometry.attribute_mask});
        input.sphere_sets.reserve(gpu_scene.sphere_sets.size());
        for (const GpuSphereSet& spheres : gpu_scene.sphere_sets) input.sphere_sets.push_back({spheres.positions_descriptor, spheres.radii_descriptor, spheres.sphere_count, spheres.sphere_capacity});
        input.primitives.assign(gpu_scene.primitives.begin(), gpu_scene.primitives.end());
        if (gpu_scene.volume_region_geometry) {
            const GpuGeometry& region    = *gpu_scene.volume_region_geometry;
            input.volume_region_geometry = {region.positions_descriptor, region.indices_descriptor, region.normals_descriptor, region.tangents_descriptor, region.texture_coordinates_descriptor, region.vertex_count, region.index_count, region.vertex_capacity, region.index_capacity, region.attribute_mask};
        }
        input.acceleration_entities.assign(gpu_scene.acceleration_entities.begin(), gpu_scene.acceleration_entities.end());
        input.structure_revision = gpu_scene.structure_revision;
        return input;
    }


    std::unique_ptr<CompiledPathFilter> build_path_filter(const scene::Film& film, const PathTracerResources& resources) {
        ...
        if (filter.kind != scene::FilterKind::Box && filter.kind != scene::FilterKind::Triangle) {
            ...
            for (std::uint32_t y = 0; y != resolution[1]; ++y) {
                ...
                for (std::uint32_t x = 0; x != resolution[0]; ++x) {
                    ...
                    } else if (filter.kind == scene::FilterKind::Mitchell) value = evaluate_1d(2.0f * position_x / filter.radius.x) * evaluate_1d(2.0f * position_y / filter.radius.y);
                    else {
        ...
        std::unique_ptr<CompiledPathFilter> compiled = std::make_unique<CompiledPathFilter>();
        compiled->film                               = film;
        compiled->distribution                       = std::move(distribution);
        compiled->sensor_response.assign(sensor_response.begin(), sensor_response.end());
        if (film.sensor_response.empty())
            for (float& value : compiled->sensor_response) value /= cie_y_integral;
        compiled->resolution        = resolution;
        compiled->absolute_integral = absolute_integral;
        return compiled;
    }


    std::unique_ptr<CompiledPathSampler> build_path_sampler(const scene::Sampler& sampler, const PathTracerResources& resources) {
        ...
        std::unique_ptr<CompiledPathSampler> compiled = std::make_unique<CompiledPathSampler>();
        compiled->sampler                             = sampler;
        compiled->pixel_samples                       = std::move(pixel_samples);
        compiled->pixel_tile_size                     = pixel_tile_size;
        return compiled;
    }


    std::unique_ptr<CompiledPathScene> build_path_scene(const scene::ResolvedSceneView scene, const math::Bounds3 bounds, const PathCompilationInput& gpu, const PathTracerResources& resources, PathTracerPreparationState* progress) {
        ...
        for (const scene::Material& material : scene.resources.materials) {
            ...
            std::visit(
                [&](const auto& data) {
                    ...
                    if constexpr (std::same_as<std::remove_cvref_t<decltype(data)>, scene::InterfaceMaterialData>) kind = PathMaterialKind::Interface;
            ...
            visit_material = [&](const std::uint32_t index) {
                ...
                std::visit(
                    [&](const auto& data) {
                        ...
                        if constexpr (std::same_as<std::remove_cvref_t<decltype(data)>, scene::InterfaceMaterialData>) return;
                        else if constexpr (std::same_as<std::remove_cvref_t<decltype(data)>, scene::DiffuseMaterialData>) {
                            add_surface_maps();
                            ...
                            std::visit(
                                [&](const auto& optics) {
                                    ...
                                    } else add_parameter(optics.reflectance);
                                },
                                data.optics);
        ...
        std::vector<CompiledPathVolume> compiled_volume_resources{};
        ...
        compiled_volume_resources.reserve(scene.resources.volumes.size());
        for (const scene::Volume& volume : scene.resources.volumes) {
            ...
            const PathCompilationInput::Volume& shared = *std::ranges::find(gpu.volumes, volume.id, &PathCompilationInput::Volume::id);
            CompiledPathVolume compiled_volume{};
            compiled_volume.id       = volume.id;
            compiled_volume.revision = shared.revision;
            if (shared.cpu_data_stale && compiled_volume.revision.content != 0u) --compiled_volume.revision.content;
            ...
            const auto field               = [&resources, &shared](const std::string_view id) -> runtime::DescriptorHandle {
            ...
            const auto retain_majorant = [&](const auto values) -> runtime::DescriptorHandle {
            ...
            if (const auto* data = std::get_if<scene::GridVolume>(&volume.data)) {
                ...
                compiled_volume.resolution = data->resolution;
                ...
                if (rgb) {
                    const std::uint32_t flags         = (rendering.sigma_a_field.empty() ? 0u : 1u) | (rendering.sigma_s_field.empty() ? 0u : 2u) | (rendering.emission_field.empty() ? 0u : 4u) | (std::to_underlying(rendering.field_color_space) << 8) | (vertex_sampled(rendering.sigma_a_field) << 19u) | (vertex_sampled(rendering.sigma_s_field) << 20u) | (vertex_sampled(rendering.emission_field) << 21u);
                    ...
                    const std::uint32_t flags         = (!rendering.temperature_field.empty() ? 1u : 0u) | (!rendering.emission_scale_field.empty() ? 2u : 0u) | (vertex_sampled(rendering.density_field) << 16u) | (vertex_sampled(rendering.temperature_field) << 17u) | (vertex_sampled(rendering.emission_scale_field) << 18u);
                    result.majorant_metadata          = {16, 16, 16, flags};
            ...
            compiled_volume_resources.push_back(std::move(compiled_volume));
            if (progress) progress->report(PathTracerPreparationStage::CompilingMedia, ++compiled_medium_volume_count, medium_volume_count);
        }
        ...
        const auto light_image_statistics = [&](const scene::Texture& texture) {
            ...
            if (texture.color_space == scene::TextureColorSpace::Rec2020) luminance = {0.262700f, 0.677998f, 0.059302f};
            else if (texture.color_space == scene::TextureColorSpace::Aces2065_1) luminance = {0.3439664498f, 0.7281660966f, -0.0721325464f};
                ...
                [&](const auto& light) {
                    if constexpr (std::same_as<std::remove_cvref_t<decltype(light)>, scene::DiffuseAreaLight>) {
                        ...
                        if constexpr (std::same_as<std::remove_cvref_t<decltype(light)>, scene::PointLight>) {
                            ...
                            if constexpr (std::same_as<std::remove_cvref_t<decltype(light)>, scene::InfiniteLight>) lights.push_back(compiled);
                            else {
        ...
        for (const GpuAccelerationEntity entity : gpu.acceleration_entities) {
            if (entity.kind == GpuAccelerationEntityKind::Volume) {
                ...
                const PathCompilationInput::Geometry& geometry = gpu.volume_region_geometry;
                ...
                std::visit(
                    [&geometry_kind, &geometry_parameters](const auto& data) {
                        if constexpr (std::same_as<std::remove_cvref_t<decltype(data)>, scene::SphereGeometry>) {
                            ...
                            geometry_parameters = {data.radius, data.z_min, data.z_max, data.phi_max * std::numbers::pi_v<float> / 180.0f};
                        } else if constexpr (std::same_as<std::remove_cvref_t<decltype(data)>, scene::BoxGeometry>) geometry_kind = shader_semantics::geometry_box;
                        else if constexpr (std::same_as<std::remove_cvref_t<decltype(data)>, scene::RectangleGeometry>) geometry_kind = shader_semantics::geometry_rectangle;
                    },
                    geometry->data);
            if (area_light != std::numeric_limits<std::uint32_t>::max()) {
                const math::Transform transform = instance.transform * primitive.transform;
                if (mesh) {
                    const PathCompilationInput::Geometry& gpu_geometry = gpu.geometries[gpu_primitive.resource_index];
                    ...
                    const PathCompilationInput::SphereSet& gpu_spheres = gpu.sphere_sets[gpu_primitive.resource_index];
        ...
        std::unique_ptr<CompiledPathScene> compiled = std::make_unique<CompiledPathScene>();
        compiled->arena                             = std::move(arena.data);
        compiled->primitives                        = std::move(new_primitives);
        compiled->materials                         = std::move(new_materials);
        compiled->textures                          = std::move(new_textures);
        compiled->light_table                       = std::move(new_lights);
        compiled->light_shapes                      = std::move(new_light_shapes);
        compiled->light_distribution                = std::move(new_light_distributions);
        compiled->light_distribution_data           = std::move(new_light_distribution_data);
        compiled->portals                           = std::move(new_portals);
        compiled->light_bvh_nodes                   = std::move(new_light_bvh_nodes);
        compiled->light_bvh_bit_trails              = std::move(new_light_bvh_bit_trails);
        compiled->face_materials                    = std::move(new_face_materials);
        compiled->media                             = std::move(new_media);
        compiled->volumes                           = std::move(new_volumes);
        compiled->spectra                           = std::move(new_spectra);
        compiled->piecewise_spectra                 = std::move(new_piecewise_spectra);
        compiled->compiled_volumes                  = std::move(volumes);
        compiled->volume_resources                  = std::move(compiled_volume_resources);
        compiled->texture_count                     = static_cast<std::uint32_t>(texture_count);
        compiled->texture_stack_size                = maximum_texture_stack_size;
        compiled->material_texture_value_count      = maximum_material_texture_requests;
        compiled->light_count                       = light_count;
        compiled->light_bvh_node_count              = light_bvh_node_count;
        compiled->light_bvh_infinite_count          = light_bvh_infinite_count;
        compiled->camera_medium_index               = camera_medium_index;
        compiled->light_sampler                     = scene.transport.light_sampler;
        compiled->bounds                            = {bounds.minimum, bounds.maximum};
        compiled->instance_transforms.reserve(scene.resources.instances.size());
        for (const scene::Instance& instance : scene.resources.instances) compiled->instance_transforms.push_back(instance.transform);
        compiled->revision               = scene.revision;
        compiled->gpu_structure_revision = gpu.structure_revision;
        compiled->dynamic_area_lights    = std::move(dynamic_area_lights);
        return compiled;
    }
