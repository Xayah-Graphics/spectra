...
namespace spectra::scene {
    namespace {
        ...
        [[nodiscard]] PlySource open_ply_source(const std::filesystem::path& path) {
            ...
            while (std::getline(source.stream, line)) {
                ...
                if (command == "format") {
                    ...
                    if (identifier == "ascii") source.format = PlyFormat::Ascii;
                    else if (identifier == "binary_little_endian") source.format = PlyFormat::BinaryLittleEndian;
                    else if (identifier == "binary_big_endian") source.format = PlyFormat::BinaryBigEndian;
                    else throw std::runtime_error(std::format("Unsupported PLY format {}", identifier));
                    format_present = true;
                    ...
                    break;
                } else throw std::runtime_error(std::format("Unsupported PLY header declaration {}", command));
            }
        ...
        [[nodiscard]] Value ply_binary_value(std::istream& stream, const bool swap_bytes) {
            ...
            if constexpr (std::integral<Value>) return std::byteswap(value);
            else if constexpr (sizeof(Value) == sizeof(std::uint32_t)) return std::bit_cast<Value>(std::byteswap(std::bit_cast<std::uint32_t>(value)));
            else return std::bit_cast<Value>(std::byteswap(std::bit_cast<std::uint64_t>(value)));
        }
    ...
    void load_triangle_mesh_source(TriangleMeshGeometry& mesh, const std::filesystem::path& path) {
        ...
        if (has_nx) mesh.normals.assign(vertex_element->count, {});
        else mesh.normals.clear();
        if (uv_properties) mesh.texture_coordinates.assign(vertex_element->count, {});
        else mesh.texture_coordinates.clear();
        if (has_tangent_x) mesh.tangents.assign(vertex_element->count, {});
        else mesh.tangents.clear();
            ...
            for (std::uint64_t element_index = 0; element_index != element.count; ++element_index) {
                ...
                for (const PlyProperty& property : element.properties) {
                    if (property.count_type) {
                        ...
                        if (property.name == "x") position.x = static_cast<float>(value);
                        else if (property.name == "y") position.y = static_cast<float>(value);
                        else if (property.name == "z") position.z = static_cast<float>(value);
                        else if (property.name == "nx") mesh.normals[element_index].x = static_cast<float>(value);
                        else if (property.name == "ny") mesh.normals[element_index].y = static_cast<float>(value);
                        else if (property.name == "nz") mesh.normals[element_index].z = static_cast<float>(value);
                        else if (property.name == "tangent_x") mesh.tangents[element_index].x = static_cast<float>(value);
                        else if (property.name == "tangent_y") mesh.tangents[element_index].y = static_cast<float>(value);
                        else if (property.name == "tangent_z") mesh.tangents[element_index].z = static_cast<float>(value);
                        else if (uv_properties && property.name == (*uv_properties)[0]) mesh.texture_coordinates[element_index].x = static_cast<float>(value);
                        else if (uv_properties && property.name == (*uv_properties)[1]) mesh.texture_coordinates[element_index].y = static_cast<float>(value);
                    }
                }
        ...
        for (const PlyElement& element : source.elements)
            for (std::uint64_t element_index = 0; element_index != element.count; ++element_index)
                for (const PlyProperty& property : element.properties) {
                    ...
                    const double value = ply_number(source.stream, source.format, property.value_type);
                    if (element.name != "vertex") continue;
                    if (property.name == "x") spheres.positions[element_index].x = static_cast<float>(value);
                    else if (property.name == "y") spheres.positions[element_index].y = static_cast<float>(value);
                    else if (property.name == "z") spheres.positions[element_index].z = static_cast<float>(value);
                    else if (property.name == "radius") spheres.radii[element_index] = static_cast<float>(value);
                }
    }
    ...
    void load_image_source(ImageTexture& image, const TextureColorSpace color_space, const std::filesystem::path& path) {
        if (path.extension() == ".png") load_png_source(image, color_space, path);
        else if (path.extension() == ".exr") load_exr_source(image, path);
