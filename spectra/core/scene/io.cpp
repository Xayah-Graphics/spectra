...
namespace spectra::scene {
    namespace {
        constexpr std::uint32_t current_scene_format_version = 37;
        ...
        [[nodiscard]] std::string visualization_view_kind_name(const SimulationVisualization& view) {
            return std::visit(
                []<typename Type>(const Type&) -> std::string {
        ...
        [[nodiscard]] std::string simulation_parameter_kind_name(const SimulationParameterKind kind) {
            switch (kind) {
            case SimulationParameterKind::Boolean: return "boolean";
            case SimulationParameterKind::Integer: return "integer";
            case SimulationParameterKind::Float: return "float";
            case SimulationParameterKind::Float3: return "float3";
            case SimulationParameterKind::Enumeration: return "enumeration";
            }
            std::unreachable();
        }

        void write_simulation(KdlWriter& writer, const SimulationSetup& setup) {
            std::string line{"simulation"};
            ...
            for (const SimulationSystem& system : setup.systems) {
                ...
                for (const SimulationParameterSetting& parameter : system.parameters) {
                    std::string parameter_line = std::format("parameter {} {}", kdl_string(parameter.parameter_id), simulation_parameter_kind_name(parameter.value.kind));
                    if (parameter.value.kind == SimulationParameterKind::Boolean) parameter_line += std::format(" #{}", parameter.value.integer != 0 ? "true" : "false");
                    else if (parameter.value.kind == SimulationParameterKind::Integer || parameter.value.kind == SimulationParameterKind::Enumeration) parameter_line += std::format(" {}", parameter.value.integer);
                    else if (parameter.value.kind == SimulationParameterKind::Float) parameter_line += std::format(" {}", parameter.value.floating[0]);
                ...
                for (const SimulationOutputBinding& binding : system.scene_bindings) writer.line(std::format("scene-bind {} {}", kdl_string(binding.output_id), binding.resource_id));
                for (const SimulationVisualization& view : system.visualizations) {
                    std::string view_line = std::format("visualize {} {} kind={} depth={}", kdl_string(view.output_id), kdl_string(view.name), kdl_string(visualization_view_kind_name(view)), kdl_string(depth_buffer_mode_name(view.depth_mode)));
        ...
        [[nodiscard]] std::string serialize_scene_kdl(const Scene& scene) {
            ...
            if (scene.simulation) write_simulation(writer, *scene.simulation);
            writer.end();
    ...
    namespace {
        ...
        [[nodiscard]] SimulationParameterKind read_dynamic_parameter_kind(const std::string_view value) {
            if (value == "boolean") return SimulationParameterKind::Boolean;
            if (value == "integer") return SimulationParameterKind::Integer;
            if (value == "float") return SimulationParameterKind::Float;
            if (value == "float3") return SimulationParameterKind::Float3;
            if (value == "enumeration") return SimulationParameterKind::Enumeration;
            throw std::runtime_error(std::format("Unknown simulation parameter kind {}", value));
        }
        ...
        [[nodiscard]] VisualizationCompositionDomain read_visualization_composition_domain(const std::string_view value) {
            ...
            throw std::runtime_error(std::format("Unknown visualization domain: {}", value));
        }
        ...
        [[nodiscard]] SimulationSetup read_simulation(const kdl::Node& node) {
            SimulationSetup setup{.seed = kdl_number_property<std::uint64_t>(node, u8"seed", 0)};
            ...
            for (const kdl::Node& system_node : node.children()) {
                ...
                for (const kdl::Node& child : system_node.children()) {
                    if (child.name() == u8"parameter") {
                        SimulationParameterSetting parameter{
                            .parameter_id = kdl_value_text(child.args()[0]),
                        };
                        parameter.value.kind = read_dynamic_parameter_kind(kdl_value_text(child.args()[1]));
                        if (parameter.value.kind == SimulationParameterKind::Boolean) parameter.value.integer = child.args()[2].as<bool>() ? 1 : 0;
                        else if (parameter.value.kind == SimulationParameterKind::Integer || parameter.value.kind == SimulationParameterKind::Enumeration) parameter.value.integer = kdl_number<std::int64_t>(child.args()[2]);
                        else if (parameter.value.kind == SimulationParameterKind::Float) parameter.value.floating[0] = kdl_number<double>(child.args()[2]);
                        else
        ...
        [[nodiscard]] Scene parse_scene(const std::filesystem::path& path) {
            ...
            for (const kdl::Node& node : root.children()) {
                ...
                else if (node.name() == u8"sphere-sets") read_sphere_sets(scene.resources, node);
                ...
                else if (node.name() == u8"neural-fields") read_neural_fields(scene.resources, node);
                ...
                else if (node.name() == u8"simulation") scene.simulation = read_simulation(node);
