...
module spectra.editor.ui.inspector;

import std;

namespace spectra::editor {
    InspectorPanel::InspectorPanel(Document& document, Viewport& viewport) noexcept : document{document}, viewport{viewport} {}

    void InspectorPanel::draw(const scene::EntityReference entity) {
        const scene::Scene& evaluated          = this->document.evaluated;
        render::EntityDiagnostics& diagnostics = this->viewport.settings.entity_diagnostics.try_emplace(entity).first->second;
        const scene::EntityKind entity_type    = scene::entity_kind(entity);
        bool geometry{};
        bool spheres{};
        bool area_emitter{};
        bool medium_boundary{};
        if (entity_type == scene::EntityKind::Instance || entity_type == scene::EntityKind::AreaEmitter) {
            const scene::InstanceId instance_id = entity_type == scene::EntityKind::Instance ? std::get<scene::InstanceId>(entity.data) : std::get<scene::EntityReference::AreaEmitter>(entity.data).instance;
        ...
        ImGui::BeginDisabled(!this->viewport.settings.guides_visible);
        if (entity_type == scene::EntityKind::Instance || entity_type == scene::EntityKind::AreaEmitter || entity_type == scene::EntityKind::ParticleSet || entity_type == scene::EntityKind::Volume || entity_type == scene::EntityKind::NeuralField) ImGui::Checkbox("Bounds", &diagnostics.bounds);
        if (entity_type == scene::EntityKind::Instance) {
        ...
        } else if (entity_type == scene::EntityKind::AreaEmitter) ImGui::Checkbox("Emitter surface", &diagnostics.area_emitter);
            ...
            if (ImGui::Checkbox("Occupancy Grid", &field_diagnostics.occupancy_grid)) {
                this->document.update_neural_field_diagnostics(field.id, std::move(field_diagnostics));
            }
        } else if (entity_type == scene::EntityKind::Light) ImGui::Checkbox("Light guide", &diagnostics.light_guide);

        const bool instance_diagnostics  = entity_type == scene::EntityKind::Instance;
        ...
        const bool emitter_diagnostics   = (instance_diagnostics && area_emitter || entity_type == scene::EntityKind::AreaEmitter) && diagnostics.area_emitter;
        ...
        if (entity_type == scene::EntityKind::ParticleSet) {
            ImGui::Spacing();
            ImGui::Separator();
            ImGui::Spacing();
            this->draw_particle(*std::ranges::find(evaluated.resources.particle_sets, std::get<scene::ParticleSetId>(entity.data), &scene::ParticleSet::id));
        } else if (entity_type == scene::EntityKind::Volume) {
            ImGui::Spacing();
            ImGui::Separator();
            ImGui::Spacing();
            this->draw_volume(*std::ranges::find(evaluated.resources.volumes, std::get<scene::VolumeId>(entity.data), &scene::Volume::id));
        }
    }

    void InspectorPanel::draw_particle(const scene::ParticleSet& particles) {
        ...
        if (visualization.display == scene::ParticleDisplayMode::Points) visualization_changed = ImGui::DragFloat("Point size", &visualization.point_size, 0.25f, 1.0f, 32.0f, "%.2f px") || visualization_changed;
        else visualization_changed = ImGui::DragFloat("Radius scale", &visualization.radius_scale, 0.01f, 0.01f, 10.0f, "%.3g") || visualization_changed;
        constexpr const char* depth_modes[] = {"Tested", "X-Ray", "Overlay"};
        ...
        if (visualization_changed) {
            this->document.update_particle_set_visualization(particles.id, std::move(visualization));
        }
        if (diagnostics_changed) {
            this->document.update_particle_set_diagnostics(particles.id, std::move(diagnostics));
        }
        ImGui::PopID();
    }
    ...
    void InspectorPanel::draw_volume(const scene::Volume& volume) {
        ...
        if (changed) {
            this->document.update_volume_diagnostics(volume.id, std::move(diagnostics));
        }
        ImGui::PopID();
    }
