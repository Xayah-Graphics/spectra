...
namespace spectra
{
    ...
    public:
        // Film Interface
        SPECTRA_CPU_GPU inline void AddSample(Point2i pFilm, SampledSpectrum L,
                                           const SampledWavelengths& lambda,
                                           const VisibleSurface* visibleSurface,
                                           Float weight);

        SPECTRA_CPU_GPU inline Bounds2f SampleBounds() const;
        ...
        SPECTRA_CPU_GPU inline SampledWavelengths SampleWavelengths(Float u) const;

        SPECTRA_CPU_GPU inline Point2i FullResolution() const;
        SPECTRA_CPU_GPU inline Bounds2i PixelBounds() const;
        SPECTRA_CPU_GPU inline Float Diagonal() const;

        void WriteImage(ImageMetadata metadata, Float splatScale = 1);

        SPECTRA_CPU_GPU inline RGB ToOutputRGB(SampledSpectrum L,
        ...
        SPECTRA_CPU_GPU inline Filter GetFilter() const;
        SPECTRA_CPU_GPU inline const PixelSensor* GetPixelSensor() const;
        ...
        SPECTRA_CPU_GPU inline void ResetPixel(Point2i p);
    };
} // namespace spectra
