...
int main(int argc, char* argv[]) {
    if (argc != 2) error("usage: soac <soac filename>\n");
    ...
    getToken = [&](bool eofOk) -> OptionalString {
        ...
        if (!isalpha(s[0]) && s[0] != '_') return OptionalString(s);
    ...
    auto isFlatType = [&](std::string type) { return flatTypes.find(type) != flatTypes.end(); };
    ...
    auto expect = [&](const char* str) {
        ...
        if (!tok) error("Premature end of file; expected \"%s\".\n", str);
        if (tok != str) error("Syntax error: expected \"%s\".\n", str);
    };
    ...
    while (true) {
        ...
        if (tok == "flat") {
            ...
            if (flatTypes.find(type) != flatTypes.end()) error("%s flat type redeclared.\n", type.c_str());
            ...
            if (!isalpha(soa.type[0])) error("%s: invalid type identifier.\n", soa.type.c_str());

            if (soaTypeExists(soa.type)) error("%s: type redefined.\n", soa.type.c_str());

            OptionalString tok = getToken(false);
            if (tok == "<") {
                tok              = getToken(false);
                soa.templateType = (std::string) tok;
                if (!isalpha(soa.templateType[0])) error("%s: invalid type identifier.\n", soa.templateType.c_str());
                expect(">");
                expect("{");
            } else if (tok == ";") {
            ...
            while (true) {
                ...
                // how to SOA pointers..
                if (member.numPointers == 0 && member.type != soa.templateType && flatTypes.find(member.type) == flatTypes.end() && !soaTypeExists(member.type)) error("%s: undefined type\n", member.type.c_str());
                ...
                while (true) {
                    ...
                    if (tok == "[") {
                        ...
                        member.arraySizes[member.arraySizes.size() - 1] = (std::string) tok;
                        expect("]");
                        tok = getToken(false);
                    }
                ...
                if (member.names.empty()) error("No members specified after type declaration.\n");
    ...
    for (const auto& soa : soaTypes) {
        ...
        for (const auto& member : soa.members) {
            for (int i = 0; i < member.names.size(); ++i) {
                ...
                if (!member.arraySizes[i].empty()) {
                    if (isFlatType(member.type) || member.numPointers > 0)
                        printf("    %s * /*SPECTRA_RESTRICT*/ %s[%s];\n", member.GetType().c_str(), name.c_str(), member.arraySizes[i].c_str());
                    else
                        printf("    SOA<%s> %s[%s];\n", member.type.c_str(), name.c_str(), member.arraySizes[i].c_str());
                } else {
                    if (isFlatType(member.type) || member.numPointers > 0)
                        printf("    %s * SPECTRA_RESTRICT %s;\n", member.GetType().c_str(), name.c_str());
                    else
