...
namespace spectra::pathtracer {
    namespace {
        ...
        [[nodiscard]] Transform ToPathtracerTransform(const spectra::math::Transform& transform) {
            return Transform(ToPathtracerMatrix(transform.matrix), ToPathtracerMatrix(transform.inverse));
        }

        [[nodiscard]] const RGBColorSpace* ToPathtracerColorSpace(spectra::scene::ColorSpace colorSpace) {
            switch (colorSpace) {
            case spectra::scene::ColorSpace::sRGB: return RGBColorSpace::SRGB();
            case spectra::scene::ColorSpace::DCI_P3: return RGBColorSpace::DCI_P3();
            case spectra::scene::ColorSpace::Rec2020: return RGBColorSpace::Rec2020();
            case spectra::scene::ColorSpace::ACES2065_1: return RGBColorSpace::ACES2065_1();
            }
            throw std::runtime_error("Unknown Spectra scene color space.");
        }

        [[nodiscard]] FileLoc ToFileLoc(const spectra::scene::SceneSourceLocation& source, const std::string& fallback) {
        ...
        [[nodiscard]] std::string SourceString(const spectra::scene::SceneSourceLocation& source) {
            return std::format("{}:{}:{}", source.filename, source.line, source.column);
        }

        [[nodiscard]] std::string FormatGpuSupportReport(const spectra::scene::SceneTranslationReport& report) {
            std::string message = "Scene is not supported by the current GPU pathtracer:";
            for (const spectra::scene::SceneDiagnostic& diagnostic : report.diagnostics) message += std::format("\n  {}: {}", SourceString(diagnostic.source), diagnostic.message);
            return message;
        }
        ...
        [[nodiscard]] std::string OneStringParameter(const std::vector<spectra::scene::SceneParameter>& parameters, const std::string& name, std::string fallback) {
            for (const spectra::scene::SceneParameter& parameter : parameters) {
        ...
        void AddDiagnostic(spectra::scene::SceneTranslationReport* report, spectra::scene::SceneSourceLocation source, std::string message) {
        ...
        void ValidateTransform(spectra::scene::SceneTranslationReport* report, const spectra::scene::SceneTransformSet& transform, const spectra::scene::SceneSourceLocation& source, const std::string_view owner) {
        ...
        void ValidateEntityType(spectra::scene::SceneTranslationReport* report, const spectra::scene::SceneEntity& entity, const std::set<std::string>& supported, const std::string_view kind) {
        ...
        void ValidateProbeFeatureType(spectra::scene::SceneTranslationReport* report, const spectra::scene::SceneProbeFeature& feature, const std::set<std::string>& supported, const std::string_view kind) {
        ...
        void AppendParameterValues(ParsedParameter* parsedParameter, const spectra::scene::SceneParameter& parameter) {
        ...
        public:
            PathtracerSceneCompiler(const spectra::scene::SceneSnapshot& sourceScene, CompiledPathtracerScene& compiledScene, const RenderConfig& config, std::optional<Point2i> resolutionOverride) : source(sourceScene), compiled(compiledScene), renderConfig(config), filmResolutionOverride(resolutionOverride), location(sourceScene.source) {}

            void Compile() {
                const spectra::scene::SceneTranslationReport supportReport = AnalyzePathtracerSceneSupport(this->source);
                ...
                for (const spectra::scene::SceneMaterial& material : this->source.materials) this->AddMaterial(material);
                for (const spectra::scene::SceneTexture& texture : this->source.textures) this->AddTexture(texture);
                for (const spectra::scene::SceneMedium& medium : this->source.media) this->AddMedium(medium);

                this->compiled.media   = this->CreateMedia();
                this->compiled.sampler = this->CreateSampler();
                this->compiled.camera  = this->CreateCamera();

                for (const spectra::scene::SceneLight& light : this->source.lights) this->AddLight(light);
                for (const spectra::scene::SceneShape& shape : this->source.shapes) this->AddShape(shape);
                for (const spectra::scene::SceneObjectDefinition& definition : this->source.objectDefinitions) this->AddObjectDefinition(definition);
                for (const spectra::scene::SceneObjectInstance& instance : this->source.objectInstances) this->AddObjectInstance(instance);
        ...
        private:
            [[nodiscard]] ParameterDictionary MakeParameterDictionary(const std::vector<spectra::scene::SceneParameter>& parameters, spectra::scene::ColorSpace colorSpace) const {
            ...
            [[nodiscard]] PathtracerSceneEntity MakeEntity(const spectra::scene::SceneEntity& entity) const {
            ...
            void OverrideIntegerParameter(spectra::scene::SceneEntity* entity, std::string name, int value) const {
                for (spectra::scene::SceneParameter& parameter : entity->parameters) {
                    if (parameter.type != "integer" || parameter.name != name) continue;
                    parameter.values = std::vector<int>{value};
                    return;
                }
                entity->parameters.push_back(spectra::scene::SceneParameter{
                    .type       = "integer",
                    .name       = std::move(name),
            ...
            [[nodiscard]] bool HasIntegerParameter(const spectra::scene::SceneEntity& entity, const std::string_view name) const {
            ...
            void ApplySamplerDefaults(spectra::scene::SceneEntity* sampler) const {
            ...
            void RegisterResourceNames() {
                for (const spectra::scene::SceneMaterial& material : this->source.materials) {
                    this->RequireUniqueName(this->materialNames, "material", material.name);
                    this->materialNames.insert(material.name);
                }
                for (const spectra::scene::SceneMedium& medium : this->source.media) {
                    this->RequireUniqueName(this->mediumNames, "medium", medium.name);
                    this->mediumNames.insert(medium.name);
                }
                for (const spectra::scene::SceneObjectDefinition& definition : this->source.objectDefinitions) {
            ...
            [[nodiscard]] Transform SceneTransform(const spectra::scene::SceneTransformSet& transform, const spectra::scene::SceneSourceLocation& source) const {
            ...
            [[nodiscard]] Transform RenderFromObjectTransform(const spectra::scene::SceneTransformSet& transform, const spectra::scene::SceneSourceLocation& source) const {
                return this->RenderFromWorldTransform() * this->SceneTransform(transform, source);
            }

            [[nodiscard]] PathtracerShapeSceneEntity MakeShapeEntity(const spectra::scene::SceneShape& shape) const {
            ...
            void SetRenderSettings(const spectra::scene::SceneRenderSettings& settings) {
                ...
                spectra::scene::SceneEntity film            = settings.film;
                spectra::scene::SceneEntity sampler         = settings.sampler;
            ...
            void AddMaterial(const spectra::scene::SceneMaterial& material) {
            ...
            void AddTexture(const spectra::scene::SceneTexture& texture) {
                this->RequireRenderSettings();
            ...
            void AddMedium(const spectra::scene::SceneMedium& medium) {
                this->RequireRenderSettings();
            ...
            void AddLight(const spectra::scene::SceneLight& light) {
                this->RequireRenderSettings();
            ...
            void AddShape(const spectra::scene::SceneShape& shape) {
                this->RequireRenderSettings();
                this->compiled.shapes.push_back(this->MakeShapeEntity(shape));
            }

            void AddObjectDefinition(const spectra::scene::SceneObjectDefinition& definition) {
                this->RequireRenderSettings();
                ...
                for (const spectra::scene::SceneShape& shape : definition.shapes) {
            ...
            void AddObjectInstance(const spectra::scene::SceneObjectInstance& instance) {
                this->RequireRenderSettings();
            ...
            const spectra::scene::SceneSnapshot& source;
    ...
    spectra::scene::SceneTranslationReport AnalyzePathtracerSceneProbe(const spectra::scene::SceneProbeReport& probe) {
        ...
        spectra::scene::SceneTranslationReport report{.target = "Spectra Pathtracer"};
        for (const spectra::scene::SceneProbeFeature& feature : probe.features) {
            switch (feature.category) {
            case spectra::scene::SceneProbeFeatureCategory::PixelFilter: ValidateProbeFeatureType(&report, feature, supportedFilters, "pixel filter"); break;
            case spectra::scene::SceneProbeFeatureCategory::Film: ValidateProbeFeatureType(&report, feature, supportedFilms, "film"); break;
            case spectra::scene::SceneProbeFeatureCategory::Camera: ValidateProbeFeatureType(&report, feature, supportedCameras, "camera"); break;
            case spectra::scene::SceneProbeFeatureCategory::Sampler: ValidateProbeFeatureType(&report, feature, supportedSamplers, "sampler"); break;
            case spectra::scene::SceneProbeFeatureCategory::Integrator: ValidateProbeFeatureType(&report, feature, supportedIntegrators, "integrator"); break;
            case spectra::scene::SceneProbeFeatureCategory::Accelerator: ValidateProbeFeatureType(&report, feature, supportedAccelerators, "accelerator"); break;
            case spectra::scene::SceneProbeFeatureCategory::Material: ValidateProbeFeatureType(&report, feature, supportedMaterials, "material"); break;
            case spectra::scene::SceneProbeFeatureCategory::Texture:
                ...
                break;
            case spectra::scene::SceneProbeFeatureCategory::Medium: ValidateProbeFeatureType(&report, feature, supportedMedia, "medium"); break;
            case spectra::scene::SceneProbeFeatureCategory::Light: ValidateProbeFeatureType(&report, feature, supportedLights, "light"); break;
            case spectra::scene::SceneProbeFeatureCategory::AreaLight: ValidateProbeFeatureType(&report, feature, supportedAreaLights, "area light"); break;
            case spectra::scene::SceneProbeFeatureCategory::Shape:
                if (feature.type != "objectinstance") ValidateProbeFeatureType(&report, feature, supportedShapes, "shape");
                break;
            case spectra::scene::SceneProbeFeatureCategory::LightSampler:
                ...
                break;
            case spectra::scene::SceneProbeFeatureCategory::Option:
                ...
                break;
            case spectra::scene::SceneProbeFeatureCategory::AnimatedTransform:
    ...
    spectra::scene::SceneTranslationReport AnalyzePathtracerSceneSupport(const spectra::scene::SceneSnapshot& scene) {
        static const std::set<std::string> supportedFilters{"box", "gaussian", "mitchell", "sinc", "triangle"};
        static const std::set<std::string> supportedFilms{"rgb", "gbuffer", "spectral"};
        static const std::set<std::string> supportedCameras{"perspective", "orthographic", "realistic", "spherical"};
        static const std::set<std::string> supportedSamplers{"zsobol", "paddedsobol", "halton", "sobol", "pmj02bn", "independent", "stratified"};
        ...
        for (const spectra::scene::SceneOption& option : scene.renderSettings.options) AddDiagnostic(&report, option.source, std::format("PBRT Option \"{}\" is represented by the scene document but is not wired into the current pathtracer runtime", option.name));
        ...
        for (const spectra::scene::SceneMaterial& material : scene.materials) {
        ...
        for (const spectra::scene::SceneMedium& medium : scene.media) {
        ...
        for (const spectra::scene::SceneTexture& texture : scene.textures) {
        ...
        const auto validateShape = [&report, &materials, &media](const spectra::scene::SceneShape& shape, const std::string_view owner) {
        ...
        for (const spectra::scene::SceneLight& light : scene.lights) {
        ...
        for (const spectra::scene::SceneShape& shape : scene.shapes) validateShape(shape, std::format("shape \"{}\"", shape.name));

        std::set<std::string> objectDefinitions{};
        for (const spectra::scene::SceneObjectDefinition& definition : scene.objectDefinitions) {
        ...
        for (const spectra::scene::SceneObjectInstance& instance : scene.objectInstances) {
    ...
    std::unique_ptr<CompiledPathtracerScene> CompilePathtracerScene(const spectra::scene::SceneSnapshot& scene, const RenderConfig& config, pstd::pmr::memory_resource* memoryResource, std::optional<Point2i> filmResolutionOverride) {
