...
namespace spectra
{
    SPECTRA_CPU_GPU pstd::optional<CameraRay> Camera::GenerateRay(
    ...
    SPECTRA_CPU_GPU void Camera::Approximate_dp_dxy(Point3f p, Normal3f n, Float time,
    ...
    CameraTransform::CameraTransform(const AnimatedTransform& worldFromCamera)
    {
        ...
        default:
            SPECTRA_FATAL("Unhandled rendering coordinate space");
        }
    ...
    SPECTRA_CPU_GPU pstd::optional<CameraRayDifferential> Camera::GenerateRayDifferential(
        CameraSample sample, SampledWavelengths& lambda) const
    {
    ...
    SPECTRA_CPU_GPU pstd::optional<CameraRayDifferential> CameraBase::GenerateRayDifferential(
                          ...
                          const FileLoc* loc, Allocator alloc)
    {
        ...
        if (!camera)
            throw std::runtime_error(spectra::diagnostics::Format(loc, "%s: unable to create camera.", name));
    ...
    SPECTRA_CPU_GPU pstd::optional<CameraRay> OrthographicCamera::GenerateRay(
        CameraSample sample, SampledWavelengths& lambda) const
    {
    ...
    SPECTRA_CPU_GPU pstd::optional<CameraRayDifferential> OrthographicCamera::GenerateRayDifferential(
        CameraSample sample, SampledWavelengths& lambda) const
    {
    ...
    SPECTRA_CPU_GPU pstd::optional<CameraRay> PerspectiveCamera::GenerateRay(
        CameraSample sample, SampledWavelengths& lambda) const
    {
        // Compute raster and camera sample positions
        Point3f pFilm = Point3f(sample.pFilm.x, sample.pFilm.y, 0);
    ...
    SPECTRA_CPU_GPU pstd::optional<CameraRayDifferential> PerspectiveCamera::GenerateRayDifferential(
        CameraSample sample, SampledWavelengths& lambda) const
    {
        // Compute raster and camera sample positions
        Point3f pFilm = Point3f(sample.pFilm.x, sample.pFilm.y, 0);
    ...
    SPECTRA_CPU_GPU pstd::optional<CameraWiSample> PerspectiveCamera::SampleWi(
