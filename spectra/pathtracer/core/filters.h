...
namespace spectra
{
    ...
    public:
        ...
        {
            return {
                Point2f(SampleTent(u[0], radius.x), SampleTent(u[1], radius.y)),
                Float(1)
            };
        }
    ...
    SPECTRA_CPU_GPU inline Float Filter::Evaluate(Point2f p) const
    {
        auto eval = [&](auto ptr) { return ptr->Evaluate(p); };
        return Dispatch(eval);
    }

    SPECTRA_CPU_GPU inline FilterSample Filter::Sample(Point2f u) const
    {
        auto sample = [&](auto ptr) { return ptr->Sample(u); };
        return Dispatch(sample);
    }

    SPECTRA_CPU_GPU inline Vector2f Filter::Radius() const
    {
        auto radius = [&](auto ptr) { return ptr->Radius(); };
        return Dispatch(radius);
    }

    SPECTRA_CPU_GPU inline Float Filter::Integral() const
    {
