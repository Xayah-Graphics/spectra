...
namespace spectra {
    ...
    public:
        ...
        SPECTRA_CPU_GPU
        HomogeneousMajorantIterator(Float tMin, Float tMax, SampledSpectrum sigma_maj) : seg{tMin, tMax, sigma_maj}, called(false) {}
    ...
    public:
        ...
        SPECTRA_CPU_GPU
        DDAMajorantIterator(Ray ray, Float tMin, Float tMax, const MajorantGrid* grid, SampledSpectrum sigma_t) : tMin(tMin), tMax(tMax), grid(grid), sigma_t(sigma_t) {
    ...
    public:
        ...
        MediumProperties SamplePoint(Point3f p, const SampledWavelengths& lambda) const {
            ...
            auto convert            = [=] SPECTRA_CPU_GPU(RGBUnboundedSpectrum s) { return s.Sample(lambda); };
            ...
            SampledSpectrum Le(0.f);
            if (LeGrid && LeScale > 0) {
                auto convert = [=] SPECTRA_CPU_GPU(RGBIlluminantSpectrum s) { return s.Sample(lambda); };
    ...
    template <typename F>
    SPECTRA_CPU_GPU SampledSpectrum SampleT_maj(Ray ray, Float tMax, Float u, RNG& rng, const SampledWavelengths& lambda, F callback) {
