...
namespace spectra {
    ...
    Sphere* Sphere::Create(const Transform* renderFromObject, const Transform* objectFromRender, bool reverseOrientation, const ParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        ...
        return alloc.new_object<Sphere>(renderFromObject, objectFromRender, reverseOrientation, radius, zmin, zmax, phimax);
    }
    ...
    Disk* Disk::Create(const Transform* renderFromObject, const Transform* objectFromRender, bool reverseOrientation, const ParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
    ...
    Cylinder* Cylinder::Create(const Transform* renderFromObject, const Transform* objectFromRender, bool reverseOrientation, const ParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        Float radius = parameters.GetOneFloat("radius", 1);
        Float zmin   = parameters.GetOneFloat("zmin", -1);
        Float zmax   = parameters.GetOneFloat("zmax", 1);
        Float phimax = parameters.GetOneFloat("phimax", 360);
        return alloc.new_object<Cylinder>(renderFromObject, objectFromRender, reverseOrientation, radius, zmin, zmax, phimax);
    }
    ...
    pstd::vector<Shape> CreateCurve(const Transform* renderFromObject, const Transform* objectFromRender, bool reverseOrientation, pstd::span<const Point3f> c, Float w0, Float w1, CurveType type, pstd::span<const Normal3f> norm, int splitDepth, Allocator alloc) {
        CurveCommon* common = alloc.new_object<CurveCommon>(c, w0, w1, type, norm, renderFromObject, objectFromRender, reverseOrientation);
    ...
    __host__ __device__ Bounds3f Curve::Bounds() const {
        ...
        Float width[2] = {Lerp(uMin, common->width[0], common->width[1]), Lerp(uMax, common->width[0], common->width[1])};
    ...
    bool Curve::IntersectRay(const Ray& r, Float tMax, pstd::optional<ShapeIntersection>* si) const {
        ...
        pstd::array<Point3f, 4> cp = {rayFromObject(cpObj[0]), rayFromObject(cpObj[1]), rayFromObject(cpObj[2]), rayFromObject(cpObj[3])};

        // Test ray against bound of projected control points
        Float maxWidth       = std::max(Lerp(uMin, common->width[0], common->width[1]), Lerp(uMax, common->width[0], common->width[1]));
        ...
        if (!Overlaps(rayBounds, curveBounds)) return false;

        // Compute refinement depth for curve, _maxDepth_
        Float L0 = 0;
        for (int i = 0; i < 2; ++i) L0 = std::max(L0, std::max(std::max(std::abs(cp[i].x - 2 * cp[i + 1].x + cp[i + 2].x), std::abs(cp[i].y - 2 * cp[i + 1].y + cp[i + 2].y)), std::abs(cp[i].z - 2 * cp[i + 1].z + cp[i + 2].z)));
        int maxDepth = 0;
        if (L0 > 0) {
        ...
        return RecursiveIntersect(ray, tMax, cpSpan, Inverse(rayFromObject), uMin, uMax, maxDepth, si);
    }

    bool Curve::RecursiveIntersect(const Ray& ray, Float tMax, pstd::span<const Point3f> cp, const Transform& objectFromRay, Float u0, Float u1, int depth, pstd::optional<ShapeIntersection>* si) const {
        ...
        if (depth > 0) {
            ...
            for (int seg = 0; seg < 2; ++seg) {
                ...
                Float maxWidth                = std::max(Lerp(u[seg], common->width[0], common->width[1]), Lerp(u[seg + 1], common->width[0], common->width[1]));
                ...
                curveBounds                   = Expand(curveBounds, 0.5f * maxWidth);
                Bounds3f rayBounds(Point3f(0, 0, 0), Point3f(0, 0, Length(ray.d) * tMax));
                if (!Overlaps(rayBounds, curveBounds)) continue;

                // Recursively test ray-segment intersection
                bool hit = RecursiveIntersect(ray, tMax, cps, objectFromRay, u[seg], u[seg + 1], depth - 1, si);
                if (hit && !si) return true;
            }
            ...
            Vector3f dpcdw;
            Point3f pc         = EvaluateCubicBezier(pstd::span<const Point3f>(cp), Clamp(w, 0, 1), &dpcdw);
            Float ptCurveDist2 = Sqr(pc.x) + Sqr(pc.y);

            if (ptCurveDist2 > Sqr(hitWidth) * 0.25f) return false;
            if (pc.z < 0 || pc.z > rayLength * tMax) return false;
            ...
            if (si) {
                ...
                if (si->has_value() && tHit > si->value().tHit) return false;
                ...
                bool flipNormal = common->reverseOrientation ^ common->transformSwapsHandedness;
                Point3fi pi(ray(tHit), pError);
                SurfaceInteraction intr(pi, {u, v}, -ray.d, dpdu, dpdv, Normal3f(), Normal3f(), ray.time, flipNormal);
    ...
    pstd::vector<Shape> Curve::Create(const Transform* renderFromObject, const Transform* objectFromRender, bool reverseOrientation, const ParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        ...
        for (int seg = 0; seg < nSegments; ++seg) {
            ...
            auto c = CreateCurve(renderFromObject, objectFromRender, reverseOrientation, segCpBezier, Lerp(Float(seg) / Float(nSegments), width0, width1), Lerp(Float(seg + 1) / Float(nSegments), width0, width1), type, nspan, sd, alloc);
    ...
    BilinearPatch::BilinearPatch(const BilinearPatchMesh* mesh, int blpIndex) : mesh(mesh), blpIndex(blpIndex) {
        ...
        if (IsRectangle(mesh))
            ...
            area = 0;
            for (int i = 0; i < na; ++i)
                for (int j = 0; j < na; ++j) area += 0.5f * Length(Cross(p[i + 1][j + 1] - p[i][j], p[i + 1][j] - p[i][j + 1]));
        }
    }
    ...
    __host__ __device__ DirectionCone BilinearPatch::NormalBounds() const {
        ...
        if (p00 == p10 || p10 == p11 || p11 == p01 || p01 == p00) {
            ...
            if (mesh->n) {
                Normal3f ns = (mesh->n[v[0]] + mesh->n[v[1]] + mesh->n[v[2]] + mesh->n[v[3]]) / 4;
                n           = FaceForward(n, ns);
            } else if (mesh->reverseOrientation ^ mesh->transformSwapsHandedness)
                n = -n;
    ...
    __host__ __device__ pstd::optional<ShapeIntersection> BilinearPatch::Intersect(const Ray& ray, Float tMax) const {
        ...
        pstd::optional<BilinearIntersection> blpIsect = IntersectBilinearPatch(ray, tMax, p00, p10, p01, p11);
        if (!blpIsect) return {};
        SurfaceInteraction intr = InteractionFromIntersection(mesh, blpIndex, blpIsect->uv, ray.time, -ray.d);
    ...
    __host__ __device__ pstd::optional<ShapeSample> BilinearPatch::Sample(Point2f u) const {
        ...
        if (LengthSquared(dpdu) == 0 || LengthSquared(dpdv) == 0) return {};
    ...
    __host__ __device__ pstd::optional<ShapeSample> BilinearPatch::Sample(const ShapeSampleContext& ctx, Point2f u) const {
        ...
        Point2f uv(Dot(p - p00, eu) / DistanceSquared(p10, p00), Dot(p - p00, ev) / DistanceSquared(p01, p00));
    ...
    __host__ __device__ Float BilinearPatch::PDF(const ShapeSampleContext& ctx, Vector3f wi) const {
        ...
        if (!IsRectangle(mesh) || mesh->imageDistribution || SphericalQuadArea(v00, v10, v11, v01) <= MinSphericalSampleArea) {
            ...
            Float pdf = PDF(isect->intr) * (DistanceSquared(ctx.p(), isect->intr.p()) / AbsDot(isect->intr.n, -wi));
            ...
            if (ctx.ns != Normal3f(0, 0, 0)) {
                ...
                Point2f u = InvertSphericalRectangleSample(ctx.p(), p00, p10 - p00, p01 - p00, isect->intr.p());
    ...
    pstd::vector<Shape> Shape::Create(const std::string& name, const Transform* renderFromObject, const Transform* objectFromRender, bool reverseOrientation, const ParameterDictionary& parameters, const std::map<std::string, FloatTexture>& floatTextures, const pathtracer::RenderConfig& config, const FileLoc* loc, MeshBufferCache& bufferCache, Allocator alloc) {
        ...
        // Create remaining single _Shape_ types
            ...
            shapes = {Cylinder::Create(renderFromObject, objectFromRender, reverseOrientation, parameters, loc, alloc)};
        } else if (name == "disk") {
