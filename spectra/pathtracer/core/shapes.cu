...
namespace spectra
{
    ...
    // Triangle Functions
    SPECTRA_CPU_GPU pstd::optional<TriangleIntersection> IntersectTriangle(const Ray& ray, Float tMax,
    ...
    SPECTRA_CPU_GPU pstd::optional<ShapeSample> Curve::Sample(const ShapeSampleContext& ctx,
                                                           Point2f u) const
    {
        SPECTRA_FATAL("Curve::Sample not implemented.");
        return {};
    ...
    SPECTRA_CPU_GPU DirectionCone BilinearPatch::NormalBounds() const
    {
        ...
        Float cosTheta = std::min(std::min(Dot(n, n00), Dot(n, n01)),
                                  std::min(Dot(n, n10), Dot(n, n11)));
        return DirectionCone(n, Clamp(cosTheta, -1, 1));
    }

    SPECTRA_CPU_GPU pstd::optional<ShapeIntersection> BilinearPatch::Intersect(const Ray& ray,
                                                                            Float tMax) const
    {
        const BilinearPatchMesh* mesh = GetMesh();
        // Get bilinear patch vertices in _p00_, _p01_, _p10_, and _p11_
    ...
    SPECTRA_CPU_GPU pstd::optional<ShapeSample> BilinearPatch::Sample(Point2f u) const
    {
        ...
        return ShapeSample{
            Interaction(Point3fi(p, pError), n, st),
    ...
    SPECTRA_CPU_GPU pstd::optional<ShapeSample> BilinearPatch::Sample(const ShapeSampleContext& ctx,
                                                                   Point2f u) const
    {
        const BilinearPatchMesh* mesh = GetMesh();
        // Get bilinear patch vertices in _p00_, _p01_, _p10_, and _p11_
        ...
        Allocator alloc)
    {
        ...
        {
            shapes = {
                Sphere::Create(renderFromObject, objectFromRender, reverseOrientation,
        ...
        {
            shapes = {
                Cylinder::Create(renderFromObject, objectFromRender, reverseOrientation,
                                 parameters, loc, alloc)
            };
        }
        else if (name == "disk")
        {
            shapes = {
                Disk::Create(renderFromObject, objectFromRender, reverseOrientation,
                             parameters, loc, alloc)
            };
        }
