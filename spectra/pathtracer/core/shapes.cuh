#ifndef SPECTRA_PATHTRACER_CORE_SHAPES_H
#define SPECTRA_PATHTRACER_CORE_SHAPES_H
...
namespace spectra {
    ...
    struct ShapeSampleContext {
        ...
        __host__ __device__ ShapeSampleContext(Point3fi pi, Normal3f n, Normal3f ns, Float time) : pi(pi), n(n), ns(ns), time(time) {}

        __host__ __device__ ShapeSampleContext(const SurfaceInteraction& si) : pi(si.pi), n(si.n), ns(si.shading.n), time(si.time) {}

        __host__ __device__ ShapeSampleContext(const MediumInteraction& mi) : pi(mi.pi), time(mi.time) {}

        __host__ __device__ Point3f p() const {
            return Point3f(pi);
        }

        __host__ __device__ Point3f OffsetRayOrigin(Vector3f w) const;
        __host__ __device__ Point3f OffsetRayOrigin(Point3f pt) const;
        __host__ __device__ Ray SpawnRay(Vector3f w) const;
    ...
    __host__ __device__ inline Point3f ShapeSampleContext::OffsetRayOrigin(Vector3f w) const {
    ...
    __host__ __device__ inline Point3f ShapeSampleContext::OffsetRayOrigin(Point3f pt) const {
        return OffsetRayOrigin(pt - p());
    }

    __host__ __device__ inline Ray ShapeSampleContext::SpawnRay(Vector3f w) const {
    ...
    public:
        ...
        static Sphere* Create(const Transform* renderFromObject, const Transform* objectFromRender, bool reverseOrientation, const ParameterDictionary& parameters, const FileLoc* loc, Allocator alloc);


        Sphere(const Transform* renderFromObject, const Transform* objectFromRender, bool reverseOrientation, Float radius, Float zMin, Float zMax, Float phiMax) : renderFromObject(renderFromObject), objectFromRender(objectFromRender), reverseOrientation(reverseOrientation), transformSwapsHandedness(renderFromObject->SwapsHandedness()), radius(radius), zMin(Clamp(std::min(zMin, zMax), -radius, radius)), zMax(Clamp(std::max(zMin, zMax), -radius, radius)), thetaZMin(std::acos(Clamp(std::min(zMin, zMax) / radius, -1, 1))), thetaZMax(std::acos(Clamp(std::max(zMin, zMax) / radius, -1, 1))), phiMax(Radians(Clamp(phiMax, 0, 360))) {}
        ...
        __host__ __device__ pstd::optional<ShapeIntersection> Intersect(const Ray& ray, Float tMax = Infinity) const {
        ...
        __host__ __device__ pstd::optional<QuadricIntersection> BasicIntersect(const Ray& r, Float tMax) const {
            Float phi;
            Point3f pHit;
        ...
        __host__ __device__ bool IntersectP(const Ray& r, Float tMax = Infinity) const {
            return BasicIntersect(r, tMax).has_value();
        }

        __host__ __device__ SurfaceInteraction InteractionFromIntersection(const QuadricIntersection& isect, Vector3f wo, Float time) const {
            ...
            Vector3f dpdv  = (thetaZMax - thetaZMin) * Vector3f(pHit.z * cosPhi, pHit.z * sinPhi, -radius * sinTheta);

            // Compute sphere $\dndu$ and $\dndv$
            Vector3f d2Pduu = -phiMax * phiMax * Vector3f(pHit.x, pHit.y, 0);
            Vector3f d2Pduv = (thetaZMax - thetaZMin) * pHit.z * phiMax * Vector3f(-sinPhi, cosPhi, 0.);
        ...
        __host__ __device__ pstd::optional<ShapeSample> Sample(Point2f u) const;

        __host__ __device__ Float PDF(const Interaction&) const {
            return 1 / Area();
        }
        ...
        __host__ __device__ Float PDF(const ShapeSampleContext& ctx, Vector3f wi) const {
            Point3f pCenter = (*renderFromObject)(Point3f(0, 0, 0));
            Point3f pOrigin = ctx.OffsetRayOrigin(pCenter);
            if (DistanceSquared(pOrigin, pCenter) <= Sqr(radius)) {
    ...
    public:
        ...
        __host__ __device__ pstd::optional<ShapeIntersection> Intersect(const Ray& ray, Float tMax = Infinity) const;
        __host__ __device__ bool IntersectP(const Ray& ray, Float tMax = Infinity) const;
        ...
        __host__ __device__ static SurfaceInteraction InteractionFromIntersection(const TriangleMesh* mesh, int triIndex, TriangleIntersection ti, Float time, Vector3f wo) {
            const int* v = &mesh->vertexIndices[3 * triIndex];
            Point3f p0 = mesh->p[v[0]], p1 = mesh->p[v[1]], p2 = mesh->p[v[2]];
            ...
            if (mesh->reverseOrientation ^ mesh->transformSwapsHandedness) isect.n = isect.shading.n = -isect.n;
            ...
            if (mesh->n || mesh->s) {
                ...
                if (mesh->s) {
                    ...
                    if (LengthSquared(ss) == 0) ss = isect.dpdu;
                } else
                    ss = isect.dpdu;
    ...
    struct CurveCommon {
        ...
        CurveCommon(pstd::span<const Point3f> c, Float w0, Float w1, CurveType type, pstd::span<const Normal3f> norm, const Transform* renderFromObject, const Transform* objectFromRender, bool reverseOrientation);
    ...
    public:
        ...
        static pstd::vector<Shape> Create(const Transform* renderFromObject, const Transform* objectFromRender, bool reverseOrientation, const ParameterDictionary& parameters, const FileLoc* loc, Allocator alloc);
    ...
    __host__ __device__ inline pstd::optional<BilinearIntersection> IntersectBilinearPatch(const Ray& ray, Float tMax, Point3f p00, Point3f p10, Point3f p01, Point3f p11) {
        ...
        Float eps = gamma(30) * (MaxComponentValue(Abs(ray.o)) + MaxComponentValue(Abs(ray.d)) + MaxComponentValue(Abs(p00)) + MaxComponentValue(Abs(p10)) + MaxComponentValue(Abs(p01)) + MaxComponentValue(Abs(p11)));
        ...
        if (0 <= u1 && u1 <= 1) {
            ...
            Float v1 = Determinant(SquareMatrix<3>(deltao.x, ray.d.x, perp.x, deltao.y, ray.d.y, perp.y, deltao.z, ray.d.z, perp.z));
            Float t1 = Determinant(SquareMatrix<3>(deltao.x, ud.x, perp.x, deltao.y, ud.y, perp.y, deltao.z, ud.z, perp.z));
        ...
        if (0 <= u2 && u2 <= 1 && u2 != u1) {
            ...
            Vector3f perp   = Cross(ray.d, ud);
            Float p2        = LengthSquared(perp);
            Float v2        = Determinant(SquareMatrix<3>(deltao.x, ray.d.x, perp.x, deltao.y, ray.d.y, perp.y, deltao.z, ray.d.z, perp.z));
            Float t2        = Determinant(SquareMatrix<3>(deltao.x, ud.x, perp.x, deltao.y, ud.y, perp.y, deltao.z, ud.z, perp.z));
            t2 /= p2;
    ...
    public:
        ...
        __host__ __device__ static SurfaceInteraction InteractionFromIntersection(const BilinearPatchMesh* mesh, int blpIndex, Point2f uv, Float time, Vector3f wo) {
    ...
    private:
        ...
        __host__ __device__ bool IsRectangle(const BilinearPatchMesh* mesh) const {
            ...
            Float d2[4]     = {DistanceSquared(p00, pCenter), DistanceSquared(p01, pCenter), DistanceSquared(p10, pCenter), DistanceSquared(p11, pCenter)};
            for (int i = 1; i < 4; ++i)
                if (std::abs(d2[i] - d2[0]) / d2[0] > 1e-4f) return false;
            return true;
        }
    ...
    __host__ __device__ inline Bounds3f Shape::Bounds() const {
        auto bounds = [&](auto ptr) { return ptr->Bounds(); };
        return Dispatch(bounds);
    }

    __host__ __device__ inline pstd::optional<ShapeIntersection> Shape::Intersect(const Ray& ray, Float tMax) const {
        auto intr = [&](auto ptr) { return ptr->Intersect(ray, tMax); };
        return Dispatch(intr);
    }

    __host__ __device__ inline bool Shape::IntersectP(const Ray& ray, Float tMax) const {
        auto intr = [&](auto ptr) { return ptr->IntersectP(ray, tMax); };
        return Dispatch(intr);
    }

    __host__ __device__ inline Float Shape::Area() const {
        auto area = [&](auto ptr) { return ptr->Area(); };
        return Dispatch(area);
    }

    __host__ __device__ inline pstd::optional<ShapeSample> Shape::Sample(Point2f u) const {
        auto sample = [&](auto ptr) { return ptr->Sample(u); };
        return Dispatch(sample);
    }

    __host__ __device__ inline Float Shape::PDF(const Interaction& in) const {
        auto pdf = [&](auto ptr) { return ptr->PDF(in); };
        return Dispatch(pdf);
    }

    __host__ __device__ inline pstd::optional<ShapeSample> Shape::Sample(const ShapeSampleContext& ctx, Point2f u) const {
        auto sample = [&](auto ptr) { return ptr->Sample(ctx, u); };
        return Dispatch(sample);
    }

    __host__ __device__ inline Float Shape::PDF(const ShapeSampleContext& ctx, Vector3f wi) const {
        auto pdf = [&](auto ptr) { return ptr->PDF(ctx, wi); };
        return Dispatch(pdf);
    }

    __host__ __device__ inline DirectionCone Shape::NormalBounds() const {
        auto nb = [&](auto ptr) { return ptr->NormalBounds(); };
        return Dispatch(nb);
    }
} // namespace spectra

#endif // SPECTRA_PATHTRACER_CORE_SHAPES_H
