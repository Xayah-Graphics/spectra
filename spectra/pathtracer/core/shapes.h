...
namespace spectra {
    ...
    struct ShapeSampleContext {
        ...
        SPECTRA_CPU_GPU
        ShapeSampleContext(Point3fi pi, Normal3f n, Normal3f ns, Float time) : pi(pi), n(n), ns(ns), time(time) {}

        SPECTRA_CPU_GPU
        ShapeSampleContext(const SurfaceInteraction& si) : pi(si.pi), n(si.n), ns(si.shading.n), time(si.time) {}
    ...
    SPECTRA_CPU_GPU inline Point3f ShapeSampleContext::OffsetRayOrigin(Vector3f w) const {
    ...
    SPECTRA_CPU_GPU inline Point3f ShapeSampleContext::OffsetRayOrigin(Point3f pt) const {
        return OffsetRayOrigin(pt - p());
    }

    SPECTRA_CPU_GPU inline Ray ShapeSampleContext::SpawnRay(Vector3f w) const {
    ...
    extern SPECTRA_GPU pstd::vector<const TriangleMesh*>* allTriangleMeshesGPU;
#endif
    ...
    SPECTRA_CPU_GPU
    pstd::optional<TriangleIntersection> IntersectTriangle(const Ray& ray, Float tMax, Point3f p0, Point3f p1, Point3f p2);
...
#if defined(__CUDACC__)
    extern SPECTRA_GPU pstd::vector<const BilinearPatchMesh*>* allBilinearMeshesGPU;
#endif
    ...
    SPECTRA_CPU_GPU inline pstd::optional<BilinearIntersection> IntersectBilinearPatch(const Ray& ray, Float tMax, Point3f p00, Point3f p10, Point3f p01, Point3f p11) {
    ...
    SPECTRA_CPU_GPU inline Bounds3f Shape::Bounds() const {
        auto bounds = [&](auto ptr) { return ptr->Bounds(); };
        return Dispatch(bounds);
    }

    SPECTRA_CPU_GPU inline pstd::optional<ShapeIntersection> Shape::Intersect(const Ray& ray, Float tMax) const {
        auto intr = [&](auto ptr) { return ptr->Intersect(ray, tMax); };
        return Dispatch(intr);
    }

    SPECTRA_CPU_GPU inline bool Shape::IntersectP(const Ray& ray, Float tMax) const {
        auto intr = [&](auto ptr) { return ptr->IntersectP(ray, tMax); };
        return Dispatch(intr);
    }

    SPECTRA_CPU_GPU inline Float Shape::Area() const {
        auto area = [&](auto ptr) { return ptr->Area(); };
        return Dispatch(area);
    }

    SPECTRA_CPU_GPU inline pstd::optional<ShapeSample> Shape::Sample(Point2f u) const {
        auto sample = [&](auto ptr) { return ptr->Sample(u); };
        return Dispatch(sample);
    }

    SPECTRA_CPU_GPU inline Float Shape::PDF(const Interaction& in) const {
        auto pdf = [&](auto ptr) { return ptr->PDF(in); };
        return Dispatch(pdf);
    }

    SPECTRA_CPU_GPU inline pstd::optional<ShapeSample> Shape::Sample(const ShapeSampleContext& ctx, Point2f u) const {
        auto sample = [&](auto ptr) { return ptr->Sample(ctx, u); };
        return Dispatch(sample);
    }

    SPECTRA_CPU_GPU inline Float Shape::PDF(const ShapeSampleContext& ctx, Vector3f wi) const {
        auto pdf = [&](auto ptr) { return ptr->PDF(ctx, wi); };
        return Dispatch(pdf);
    }

    SPECTRA_CPU_GPU inline DirectionCone Shape::NormalBounds() const {
