...
namespace spectra {
    TextureMapping2D TextureMapping2D::Create(const ParameterDictionary& parameters, const Transform& renderFromTexture, const FileLoc* loc, Allocator alloc) {
    ...
    TextureMapping3D TextureMapping3D::Create(const ParameterDictionary& parameters, const Transform& renderFromTexture, const FileLoc* loc, Allocator alloc) {
        return alloc.new_object<PointTransformMapping>(Inverse(renderFromTexture));
    }


    FloatConstantTexture* FloatConstantTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        return alloc.new_object<FloatConstantTexture>(parameters.GetOneFloat("value", 1.f));
    }


    SpectrumConstantTexture* SpectrumConstantTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, SpectrumType spectrumType, const FileLoc* loc, Allocator alloc) {
    ...
    FloatBilerpTexture* FloatBilerpTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        // Initialize 2D texture mapping _map_ from _tp_
        TextureMapping2D map = TextureMapping2D::Create(parameters, renderFromTexture, loc, alloc);

        return alloc.new_object<FloatBilerpTexture>(map, parameters.GetOneFloat("v00", 0.f), parameters.GetOneFloat("v01", 1.f), parameters.GetOneFloat("v10", 0.f), parameters.GetOneFloat("v11", 1.f));
    }


    SpectrumBilerpTexture* SpectrumBilerpTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, SpectrumType spectrumType, const FileLoc* loc, Allocator alloc) {
        // Initialize 2D texture mapping _map_ from _tp_
        TextureMapping2D map = TextureMapping2D::Create(parameters, renderFromTexture, loc, alloc);
        ...
        return alloc.new_object<SpectrumBilerpTexture>(map, parameters.GetOneSpectrum("v00", zero, spectrumType, alloc), parameters.GetOneSpectrum("v01", one, spectrumType, alloc), parameters.GetOneSpectrum("v10", zero, spectrumType, alloc), parameters.GetOneSpectrum("v11", one, spectrumType, alloc));
    }
    ...
    Float Checkerboard(TextureEvalContext ctx, TextureMapping2D map2D, TextureMapping3D map3D) {
        ...
        auto bf = [&](Float x, Float r) -> Float {
            if (pstd::floor(x - r) == pstd::floor(x + r)) return 1 - 2 * (int(pstd::floor(x)) & 1);
    ...
    FloatCheckerboardTexture* FloatCheckerboardTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        ...
        if (dim == 2) {
            ...
            TextureMapping3D map = TextureMapping3D::Create(parameters, renderFromTexture, loc, alloc);
            return alloc.new_object<FloatCheckerboardTexture>(nullptr, map, tex1, tex2);
        }
    }


    SpectrumCheckerboardTexture* SpectrumCheckerboardTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, SpectrumType spectrumType, const FileLoc* loc, Allocator alloc) {
        int dim = parameters.GetOneInt("dimension", 2);
        if (dim != 2 && dim != 3) {
            throw std::runtime_error(spectra::diagnostics::Format(loc, "%d dimensional checkerboard texture not supported", dim));
            return nullptr;
        ...
        SpectrumTexture tex1 = parameters.GetSpectrumTexture("tex1", one, spectrumType, alloc);
        SpectrumTexture tex2 = parameters.GetSpectrumTexture("tex2", zero, spectrumType, alloc);
        if (dim == 2) {
            // Initialize 2D texture mapping _map_ from _tp_
            TextureMapping2D map = TextureMapping2D::Create(parameters, renderFromTexture, loc, alloc);
    ...
    bool InsidePolkaDot(Point2f st) {
        ...
        if (Noise(sCell + .5f, tCell + .5f) > 0) {
            ...
            if (LengthSquared(dst) < Sqr(radius)) return true;
        }
        return false;
    }

    FloatDotsTexture* FloatDotsTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        // Initialize 2D texture mapping _map_ from _tp_
        TextureMapping2D map = TextureMapping2D::Create(parameters, renderFromTexture, loc, alloc);

        return alloc.new_object<FloatDotsTexture>(map, parameters.GetFloatTexture("inside", 1.f, alloc), parameters.GetFloatTexture("outside", 0.f, alloc));
    }


    SpectrumDotsTexture* SpectrumDotsTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, SpectrumType spectrumType, const FileLoc* loc, Allocator alloc) {
        // Initialize 2D texture mapping _map_ from _tp_
        TextureMapping2D map = TextureMapping2D::Create(parameters, renderFromTexture, loc, alloc);
        Spectrum zero        = alloc.new_object<ConstantSpectrum>(0.);
        Spectrum one         = alloc.new_object<ConstantSpectrum>(1.);

        return alloc.new_object<SpectrumDotsTexture>(map, parameters.GetSpectrumTexture("inside", one, spectrumType, alloc), parameters.GetSpectrumTexture("outside", zero, spectrumType, alloc));
    }


    // FBmTexture Method Definitions
    FBmTexture* FBmTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        // Initialize 3D texture mapping _map_ from _tp_
        TextureMapping3D map = TextureMapping3D::Create(parameters, renderFromTexture, loc, alloc);
    ...
    FloatImageTexture* FloatImageTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        ...
        if (!wrapMode) throw std::runtime_error(spectra::diagnostics::Format("%s: wrap mode unknown", wrapString));
    ...
    MarbleTexture* MarbleTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        // Initialize 3D texture mapping _map_ from _tp_
        TextureMapping3D map = TextureMapping3D::Create(parameters, renderFromTexture, loc, alloc);
        return alloc.new_object<MarbleTexture>(map, parameters.GetOneInt("octaves", 8), parameters.GetOneFloat("roughness", .5f), parameters.GetOneFloat("scale", 1.f), parameters.GetOneFloat("variation", .2f));
    }

    // MixTexture Method Definitions


    FloatMixTexture* FloatMixTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        return alloc.new_object<FloatMixTexture>(parameters.GetFloatTexture("tex1", 0.f, alloc), parameters.GetFloatTexture("tex2", 1.f, alloc), parameters.GetFloatTexture("amount", 0.5f, alloc));
    }

    SpectrumMixTexture* SpectrumMixTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, SpectrumType spectrumType, const FileLoc* loc, Allocator alloc) {
        Spectrum zero = alloc.new_object<ConstantSpectrum>(0.);
        Spectrum one  = alloc.new_object<ConstantSpectrum>(1.);
        return alloc.new_object<SpectrumMixTexture>(parameters.GetSpectrumTexture("tex1", zero, spectrumType, alloc), parameters.GetSpectrumTexture("tex2", one, spectrumType, alloc), parameters.GetFloatTexture("amount", 0.5f, alloc));
    }


    FloatDirectionMixTexture* FloatDirectionMixTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        Vector3f dir = parameters.GetOneVector3f("dir", Vector3f(0, 1, 0));
        dir          = Normalize(renderFromTexture(dir));
        return alloc.new_object<FloatDirectionMixTexture>(parameters.GetFloatTexture("tex1", 0.f, alloc), parameters.GetFloatTexture("tex2", 1.f, alloc), dir);
    }

    SpectrumDirectionMixTexture* SpectrumDirectionMixTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, SpectrumType spectrumType, const FileLoc* loc, Allocator alloc) {
        Spectrum zero = alloc.new_object<ConstantSpectrum>(0.);
        Spectrum one  = alloc.new_object<ConstantSpectrum>(1.);
        Vector3f dir  = parameters.GetOneVector3f("dir", Vector3f(0, 1, 0));
        dir           = Normalize(renderFromTexture(dir));
    ...
    int PtexTextureBase::SampleTexture(TextureEvalContext ctx, float result[3]) const {
        ...
        PtexFilter* filter = PtexFilter::getFilter(texture, opts);
        int nc             = texture->numChannels();

        int firstChan = 0;
        filter->eval(result, firstChan, nc, ctx.faceIndex, ctx.uv[0], ctx.uv[1], ctx.dudx, ctx.dvdx, ctx.dudy, ctx.dvdy);
        filter->release();
        texture->release();
        ...
        for (int c = 0; c < nc; ++c) result[c] *= scale;
    ...
    FloatTexture FloatScaledTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
    ...
    SpectrumTexture SpectrumScaledTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, SpectrumType spectrumType, const FileLoc* loc, Allocator alloc) {
        Spectrum one        = alloc.new_object<ConstantSpectrum>(1.);
    ...
    WindyTexture* WindyTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        // Initialize 3D texture mapping _map_ from _tp_
        TextureMapping3D map = TextureMapping3D::Create(parameters, renderFromTexture, loc, alloc);
    ...
    WrinkledTexture* WrinkledTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        // Initialize 3D texture mapping _map_ from _tp_
        TextureMapping3D map = TextureMapping3D::Create(parameters, renderFromTexture, loc, alloc);
    ...
    static cudaMipmappedArray_t createSingleChannelTextureArray(const Image& image, const RGBColorSpace* colorSpace, int* nMIPMapLevels) {
        ...
        switch (image.Format()) {
        case PixelFormat::U256: channelDesc = cudaCreateChannelDesc(8, 0, 0, 0, cudaChannelFormatKindUnsigned); break;
        case PixelFormat::Half: channelDesc = cudaCreateChannelDesc(16, 0, 0, 0, cudaChannelFormatKindFloat); break;
        case PixelFormat::Float: channelDesc = cudaCreateChannelDesc(32, 0, 0, 0, cudaChannelFormatKindFloat); break;
        default: SPECTRA_FATAL("Unhandled PixelFormat");
        }
        ...
        *nMIPMapLevels = mipmap.Levels();
        ...
        for (int level = 0; level < mipmap.Levels(); ++level) {
            ...
            int pitch;
            switch (image.Format()) {
            case PixelFormat::U256: pitch = levelImage.Resolution().x * sizeof(uint8_t); break;
            case PixelFormat::Half: pitch = levelImage.Resolution().x * sizeof(Half); break;
            case PixelFormat::Float: pitch = levelImage.Resolution().x * sizeof(float); break;
            default: SPECTRA_FATAL("Unhandled PixelFormat");
            }
            CUDA_CHECK(cudaMemcpy2DToArray(levelArray, /* offset */ 0, 0, levelImage.RawPointer({0, 0}), pitch, pitch, levelImage.Resolution().y, cudaMemcpyHostToDevice));
        }
    ...
    GPUSpectrumImageTexture* GPUSpectrumImageTexture::Create(const Transform& renderFromTexture, const TextureParameterDictionary& parameters, SpectrumType spectrumType, const FileLoc* loc, Allocator alloc) {
                                ...
                                for (int level = 0; level < mipmap.Levels(); ++level) {
                                    ...
                                    for (int y = 0; y < levelImage.Resolution().y; ++y)
                                        for (int x = 0; x < levelImage.Resolution().x; ++x) {
                                            for (int c = 0; c < 3; ++c) rgba[offset++] = ((uint8_t*) levelImage.RawPointer({x, y}))[c];
                                            rgba[offset++] = 255;
                                        }

                                    int pitch = levelImage.Resolution().x * 4 * sizeof(uint8_t);

                                    CUDA_CHECK(cudaMemcpy2DToArray(levelArray,
                                        /* offset */ 0, 0, rgba.data(), pitch, pitch, levelImage.Resolution().y, cudaMemcpyHostToDevice));
                                }
                                break;
                            }
                        case PixelFormat::Half:
                            {
                                ...
                                for (int level = 0; level < mipmap.Levels(); ++level) {
                                    ...
                                    for (int y = 0; y < levelImage.Resolution().y; ++y)
                                        for (int x = 0; x < levelImage.Resolution().x; ++x) {
                                            for (int c = 0; c < 3; ++c) rgba[offset++] = Half(levelImage.GetChannel({x, y}, c));
                        ...
                        case PixelFormat::Float:
                            {
                                ...
                                for (int level = 0; level < mipmap.Levels(); ++level) {
                                    ...
                                    for (int y = 0; y < levelImage.Resolution().y; ++y)
                                        for (int x = 0; x < levelImage.Resolution().x; ++x) {
                                            for (int c = 0; c < 3; ++c) rgba[offset++] = levelImage.GetChannel({x, y}, c);
                                            rgba[offset++] = 1.f;
                                        }
        ...
        texDesc.mipmapFilterMode    = (filter == "trilinear" || filter == "ewa" || filter == "EWA") ? cudaFilterModeLinear : cudaFilterModePoint;
                ...
                auto allOnes              = [&]() {
                    for (int y = 0; y < image.Resolution().y; ++y)
                        for (int x = 0; x < image.Resolution().x; ++x)
                            if (image.GetChannels({x, y}, rgbaDesc)[3] != 1) return false;
                    return true;
                };
                if (rgbaDesc && !allOnes()) {
                    ...
                    for (int y = 0; y < image.Resolution().y; ++y)
                        for (int x = 0; x < image.Resolution().x; ++x) avgImage.SetChannel({x, y}, 0, image.GetChannels({x, y}, rgbDesc).Average());

                    image          = std::move(avgImage);
                    convertedImage = true;
                } else
                    throw std::runtime_error(spectra::diagnostics::Format(loc, "%s: %d channel image, without RGB channels.", filename, image.NChannels()));
            }
    ...
    FloatTexture FloatTexture::Create(const std::string& name, const Transform& renderFromTexture, const TextureParameterDictionary& parameters, const FileLoc* loc, Allocator alloc) {
        ...
        if (!tex) throw std::runtime_error(spectra::diagnostics::Format(loc, "%s: unable to create texture.", name));
    ...
    SpectrumTexture SpectrumTexture::Create(const std::string& name, const Transform& renderFromTexture, const TextureParameterDictionary& parameters, SpectrumType spectrumType, const FileLoc* loc, Allocator alloc) {
        ...
        if (name == "constant")
            ...
            tex = SpectrumScaledTexture::Create(renderFromTexture, parameters, spectrumType, loc, alloc);
        else if (name == "mix")
            tex = SpectrumMixTexture::Create(renderFromTexture, parameters, spectrumType, loc, alloc);
        else if (name == "directionmix")
            tex = SpectrumDirectionMixTexture::Create(renderFromTexture, parameters, spectrumType, loc, alloc);
        else if (name == "bilerp")
            ...
            tex = SpectrumDotsTexture::Create(renderFromTexture, parameters, spectrumType, loc, alloc);
        else if (name == "marble")
