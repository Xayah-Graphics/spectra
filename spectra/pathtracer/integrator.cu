...
namespace spectra::pathtracer {
    ...
    static LONG WINAPI handle_windows_exception(PEXCEPTION_POINTERS info) {
        ...
        case EXCEPTION_ACCESS_VIOLATION: report_windows_exception("Access violation--terminating execution"); break;
        case EXCEPTION_ARRAY_BOUNDS_EXCEEDED: report_windows_exception("Array bounds violation--terminating execution"); break;
        case EXCEPTION_DATATYPE_MISALIGNMENT: report_windows_exception("Accessed misaligned data--terminating execution"); break;
        case EXCEPTION_STACK_OVERFLOW: report_windows_exception("Stack overflow--terminating execution"); break;
        default: std::fprintf(stderr, "Spectra: Program generated exception %d--terminating execution\n", int(info->ExceptionRecord->ExceptionCode)); std::fflush(stderr);
        }
    ...
    static void updateMaterialNeeds(Material m, pstd::array<bool, Material::NumTags()>* haveBasicEvalMaterial, pstd::array<bool, Material::NumTags()>* haveUniversalEvalMaterial, bool* haveSubsurface, bool* haveMedia) {
        ...
        *haveSubsurface |= m.HasSubsurfaceScattering();
        ...
        if (m.CanEvaluateTextures(BasicTextureEvaluator()) && (!displace || BasicTextureEvaluator().CanEvaluate({displace}, {})))
            (*haveBasicEvalMaterial)[m.Tag()] = true;
        else
            (*haveUniversalEvalMaterial)[m.Tag()] = true;
    }

    WavefrontPathtracer::WavefrontPathtracer(pstd::pmr::memory_resource* memoryResource, scene::Scene& scene) : memoryResource(memoryResource) {
        ...
        for (const auto& m : media) haveLights |= m.second.IsEmissive();
        ...
        if (allLights.size() == 1) lightSamplerName = "uniform";
    ...
    Float WavefrontPathtracer::Render() {
        ...
        std::chrono::steady_clock::time_point lastProgressReport = std::chrono::steady_clock::now();
        for (int sampleIndex = firstSampleIndex; sampleIndex < lastSampleIndex; ++sampleIndex) {
            ...
            if (!Options->quiet) {
                ...
                if (lastSample || now - lastProgressReport >= std::chrono::seconds(1)) {
                    ...
                    double elapsedSeconds = std::chrono::duration<double>(now - renderStart).count();
    ...
    void WavefrontPathtracer::RenderSample(Bounds2i pixelBounds, Transform cameraMotion, int sampleIndex) {
        for (int y0 = pixelBounds.pMin.y; y0 < pixelBounds.pMax.y; y0 += scanlinesPerPass) {
            ...
            Do("Reset ray queue", SPECTRA_CPU_GPU_LAMBDA() { cameraRayQueue->Reset(); });
            ...
            for (int wavefrontDepth = 0; true; ++wavefrontDepth) {
                ...
                aggregate->IntersectClosest(maxQueueSize, CurrentRayQueue(wavefrontDepth), escapedRayQueue, hitAreaLightQueue, basicEvalMaterialQueue, universalEvalMaterialQueue, mediumSampleQueue, NextRayQueue(wavefrontDepth));
    ...
    void WavefrontPathtracer::ResetFilm(Bounds2i pixelBounds) {
        Vector2i resolution = pixelBounds.Diagonal();
        ParallelFor(
            "Reset pixels", resolution.x * resolution.y, SPECTRA_CPU_GPU_LAMBDA(int i) {
    ...
    void WavefrontPathtracer::HandleEscapedRays() {
        ...
        ForAllQueued(
            "Handle escaped rays", escapedRayQueue, maxQueueSize, SPECTRA_CPU_GPU_LAMBDA(const spectra::EscapedRayWorkItem w) {
                ...
                for (const auto& light : *infiniteLights) {
                    if (SampledSpectrum Le = light.Le(Ray(w.rayo, w.rayd), w.lambda); Le) {
                        ...
                        if (w.depth == 0 || w.specularBounce) {
                            ...
                            SampledSpectrum r_l    = w.r_l * lightChoicePDF * light.PDF_Li(ctx, w.rayd, true);
    ...
    void WavefrontPathtracer::HandleEmissiveIntersection() {
        ForAllQueued(
            "Handle emitters hit by indirect rays", hitAreaLightQueue, maxQueueSize, SPECTRA_CPU_GPU_LAMBDA(const spectra::HitAreaLightWorkItem w) {
    ...
    void WavefrontPathtracer::TraceShadowRays(int wavefrontDepth) {
        ...
        Do("Reset shadowRayQueue", SPECTRA_CPU_GPU_LAMBDA() { shadowRayQueue->Reset(); });
    }
    ...
    void WavefrontPathtracer::GenerateCameraRays(int y0, Transform movingFromCamera, int sampleIndex) {
        ...
        ParallelFor(
            "Generate camera rays", maxQueueSize, SPECTRA_CPU_GPU_LAMBDA(int pixelIndex) {
                ...
                Point2i pPixel(pixelBounds.pMin.x + pixelIndex % xResolution, y0 + pixelIndex / xResolution);
                ...
                if (cameraRay) cameraRay->ray = movingFromCamera(cameraRay->ray);
                ...
                if (initializeVisibleSurface) pixelSampleState.visibleSurface[pixelIndex] = VisibleSurface();
    ...
    void WavefrontPathtracer::GenerateRaySamples(int wavefrontDepth, int sampleIndex) {
        ...
        spectra::ForAllQueued(
            desc.c_str(), rayQueue, maxQueueSize, SPECTRA_CPU_GPU_LAMBDA(const spectra::RayWorkItem w) {
                ...
                if (haveSubsurface) dimension += 3 * w.depth;
    ...
    void WavefrontPathtracer::EvaluateMaterialAndBSDF(MaterialEvalQueue* evalQueue, Transform movingFromCamera, int wavefrontDepth) {
        ...
        spectra::ForAllQueued(
            desc.c_str(), queue, maxQueueSize, SPECTRA_CPU_GPU_LAMBDA(const spectra::MaterialEvalWorkItem<ConcreteMaterial> w) {
                ...
                MaterialEvalContext ctx              = w.GetMaterialEvalContext(dudx, dudy, dvdx, dvdy, ns, dpdus);
                typename ConcreteMaterial::BxDF bxdf = w.material->GetBxDF(texEval, ctx, lambda);
                BSDF bsdf(ctx.ns, ctx.dpdus, &bxdf);
                // Handle terminated secondary wavelengths after BSDF creation
                if (lambda.SecondaryTerminated()) pixelSampleState.lambda[w.pixelIndex] = lambda;

                // Regularize BSDF, if appropriate
                if (regularize && w.anyNonSpecularBounces) bsdf.Regularize();
                ...
                pstd::optional<BSDFSample> bsdfSample = bsdf.Sample_f<typename ConcreteMaterial::BxDF>(wo, raySamples.indirect.uc, raySamples.indirect.u);
                if (bsdfSample) {
                    ...
                    if (bsdfSample->pdfIsProportional)
                        r_l = r_u / bsdf.PDF<typename ConcreteMaterial::BxDF>(wo, bsdfSample->wi);
                    else
                        r_l = r_u / bsdfSample->pdf;

                    // Update _etaScale_ accounting for BSDF scattering
                    Float etaScale = w.etaScale;
                    if (bsdfSample->IsTransmission()) etaScale *= Sqr(bsdfSample->eta);
                    ...
                    if (beta) {
                        ...
                        if (bsdfSample->IsTransmission() && w.material->HasSubsurfaceScattering()) {
                            bssrdfEvalQueue->Push(w.material, lambda, beta, r_u, Point3f(w.pi), wo, w.n, ns, dpdus, w.uv, w.depth, w.mediumInterface, etaScale, w.pixelIndex);
                        } else {
                            Ray ray = SpawnRay(w.pi, w.n, w.time, wi);
                            // Initialize _ray_ medium if media are present
                            if (haveMedia) ray.medium = spectra::Dot(ray.d, w.n) > 0 ? w.mediumInterface.outside : w.mediumInterface.inside;

                            bool anyNonSpecularBounces = !bsdfSample->IsSpecular() || w.anyNonSpecularBounces;
                            LightSampleContext ctx(w.pi, w.n, ns);
                            nextRayQueue->PushIndirectRay(ray, w.depth + 1, ctx, beta, r_u, r_l, lambda, etaScale, bsdfSample->IsSpecular(), anyNonSpecularBounces, w.pixelIndex);
                        }
                    }
                }
                ...
                if (IsNonSpecular(flags)) {
                    ...
                    pstd::optional<LightLiSample> ls = light.SampleLi(ctx, raySamples.direct.u, lambda, true);
                    if (!ls || !ls->L || ls->pdf == 0) return;
                    Vector3f wi       = ls->wi;
                    SampledSpectrum f = bsdf.f<typename ConcreteMaterial::BxDF>(wo, wi);
                    if (!f) return;
                    ...
                    Float bsdfPDF       = IsDeltaLight(light.Type()) ? 0.f : bsdf.PDF<typename ConcreteMaterial::BxDF>(wo, wi);
                    ...
                    shadowRayQueue->Push(ShadowRayWorkItem{ray, 1 - Float(0.0001f), lambda, Ld, r_u, r_l, w.pixelIndex});
                }
            });
    }
    ...
    void WavefrontPathtracer::SampleMediumInteraction(int wavefrontDepth) {
        ...
        RayQueue* nextRayQueue = NextRayQueue(wavefrontDepth);
        ForAllQueued(
            "Sample medium interaction", mediumSampleQueue, maxQueueSize, SPECTRA_CPU_GPU_LAMBDA(spectra::MediumSampleWorkItem w) {
                Ray ray    = w.ray;
                Float tMax = w.tMax;
                ...
                SampledSpectrum T_maj = SampleT_maj(ray, tMax, uDist, rng, lambda, [&](Point3f p, MediumProperties mp, SampledSpectrum sigma_maj, SampledSpectrum T_maj) {
                    ...
                    if (w.depth < maxDepth && mp.Le) {
                        ...
                        if (r_e) L += beta * mp.sigma_a * T_maj * mp.Le / (pr * r_e.Average());
                    }
                ...
                // the medium.
                if (scattered || !beta || !r_u || w.depth == maxDepth) return;

                // Otherwise, enqueue bump and medium work.
                if (w.tMax == std::numeric_limits<Float>::infinity()) {
                    // no intersection
                    if (escapedRayQueue) {
                        escapedRayQueue->Push(EscapedRayWorkItem{ray.o, ray.d, w.depth, lambda, w.pixelIndex, beta, (int) w.specularBounce, r_u, r_l, w.prevIntrCtx});
                    }
                    return;
                }
        ...
        if (wavefrontDepth == maxDepth) return;

        ForEachType(SampleMediumScatteringCallback{wavefrontDepth, this}, PhaseFunction::Types());
    }
    ...
    void WavefrontPathtracer::SampleMediumScattering(int wavefrontDepth) {
        ...
        spectra::ForAllQueued(
            desc.c_str(), mediumScatterQueue->Get<MediumScatterWorkItem<ConcretePhaseFunction>>(), maxQueueSize, SPECTRA_CPU_GPU_LAMBDA(const spectra::MediumScatterWorkItem<ConcretePhaseFunction> w) {
                RaySamples raySamples = pixelSampleState.samples[w.pixelIndex];
                Vector3f wo           = w.wo;
                ...
                if (sampledLight) {
                    ...
                    pstd::optional<LightLiSample> ls = light.SampleLi(ctx, raySamples.direct.u, w.lambda, true);
                    if (ls && ls->L && ls->pdf > 0) {
                        Vector3f wi          = ls->wi;
                        ...
                        Float lightPDF      = ls->pdf * sampledLight->p;
                        Float phasePDF      = IsDeltaLight(light.Type()) ? 0.f : w.phase->PDF(wo, wi);
                        ...
                        // Enqueue shadow ray
                        shadowRayQueue->Push(ShadowRayWorkItem{ray, 1 - Float(0.0001f), w.lambda, Ld, r_u, r_l, w.pixelIndex});
                    }
                }

                // Sample indirect lighting.
                pstd::optional<PhaseFunctionSample> phaseSample = w.phase->Sample_p(wo, raySamples.indirect.u);
                if (!phaseSample || phaseSample->pdf == 0) return;
                ...
                // Spawn indirect ray.
                nextRayQueue->PushIndirectRay(ray, w.depth + 1, ctx, beta, r_u, r_l, w.lambda, w.etaScale, specularBounce, anyNonSpecularBounces, w.pixelIndex);
            });
    }
    ...
    void WavefrontPathtracer::SampleSubsurface(int wavefrontDepth) {
        ...
        ForAllQueued(
            "Get BSSRDF and enqueue probe ray", bssrdfEvalQueue, maxQueueSize, SPECTRA_CPU_GPU_LAMBDA(const spectra::GetBSSRDFAndProbeRayWorkItem w) {
                const SubsurfaceMaterial* material = w.material.Cast<SubsurfaceMaterial>();
                MaterialEvalContext ctx            = w.GetMaterialEvalContext();
                SampledWavelengths lambda          = w.lambda;
                SubsurfaceMaterial::BSSRDF bssrdf  = material->GetBSSRDF(BasicTextureEvaluator(), ctx, lambda);
                ...
                if (probeSeg) subsurfaceScatterQueue->Push(probeSeg->p0, probeSeg->p1, w.depth, material, bssrdf, lambda, w.beta, w.r_u, w.mediumInterface, w.etaScale, w.pixelIndex);
            });

        aggregate->IntersectOneRandom(maxQueueSize, subsurfaceScatterQueue);

        ForAllQueued(
            "Handle out-scattering after SSS", subsurfaceScatterQueue, maxQueueSize, SPECTRA_CPU_GPU_LAMBDA(spectra::SubsurfaceScatterWorkItem w) {
                ...
                if (!bssrdfSample.Sp || !bssrdfSample.pdf) return;
                ...
                // Indirect...
                {
                    ...
                    pstd::optional<BSDFSample> bsdfSample = bsdf.Sample_f<TabulatedBSSRDF::BxDF>(wo, uc, u);
                    if (bsdfSample) {
                        Vector3f wi               = bsdfSample->wi;
                        ...
                        SampledSpectrum r_l;
                        if (bsdfSample->pdfIsProportional)
                            r_l = r_u / bsdf.PDF<TabulatedBSSRDF::BxDF>(wo, bsdfSample->wi);
                        else
                            r_l = r_u / bsdfSample->pdf;
                        ...
                        if (beta) {
                            ...
                            if (haveMedia) ray.medium = Dot(ray.d, intr.n) > 0 ? w.mediumInterface.outside : w.mediumInterface.inside;
                            ...
                            nextRayQueue->PushIndirectRay(ray, w.depth + 1, ctx, beta, indir_r_u, r_l, lambda, etaScale, bsdfSample->IsSpecular(), anyNonSpecularBounces, w.pixelIndex);
                        }
                    }
                }
                ...
                if (IsNonSpecular(bsdf.Flags())) {
                    ...
                    // This causes r_u to be zero for the shadow ray, so that
                    // part of MIS just becomes a no-op.
                    Float bsdfPDF       = IsDeltaLight(light.Type()) ? 0.f : bsdf.PDF<TabulatedBSSRDF::BxDF>(wo, wi);
    ...
    void WavefrontPathtracer::UpdateFilm() {
        ParallelFor(
            "Update film", maxQueueSize, SPECTRA_CPU_GPU_LAMBDA(int pixelIndex) {
                ...
                SampledSpectrum Lw = SampledSpectrum(pixelSampleState.L[pixelIndex]) * pixelSampleState.cameraRayWeight[pixelIndex];
    ...
    void WavefrontPathtracer::UpdateFramebufferFromFilm(Bounds2i pixelBounds, Float exposure, float* rgba) {
        Vector2i resolution = pixelBounds.Diagonal();
        ParallelFor(
            "Update RGBA framebuffer", resolution.x * resolution.y, SPECTRA_CPU_GPU_LAMBDA(int index) {
