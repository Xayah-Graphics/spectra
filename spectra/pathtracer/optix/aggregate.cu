...
namespace spectra::optix
{
    ...
    static constexpr unsigned int SpectraOptiXPayloadSemantics =
    ...
    SpectraOptiXAggregate::Accel SpectraOptiXAggregate::buildOptixBVH(
        ...
        ThreadLocal<cudaStream_t>& threadCUDAStreams)
    {
        ...
        SPECTRA_OPTIX_CHECK(optixAccelComputeMemoryUsage(optixContext, &accelOptions,
        ...
        SPECTRA_OPTIX_CHECK(optixAccelBuild(
        ...
        SPECTRA_CUDA_CHECK(cudaMemcpyAsync(&compactedSize, compactedSizePtr, sizeof(uint64_t),
        ...
        if (compactedSize < blasBufferSizes.outputSizeInBytes)
        {
            ...
            SPECTRA_OPTIX_CHECK(optixAccelCompact(optixContext, buildStream, traversableHandle,
    ...
    static Material getMaterial(const scene::ShapeSceneEntity& shape,
                                ...
                                const std::vector<Material>& materials)
    {
        if (!shape.materialName.empty())
        {
            ...
            if (iter == namedMaterials.end())
                throw std::runtime_error(spectra::diagnostics::Format(&shape.loc, "%s: material not defined", shape.materialName));
            return iter->second;
        }
        else
        {
    ...
    static FloatTexture getAlphaTexture(
        const scene::ShapeSceneEntity& shape,
        const std::map<std::string, FloatTexture>& floatTextures, Allocator alloc)
    {
        ...
        else
        {
            ...
            if (iter == floatTextures.end())
                throw std::runtime_error(spectra::diagnostics::Format(&shape.loc, "%s: alpha texture not defined.", alphaTexName));
    ...
    static MediumInterface* getMediumInterface(const scene::ShapeSceneEntity& shape,
                                               ...
                                               Allocator alloc)
    {
        ...
        auto getMedium = [&](const std::string& name) -> Medium
        {
            ...
            if (iter == media.end())
                throw std::runtime_error(spectra::diagnostics::Format(&shape.loc, "%s: medium not defined", name));
            return iter->second;
        };
    ...
    std::map<int, TriQuadMesh> SpectraOptiXAggregate::PreparePLYMeshes(
        ...
        const std::map<std::string, FloatTexture>& floatTextures)
    {
        ...
        ParallelFor(0, shapes.size(), [&](int64_t i)
        {
            ...
            if (filename.empty())
                throw std::runtime_error(spectra::diagnostics::Format(&shape.loc, "plymesh: \"filename\" must be provided."));
    ...
    SpectraOptiXAggregate::BVH SpectraOptiXAggregate::buildBVHForTriangles(
        const std::vector<scene::ShapeSceneEntity>& shapes,
        ...
        ThreadLocal<cudaStream_t>& threadCUDAStreams)
    {
        ...
        ParallelFor(0, nMeshes, [&](int64_t startIndex, int64_t endIndex)
        {
            ...
            for (int meshIndex = startIndex; meshIndex < endIndex; ++meshIndex)
            {
                ...
                SPECTRA_CUDA_CHECK(cudaMemcpy(pGPU, p32.data(), mesh->nVertices * 3 * sizeof(float),
                ...
                SPECTRA_CUDA_CHECK(cudaMemcpy(indicesGPU, mesh->vertexIndices, mesh->nTriangles * 3 * sizeof(int),
                    cudaMemcpyHostToDevice));
    ...
    SpectraOptiXAggregate::BVH SpectraOptiXAggregate::buildBVHForBLPs(
        ...
        ThreadLocal<cudaStream_t>& threadCUDAStreams)
    {
        ...
        ParallelFor(0, nMeshes, [&](int64_t meshIndex)
        {
            ...
            for (int patchIndex = 0; patchIndex < mesh->nPatches; ++patchIndex)
            {
                ...
                OptixAabb aabb = {
                    float(patchBounds.pMin.x), float(patchBounds.pMin.y),
        ...
        SPECTRA_CUDA_CHECK(cudaMemcpyAsync(deviceAABBs, aabbs.data(),
    ...
    SpectraOptiXAggregate::BVH SpectraOptiXAggregate::buildBVHForQuadrics(
        const std::vector<scene::ShapeSceneEntity>& shapes, OptixDeviceContext optixContext,
        const OptixProgramGroup& intersectPG, const OptixProgramGroup& shadowPG,
        const OptixProgramGroup& randomHitPG,
        const std::map<std::string, FloatTexture>& floatTextures,
        ...
        ThreadLocal<cudaStream_t>& threadCUDAStreams)
    {
        ...
        for (size_t shapeIndex = 0; shapeIndex < shapes.size(); ++shapeIndex)
        {
            ...
            OptixAabb aabb = {
                float(shapeBounds.pMin.x), float(shapeBounds.pMin.y),
        ...
        SPECTRA_CUDA_CHECK(cudaMemcpyAsync(deviceShapeAABBs, shapeAABBs.data(),
    ...
    static void optixDiagnosticCallback(unsigned int level, const char* tag, const char* message,
                                        void* cbdata)
    {
    ...
    int SpectraOptiXAggregate::addHGRecords(const BVH& bvh)
    {
        ...
        if (bvh.accelBuffer)
            accelBuffers.push_back(bvh.accelBuffer);
        return sbtOffset;
    }
                                                         ...
                                                         const char* input, size_t inputSize)
    {
        ...
        SPECTRA_OPTIX_CHECK_WITH_DIAGNOSTICS(
            optixModuleCreateWithTasks(
                optixContext, &moduleCompileOptions, &pipelineCompileOptions, input,
                inputSize, optixDiagnostics, &optixDiagnosticsSize, &optixModule,
                &firstTask),
            optixDiagnostics
        );
        ...
        while (!tasks.empty())
        {
            ...
            SPECTRA_OPTIX_CHECK_WITH_DIAGNOSTICS(
                optixTaskExecute(task, additionalTasks.data(),
                    static_cast<unsigned int>(additionalTasks.size()), &additionalTaskCount),
                optixDiagnostics);
            for (unsigned int i = 0; i < additionalTaskCount; ++i)
                tasks.push_back(additionalTasks[i]);
        }
    ...
    OptixProgramGroup SpectraOptiXAggregate::createRaygenPG(const char* entrypoint)
    {
        ...
        SPECTRA_OPTIX_CHECK_WITH_DIAGNOSTICS(
            optixProgramGroupCreate(optixContext, &desc, 1, &pgOptions, optixDiagnostics, &optixDiagnosticsSize, &pg),
            optixDiagnostics);
    ...
    SpectraOptiXAggregate::SpectraOptiXAggregate(
        const scene::Scene& scene, CUDATrackedMemoryResource* memoryResource,
        ...
        : memoryResource(memoryResource), cudaStream(nullptr)
    {
        ...
        SPECTRA_OPTIX_CHECK(optixDeviceContextSetCacheDatabaseSizes(
            optixContext, SpectraOptiXCacheSizeBytes / 2, SpectraOptiXCacheSizeBytes));


        // OptiX module
        optixModule = createOptiXModule(
            optixContext, reinterpret_cast<const char*>(SPECTRA_EMBEDDED_OPTIX_INPUT),
            static_cast<size_t>(SPECTRA_EMBEDDED_OPTIX_INPUT_SIZE));
        ...
        SPECTRA_OPTIX_CHECK_WITH_DIAGNOSTICS(
            optixPipelineCreate(optixContext, &pipelineCompileOptions, &pipelineLinkOptions,
                allPGs, sizeof(allPGs) / sizeof(allPGs[0]), optixDiagnostics, &optixDiagnosticsSize,
                &optixPipeline),
            optixDiagnostics);
        ...
        SPECTRA_OPTIX_CHECK(optixUtilComputeStackSizes(
            &stackSizes, pipelineLinkOptions.maxTraceDepth,
            pipelineLinkOptions.maxContinuationCallableDepth,
            pipelineLinkOptions.maxDirectCallableDepthFromState,
            &directCallableStackSizeFromTraversal, &directCallableStackSizeFromState,
            &continuationStackSize));
        SPECTRA_OPTIX_CHECK(optixPipelineSetStackSize(
            optixPipeline, directCallableStackSizeFromTraversal,
            directCallableStackSizeFromState, continuationStackSize,
            pipelineLinkOptions.maxTraversableGraphDepth));
        ...
        for (const auto& shape : scene.shapes)
            if (shape.name != "sphere" && shape.name != "cylinder" && shape.name != "disk" &&
                ...
                shape.name != "curve")
                throw std::runtime_error(spectra::diagnostics::Format(&shape.loc, "%s: unknown shape", shape.name));
        ...
        if (rootAccel.buffer)
            accelBuffers.push_back(rootAccel.buffer);

        SPECTRA_CUDA_CHECK(cudaFree((void *)instanceDevicePtr));


        if (!scene.animatedShapes.empty())
            spectra::diagnostics::PrintWarning("Ignoring %d animated shapes", scene.animatedShapes.size());
        ...
        threadCUDAStreams.ForAll([](cudaStream_t stream)
        {
    ...
    SpectraOptiXAggregate::~SpectraOptiXAggregate()
    {
        ...
        if (optixPipeline)
            SPECTRA_OPTIX_CHECK(optixPipelineDestroy(optixPipeline));
        for (OptixProgramGroup programGroup : programGroups)
            SPECTRA_OPTIX_CHECK(optixProgramGroupDestroy(programGroup));
        if (optixModule)
            SPECTRA_OPTIX_CHECK(optixModuleDestroy(optixModule));

        for (CUdeviceptr buffer : sbtBuffers)
            SPECTRA_CUDA_CHECK(cudaFree(reinterpret_cast<void*>(buffer)));
        for (CUdeviceptr buffer : accelBuffers)
            SPECTRA_CUDA_CHECK(cudaFree(reinterpret_cast<void*>(buffer)));

        for (ParamBufferState& ps : paramsPool)
        {
            if (ps.used)
                SPECTRA_CUDA_CHECK(cudaEventSynchronize(ps.finishedEvent));
            if (ps.ptr)
                SPECTRA_CUDA_CHECK(cudaFree(reinterpret_cast<void*>(ps.ptr)));
            if (ps.hostPtr)
                SPECTRA_CUDA_CHECK(cudaFreeHost(ps.hostPtr));
            if (ps.finishedEvent)
                SPECTRA_CUDA_CHECK(cudaEventDestroy(ps.finishedEvent));
        }

        if (optixContext)
            SPECTRA_OPTIX_CHECK(optixDeviceContextDestroy(optixContext));
    }

    SpectraOptiXAggregate::ParamBufferState& SpectraOptiXAggregate::getParamBuffer(
        const RayIntersectParameters& params) const
    {
        ...
        SPECTRA_CUDA_CHECK(cudaMemcpyAsync((void *)pbs.ptr, pbs.hostPtr, sizeof(params),
            cudaMemcpyHostToDevice));
                                                 ...
                                                 RayQueue* nextRayQueue) const
    {
        if (rootTraversable)
        {
            ...
            SPECTRA_OPTIX_CHECK(optixLaunch(optixPipeline, cudaStream, pbs.ptr,
    ...
    void SpectraOptiXAggregate::IntersectOneRandom(
