...
namespace spectra {
    ...
    __host__ __device__ Float ColorEncoding::ToFloatLinear(Float v) const {
        auto tfl = [&](auto ptr) { return ptr->ToFloatLinear(v); };
        return Dispatch(tfl);
    }

    __host__ __device__ void ColorEncoding::FromLinear(pstd::span<const Float> vin, pstd::span<uint8_t> vout) const {
    ...
    __host__ __device__ Float sRGBColorEncoding::ToFloatLinear(Float v) const {
    ...
    __host__ __device__ GammaColorEncoding::GammaColorEncoding(Float gamma) : gamma(gamma) {
        for (int i = 0; i < 256; ++i) {
    ...
    __host__ __device__ Float GammaColorEncoding::ToFloatLinear(Float v) const {
