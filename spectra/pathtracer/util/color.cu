...
namespace spectra {
    SPECTRA_CPU_GPU void ColorEncoding::ToLinear(pstd::span<const uint8_t> vin, pstd::span<Float> vout) const {
    ...
    SPECTRA_CPU_GPU void ColorEncoding::FromLinear(pstd::span<const Float> vin, pstd::span<uint8_t> vout) const {
    ...
    SPECTRA_CPU_GPU RGBSigmoidPolynomial RGBToSpectrumTable::operator()(RGB rgb) const {
    ...
    void RGBToSpectrumTable::Init(Allocator alloc) {
        // sRGB
        float* sRGBToSpectrumTableScalePtr = (float*) alloc.allocate_bytes(sizeof(sRGBToSpectrumTable_Scale));
        memcpy(sRGBToSpectrumTableScalePtr, sRGBToSpectrumTable_Scale, sizeof(sRGBToSpectrumTable_Scale));
        CoefficientArray* sRGBToSpectrumTableDataPtr = (CoefficientArray*) alloc.allocate_bytes(sizeof(CoefficientArray));
        memcpy(sRGBToSpectrumTableDataPtr, sRGBToSpectrumTable_Data, sizeof(sRGBToSpectrumTable_Data));

        sRGB = alloc.new_object<RGBToSpectrumTable>(sRGBToSpectrumTableScalePtr, sRGBToSpectrumTableDataPtr);

        // DCI_P3
        float* DCI_P3ToSpectrumTableScalePtr = (float*) alloc.allocate_bytes(sizeof(DCI_P3ToSpectrumTable_Scale));
        memcpy(DCI_P3ToSpectrumTableScalePtr, DCI_P3ToSpectrumTable_Scale, sizeof(DCI_P3ToSpectrumTable_Scale));
        CoefficientArray* DCI_P3ToSpectrumTableDataPtr = (CoefficientArray*) alloc.allocate_bytes(sizeof(DCI_P3ToSpectrumTable_Data));
        memcpy(DCI_P3ToSpectrumTableDataPtr, DCI_P3ToSpectrumTable_Data, sizeof(DCI_P3ToSpectrumTable_Data));

        DCI_P3 = alloc.new_object<RGBToSpectrumTable>(DCI_P3ToSpectrumTableScalePtr, DCI_P3ToSpectrumTableDataPtr);

        // Rec2020
        float* REC2020ToSpectrumTableScalePtr = (float*) alloc.allocate_bytes(sizeof(REC2020ToSpectrumTable_Scale));
        memcpy(REC2020ToSpectrumTableScalePtr, REC2020ToSpectrumTable_Scale, sizeof(REC2020ToSpectrumTable_Scale));
        CoefficientArray* REC2020ToSpectrumTableDataPtr = (CoefficientArray*) alloc.allocate_bytes(sizeof(REC2020ToSpectrumTable_Data));
        memcpy(REC2020ToSpectrumTableDataPtr, REC2020ToSpectrumTable_Data, sizeof(REC2020ToSpectrumTable_Data));

        Rec2020 = alloc.new_object<RGBToSpectrumTable>(REC2020ToSpectrumTableScalePtr, REC2020ToSpectrumTableDataPtr);

        // ACES2065_1
        float* ACES2065_1ToSpectrumTableScalePtr = (float*) alloc.allocate_bytes(sizeof(ACES2065_1ToSpectrumTable_Scale));
        memcpy(ACES2065_1ToSpectrumTableScalePtr, ACES2065_1ToSpectrumTable_Scale, sizeof(ACES2065_1ToSpectrumTable_Scale));
        CoefficientArray* ACES2065_1ToSpectrumTableDataPtr = (CoefficientArray*) alloc.allocate_bytes(sizeof(ACES2065_1ToSpectrumTable_Data));
        memcpy(ACES2065_1ToSpectrumTableDataPtr, ACES2065_1ToSpectrumTable_Data, sizeof(ACES2065_1ToSpectrumTable_Data));
    ...
    SPECTRA_CPU_GPU void sRGBColorEncoding::FromLinear(pstd::span<const Float> vin, pstd::span<uint8_t> vout) const {
        DCHECK_EQ(vin.size(), vout.size());
        for (size_t i = 0; i < vin.size(); ++i) vout[i] = LinearToSRGB8(vin[i]);
    }

    SPECTRA_CPU_GPU void sRGBColorEncoding::ToLinear(pstd::span<const uint8_t> vin, pstd::span<Float> vout) const {
        DCHECK_EQ(vin.size(), vout.size());
        for (size_t i = 0; i < vin.size(); ++i) vout[i] = SRGB8ToLinear(vin[i]);
    }

    SPECTRA_CPU_GPU Float sRGBColorEncoding::ToFloatLinear(Float v) const {
    ...
    SPECTRA_CPU_GPU GammaColorEncoding::GammaColorEncoding(Float gamma) : gamma(gamma) {
        for (int i = 0; i < 256; ++i) {
