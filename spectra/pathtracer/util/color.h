...
namespace spectra {
    ...
    SPECTRA_CPU_GPU
    inline Float MaxComponentValue(RGB rgb) {
        return std::max(rgb.r, std::max(rgb.g, rgb.b));
    }

    SPECTRA_CPU_GPU
    inline Float MinComponentValue(RGB rgb) {
    ...
    SPECTRA_CPU_GPU inline RGB ClampZero(RGB rgb) {
    ...
    public:
        ...
        SPECTRA_CPU_GPU void ToLinear(pstd::span<const uint8_t> vin, pstd::span<Float> vout) const;
        SPECTRA_CPU_GPU void FromLinear(pstd::span<const Float> vin, pstd::span<uint8_t> vout) const;
    ...
    extern SPECTRA_CONST Float SRGBToLinearLUT[256];
