...
namespace spectra {
    ...
    void RGBColorSpace::Init(Allocator alloc) {
        ...
        // P3-D65 (display)
        DCI_P3 = alloc.new_object<RGBColorSpace>(Point2f(.68, .32), Point2f(.265, .690), Point2f(.15, .06), GetNamedSpectrum("stdillum-D65"), RGBToSpectrumTable::DCI_P3, alloc);
        // ITU-R Rec BT.2020
        ...
        CUDA_CHECK(cudaMemcpyToSymbol(RGBColorSpace_sRGB, &RGBColorSpace::sRGB, sizeof(RGBColorSpace_sRGB)));
        CUDA_CHECK(cudaMemcpyToSymbol(RGBColorSpace_DCI_P3, &RGBColorSpace::DCI_P3, sizeof(RGBColorSpace_DCI_P3)));
        CUDA_CHECK(cudaMemcpyToSymbol(RGBColorSpace_Rec2020, &RGBColorSpace::Rec2020, sizeof(RGBColorSpace_Rec2020)));
        CUDA_CHECK(cudaMemcpyToSymbol(RGBColorSpace_ACES2065_1, &RGBColorSpace::ACES2065_1, sizeof(RGBColorSpace_ACES2065_1)));
    }
} // namespace spectra
