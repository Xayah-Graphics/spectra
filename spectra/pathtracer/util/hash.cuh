...
namespace spectra {
    ...
    __host__ __device__ inline uint64_t MurmurHash64A(const unsigned char* key, size_t len, uint64_t seed) {
    ...
    __host__ __device__ inline uint64_t MixBits(uint64_t v);
    ...
    template <typename T>
    __host__ __device__ uint64_t HashBuffer(const T* ptr, size_t nElements, uint64_t seed = 0) {
        return MurmurHash64A((const unsigned char*) ptr, nElements * sizeof(T), seed);
    }

    template <typename... Args>
    __host__ __device__ uint64_t Hash(Args... args);

    template <typename... Args>
    __host__ __device__ void hashRecursiveCopy(char* buf, Args...);

    template <>
    __host__ __device__ inline void hashRecursiveCopy(char* buf) {}

    template <typename T, typename... Args>
    __host__ __device__ void hashRecursiveCopy(char* buf, T v, Args... args) {
    ...
    template <typename... Args>
    __host__ __device__ Float HashFloat(Args... args) {
