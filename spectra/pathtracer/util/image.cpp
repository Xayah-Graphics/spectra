...
namespace spectra {
    SPECTRA_CPU_GPU int TexelBytes(PixelFormat format) {
        switch (format) {
        ...
        for (int y = 0; y < resolution.y; ++y)
            for (int x = 0; x < resolution.x; ++x)
                for (int c = 0; c < NChannels(); ++c)
                    if (IsInf(GetChannel({x, y}, c))) return true;
        return false;
    }
    ...
    bool Image::HasAnyNaNPixels() const {
        ...
        for (int y = 0; y < resolution.y; ++y)
            for (int x = 0; x < resolution.x; ++x)
                for (int c = 0; c < NChannels(); ++c)
                    if (IsNaN(GetChannel({x, y}, c))) return true;
        return false;
    }
    ...
    Image Image::FloatResizeUp(Point2i newRes, WrapMode2D wrapMode) const {
        ...
        ParallelFor2D(Bounds2i({0, 0}, newRes), [&](Bounds2i outExtent) {
            ...
            Bounds2i inExtent(Point2i(xWeights[outExtent.pMin.x].firstPixel, yWeights[outExtent.pMin.y].firstPixel), Point2i(xWeights[outExtent.pMax.x - 1].firstPixel + 4, yWeights[outExtent.pMax.y - 1].firstPixel + 4));
    ...
    pstd::vector<Image> Image::GeneratePyramid(Image image, WrapMode2D wrapMode, Allocator alloc) {
        ...
        if (!IsPowerOf2(image.resolution[0]) || !IsPowerOf2(image.resolution[1]))
            image = image.FloatResizeUp(Point2i(RoundUpPow2(image.resolution[0]), RoundUpPow2(image.resolution[1])), wrapMode);
        ...
        for (int i = 0; i < nLevels - 1; ++i) {
            ...
            ParallelFor(0, nextResolution[1], [&](int64_t y) {
                ...
                for (int x = 0; x < nextResolution[0]; ++x, srcOffset += nChannels)
                    for (int c = 0; c < nChannels; ++c, ++srcOffset, ++nextOffset) nextImage.p32[nextOffset] = (image.p32[srcOffset] + image.p32[srcOffset + srcDeltas[1]] + image.p32[srcOffset + srcDeltas[2]] + image.p32[srcOffset + srcDeltas[3]]) / 4;
                ...
                pyramid[i].CopyRectIn(Bounds2i({0, yStart}, {image.resolution[0], yEnd}), pstd::span<const float>(image.p32.data() + offset, count));
            });
        ...
        pyramid[nLevels - 1].CopyRectIn(Bounds2i({0, 0}, {1, 1}), pstd::span<const float>(image.p32.data(), nChannels));
        return pyramid;
    }

    Image::Image(PixelFormat format, Point2i resolution, pstd::span<const std::string> channels, ColorEncoding encoding, Allocator alloc) : format(format), resolution(resolution), channelNames(channels.begin(), channels.end()), encoding(encoding), p8(alloc), p16(alloc), p32(alloc) {
        if (Is8Bit(format)) {
            ...
            SPECTRA_FATAL("Unhandled format in Image::Image()");
    }
    ...
    ImageChannelDesc Image::GetChannelDesc(pstd::span<const std::string> requestedChannels) const {
        ...
        for (size_t i = 0; i < requestedChannels.size(); ++i) {
            ...
            if (j == channelNames.size()) return {};
        }

        return desc;
    }

    ImageChannelValues Image::GetChannels(Point2i p, const ImageChannelDesc& desc, WrapMode2D wrapMode) const {
        ...
        case PixelFormat::U256:
            {
                for (int i = 0; i < desc.offset.size(); ++i) encoding.ToLinear({&p8[pixelOffset + desc.offset[i]], 1}, {&cv[i], 1});
                break;
            }
        case PixelFormat::Half:
            {
                for (int i = 0; i < desc.offset.size(); ++i) cv[i] = Float(p16[pixelOffset + desc.offset[i]]);
                break;
            }
        case PixelFormat::Float:
            {
                for (int i = 0; i < desc.offset.size(); ++i) cv[i] = p32[pixelOffset + desc.offset[i]];
                break;
            }
    ...
    void Image::SetChannels(Point2i p, const ImageChannelDesc& desc, pstd::span<const Float> values) {
        ...
        for (size_t i = 0; i < values.size(); ++i) SetChannel(p, desc.offset[i], values[i]);
    }

    Image::Image(pstd::vector<uint8_t> p8c, Point2i resolution, pstd::span<const std::string> channels, ColorEncoding encoding) : format(PixelFormat::U256), resolution(resolution), channelNames(channels.begin(), channels.end()), encoding(encoding), p8(std::move(p8c)) {
        CHECK_EQ(p8.size(), NChannels() * resolution[0] * resolution[1]);
    }

    Image::Image(pstd::vector<Half> p16c, Point2i resolution, pstd::span<const std::string> channels) : format(PixelFormat::Half), resolution(resolution), channelNames(channels.begin(), channels.end()), p16(std::move(p16c)) {
        CHECK_EQ(p16.size(), NChannels() * resolution[0] * resolution[1]);
        CHECK(Is16Bit(format));
    }

    Image::Image(pstd::vector<float> p32c, Point2i resolution, pstd::span<const std::string> channels) : format(PixelFormat::Float), resolution(resolution), channelNames(channels.begin(), channels.end()), p32(std::move(p32c)) {
    ...
    Image Image::ConvertToFormat(PixelFormat newFormat, ColorEncoding encoding) const {
        ...
        for (int y = 0; y < resolution.y; ++y)
            for (int x = 0; x < resolution.x; ++x)
                for (int c = 0; c < NChannels(); ++c) newImage.SetChannel({x, y}, c, GetChannel({x, y}, c));
        return newImage;
    }
    ...
    ImageChannelValues Image::GetChannels(Point2i p, WrapMode2D wrapMode) const {
        ...
        case PixelFormat::Half:
            {
                for (int i = 0; i < NChannels(); ++i) cv[i] = Float(p16[pixelOffset + i]);
                break;
            }
        case PixelFormat::Float:
            {
                for (int i = 0; i < NChannels(); ++i) cv[i] = p32[pixelOffset + i];
                break;
            }
        default: SPECTRA_FATAL("Unhandled PixelFormat");
        }
    ...
    std::vector<std::string> Image::ChannelNames(const ImageChannelDesc& desc) const {
        ...
        for (int i = 0; i < desc.size(); ++i) names.push_back(channelNames[desc.offset[i]]);
        return names;
    }
    ...
    ImageChannelValues Image::Average(const ImageChannelDesc& desc) const {
        ...
        for (int y = 0; y < Resolution().y; ++y)
            for (int x = 0; x < Resolution().x; ++x) {
                ImageChannelValues v = GetChannels({x, y}, desc);
                for (int c = 0; c < desc.size(); ++c) sum[c] += v[c];
            }

        ImageChannelValues average(desc.size());
        for (int c = 0; c < desc.size(); ++c) average[c] = sum[c] / (Float(Resolution().x) * Float(Resolution().y));
        return average;
    }

    void Image::CopyRectOut(const Bounds2i& extent, pstd::span<float> buf, WrapMode2D wrapMode) const {
        ...
        case PixelFormat::Half: ForExtent(extent, wrapMode, *this, [&bufIter, this](int offset) { *bufIter++ = Float(p16[offset]); }); break;

        case PixelFormat::Float: ForExtent(extent, wrapMode, *this, [&bufIter, this](int offset) { *bufIter++ = Float(p32[offset]); }); break;
    ...
    void Image::CopyRectIn(const Bounds2i& extent, pstd::span<const float> buf) {
        ...
        case PixelFormat::Half: ForExtent(extent, WrapMode::Clamp, *this, [&bufIter, this](int offset) { p16[offset] = Half(*bufIter++); }); break;

        case PixelFormat::Float: ForExtent(extent, WrapMode::Clamp, *this, [&bufIter, this](int offset) { p32[offset] = *bufIter++; }); break;
    ...
    ImageChannelValues Image::LookupNearest(Point2f p, WrapMode2D wrapMode) const {
        ImageChannelValues cv(NChannels(), Float(0));
        for (int c = 0; c < NChannels(); ++c) cv[c] = LookupNearestChannel(p, c, wrapMode);
        return cv;
    }

    ImageChannelValues Image::LookupNearest(Point2f p, const ImageChannelDesc& desc, WrapMode2D wrapMode) const {
        ImageChannelValues cv(desc.offset.size(), Float(0));
        for (int i = 0; i < desc.offset.size(); ++i) cv[i] = LookupNearestChannel(p, desc.offset[i], wrapMode);
        return cv;
    }

    ImageChannelValues Image::Bilerp(Point2f p, WrapMode2D wrapMode) const {
        ImageChannelValues cv(NChannels(), Float(0));
        for (int c = 0; c < NChannels(); ++c) cv[c] = BilerpChannel(p, c, wrapMode);
        return cv;
    }

    ImageChannelValues Image::Bilerp(Point2f p, const ImageChannelDesc& desc, WrapMode2D wrapMode) const {
        ImageChannelValues cv(desc.offset.size(), Float(0));
        for (int i = 0; i < desc.offset.size(); ++i) cv[i] = BilerpChannel(p, desc.offset[i], wrapMode);
        return cv;
    }
    ...
    void Image::SetChannels(Point2i p, const ImageChannelValues& values) {
        CHECK_LE(values.size(), NChannels());
        int i = 0;
        for (auto iter = values.begin(); iter != values.end(); ++iter, ++i) SetChannel(p, i, *iter);
    }

    void Image::SetChannels(Point2i p, pstd::span<const Float> values) {
        CHECK_LE(values.size(), NChannels());
        for (size_t i = 0; i < values.size(); ++i) SetChannel(p, i, values[i]);
    }
            ...
            for (int x = 0; x < resolution.x; ++x) {
                ...
                for (int c = 0; c < NChannels(); ++c) {
                    if (Is8Bit(format))
                        ...
                        SPECTRA_FATAL("unexpected format");
                }
            }
        }
    }
    ...
    ImageAndMetadata Image::Read(std::string name, Allocator alloc, ColorEncoding encoding) {
        if (HasExtension(name, "exr"))
            ...
            if (data) {
                ...
                switch (n) {
                case 1: return ImageAndMetadata{Image(std::move(pixels), {x, y}, {"Y"}, ColorEncoding::sRGB), ImageMetadata()};
                case 2:
                    {
                ...
                case 3: return ImageAndMetadata{Image(std::move(pixels), {x, y}, {"R", "G", "B"}, ColorEncoding::sRGB), ImageMetadata()};
                case 4:
                    {
    ...
    bool Image::Write(std::string name, const ImageMetadata& metadata) const {
        if (metadata.pixelBounds) CHECK_EQ(metadata.pixelBounds->Area(), size_t(resolution.x) * size_t(resolution.y));

        if (HasExtension(name, "exr")) return WriteEXR(name, metadata);
    ...
    // OpenEXR
#if !defined(__CUDA_ARCH__)
    static Imf::FrameBuffer imageToFrameBuffer(const Image& image, const ImageChannelDesc& desc, const Imath::Box2i& dataWindow) {
        ...
        switch (image.Format()) {
        case PixelFormat::Half:
            for (int channelIndex : desc.offset) fb.insert(channelNames[channelIndex], Imf::Slice(Imf::HALF, originPtr + channelIndex * sizeof(Half), xStride, yStride));
            break;
        case PixelFormat::Float:
            for (int channelIndex : desc.offset) fb.insert(channelNames[channelIndex], Imf::Slice(Imf::FLOAT, originPtr + channelIndex * sizeof(float), xStride, yStride));
            break;
        default: SPECTRA_FATAL("Unexpected image format");
        }
        return fb;
    }
    ...
    static ImageAndMetadata ReadEXR(const std::string& name, Allocator alloc) {
        try {
            ...
            const Imf::FloatAttribute* renderTimeAttrib = file.header().findTypedAttribute<Imf::FloatAttribute>("renderTimeSeconds");
            if (renderTimeAttrib) metadata.renderTimeSeconds = renderTimeAttrib->value();

            const Imf::M44fAttribute* worldToCameraAttrib = file.header().findTypedAttribute<Imf::M44fAttribute>("worldToCamera");
            ...
            const Imf::M44fAttribute* worldToNDCAttrib = file.header().findTypedAttribute<Imf::M44fAttribute>("worldToNDC");
            if (worldToNDCAttrib) {
                SquareMatrix<4> m;
                for (int i = 0; i < 4; ++i)
                    for (int j = 0; j < 4; ++j) m[i][j] = worldToNDCAttrib->value().getValue()[4 * i + j];
            ...
            const Imf::IntAttribute* sppAttrib = file.header().findTypedAttribute<Imf::IntAttribute>("samplesPerPixel");
            if (sppAttrib) metadata.samplesPerPixel = sppAttrib->value();

            const Imf::FloatAttribute* mseAttrib = file.header().findTypedAttribute<Imf::FloatAttribute>("MSE");
            if (mseAttrib) metadata.MSE = mseAttrib->value();
            ...
            const Imf::ChromaticitiesAttribute* chromaticitiesAttrib = file.header().findTypedAttribute<Imf::ChromaticitiesAttribute>("chromaticities");
            ...
            for (auto iter = channels.begin(); iter != channels.end(); ++iter) {
                if (nChannels++ == 0)
                    ...
                    if (pixelType != iter.channel().type)
                        SPECTRA_FATAL("ReadEXR() doesn't currently support images with "
            ...
            Image image(pixelType == Imf::HALF ? PixelFormat::Half : PixelFormat::Float, {width, height}, channelNames, nullptr, alloc);
    ...
    bool Image::WriteEXR(const std::string& name, const ImageMetadata& metadata) const {
        if (Is8Bit(format)) return ConvertToFormat(PixelFormat::Half).WriteEXR(name, metadata);
        ...
        try {
            ...
            if (metadata.fullResolution)
                ...
                displayWindow = {Imath::V2i(0, 0), Imath::V2i(metadata.fullResolution->x - 1, metadata.fullResolution->y - 1)};
            else
                displayWindow = {Imath::V2i(0, 0), Imath::V2i(resolution.x - 1, resolution.y - 1)};

            if (metadata.pixelBounds)
                dataWindow = {Imath::V2i(metadata.pixelBounds->pMin.x, metadata.pixelBounds->pMin.y), Imath::V2i(metadata.pixelBounds->pMax.x - 1, metadata.pixelBounds->pMax.y - 1)};
            else
            ...
            for (auto iter = fb.begin(); iter != fb.end(); ++iter) header.channels().insert(iter.name(), iter.slice().type);

            if (metadata.renderTimeSeconds) header.insert("renderTimeSeconds", Imf::FloatAttribute(*metadata.renderTimeSeconds));
            if (metadata.cameraFromWorld) {
                float m[4][4];
                for (int i = 0; i < 4; ++i)
                    for (int j = 0; j < 4; ++j) m[i][j] = (*metadata.cameraFromWorld)[i][j];
                header.insert("worldToCamera", Imf::M44fAttribute(m));
            }
            if (metadata.NDCFromWorld) {
                float m[4][4];
                for (int i = 0; i < 4; ++i)
                    for (int j = 0; j < 4; ++j) m[i][j] = (*metadata.NDCFromWorld)[i][j];
                header.insert("worldToNDC", Imf::M44fAttribute(m));
            }
            if (metadata.samplesPerPixel) header.insert("samplesPerPixel", Imf::IntAttribute(*metadata.samplesPerPixel));
            if (metadata.MSE) header.insert("MSE", Imf::FloatAttribute(*metadata.MSE));
            for (const auto& iter : metadata.strings) header.insert(iter.first, Imf::StringAttribute(iter.second));
            for (const auto& iter : metadata.stringVectors) header.insert(iter.first, Imf::StringVectorAttribute(iter.second));
    ...
    static ImageAndMetadata ReadPNG(const std::string& name, Allocator alloc, ColorEncoding encoding) {
        ...
        if (!encoding) encoding = ColorEncoding::sRGB;
    ...
    Image Image::SelectChannels(const ImageChannelDesc& desc, Allocator alloc) const {
        ...
        for (size_t i = 0; i < desc.offset.size(); ++i) descChannelNames.push_back(channelNames[desc.offset[i]]);
        ...
        case PixelFormat::U256:
            for (int y = 0; y < resolution.y; ++y)
                for (int x = 0; x < resolution.x; ++x) {
                    ...
                    for (size_t i = 0; i < desc.offset.size(); ++i) dst[i] = src[desc.offset[i]];
                }
            break;
        case PixelFormat::Half:
            for (int y = 0; y < resolution.y; ++y)
    ...
    Image Image::Crop(const Bounds2i& bounds, Allocator alloc) const {
        ...
        for (Point2i p : bounds)
            for (int c = 0; c < NChannels(); ++c) image.SetChannel(Point2i(p - bounds.pMin), c, GetChannel(p, c));
        return image;
    }


    std::unique_ptr<uint8_t[]> Image::QuantizePixelsToU256(int* nOutOfGamut) const {
        std::unique_ptr<uint8_t[]> u256 = std::make_unique<uint8_t[]>(NChannels() * size_t(resolution.x) * size_t(resolution.y));
        for (int y = 0; y < resolution.y; ++y)
            for (int x = 0; x < resolution.x; ++x)
                for (int c = 0; c < NChannels(); ++c) {
    ...
    bool Image::WritePNG(const std::string& filename, const ImageMetadata& metadata) const {
        ...
        default: SPECTRA_FATAL("Unexpected number of channels in WritePNG()");
        }
...
#define BUFFER_SIZE 80
    ...
    static ImageAndMetadata ReadPFM(const std::string& filename, Allocator alloc) {
        ...
        for (int y = height - 1; y >= 0; --y)
            if (fread(&rgb32[nChannels * y * width], sizeof(float), nChannels * width, fp) != nChannels * width) goto fail;
    ...
    bool Image::WritePFM(const std::string& filename, const ImageMetadata& metadata) const {
        ...
        for (int y = resolution.y - 1; y >= 0; y--) {
            ...
            if (fwrite(&scanline[0], sizeof(float), 3 * resolution.x, fp) < (3 * resolution.x)) goto fail;
        }
