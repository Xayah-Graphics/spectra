#ifndef SPECTRA_PATHTRACER_UTIL_IMAGE_H
#define SPECTRA_PATHTRACER_UTIL_IMAGE_H
...
namespace spectra {
    ...
    __host__ __device__ inline bool Is8Bit(PixelFormat format) {
        return format == PixelFormat::U256;
    }

    __host__ __device__ inline bool Is16Bit(PixelFormat format) {
        return format == PixelFormat::Half;
    }

    __host__ __device__ inline bool Is32Bit(PixelFormat format) {
    ...
    struct WrapMode2D {
        __host__ __device__ WrapMode2D(WrapMode w) : wrap{w, w} {}

        __host__ __device__ WrapMode2D(WrapMode x, WrapMode y) : wrap{x, y} {}
    ...
    __host__ __device__ inline bool RemapPixelCoords(Point2i* pp, Point2i resolution, WrapMode2D wrapMode);
    ...
    __host__ __device__ inline bool RemapPixelCoords(Point2i* pp, Point2i resolution, WrapMode2D wrapMode) {
        ...
        for (int c = 0; c < 2; ++c) {
            ...
            default: SPECTRA_FATAL("Unhandled WrapMode mode");
            }
        }
        return true;
    }
    ...
    struct ImageChannelValues : public InlinedVector<Float, 4> {
        ...
        Float MaxValue() const {
            Float m = (*this)[0];
            for (int i = 1; i < size(); ++i) m = std::max(m, (*this)[i]);
            return m;
        }
    ...
    public:
        ...
        __host__ __device__ int NChannels() const {
        ...
        __host__ __device__ Float GetChannel(Point2i p, int c, WrapMode2D wrapMode = WrapMode::Clamp) const {
            ...
            default: SPECTRA_FATAL("Unhandled PixelFormat"); return 0;
            }
        }

        __host__ __device__ Float BilerpChannel(Point2f p, int c, WrapMode2D wrapMode = WrapMode::Clamp) const {
            ...
            pstd::array<Float, 4> v = {GetChannel({xi, yi}, c, wrapMode), GetChannel({xi + 1, yi}, c, wrapMode), GetChannel({xi, yi + 1}, c, wrapMode), GetChannel({xi + 1, yi + 1}, c, wrapMode)};
            return ((1 - dx) * (1 - dy) * v[0] + dx * (1 - dy) * v[1] + (1 - dx) * dy * v[2] + dx * dy * v[3]);
        }

        __host__ __device__ void SetChannel(Point2i p, int c, Float value);
        ...
        ImageChannelDesc AllChannelsDesc() const {
            ...
            for (int i = 0; i < NChannels(); ++i) desc.offset[i] = i;
            return desc;
        }
        ...
        void CopyRectOut(const Bounds2i& extent, pstd::span<float> buf, WrapMode2D wrapMode = WrapMode::Clamp) const;
        ...
        __host__ __device__ Float LookupNearestChannel(Point2f p, int c, WrapMode2D wrapMode = WrapMode::Clamp) const {
        ...
        void SetChannels(Point2i p, const ImageChannelDesc& desc, pstd::span<const Float> values);
        ...
        __host__ __device__ const void* RawPointer(Point2i p) const {
    ...
    inline void Image::SetChannel(Point2i p, int c, Float value) {
        ...
        default: SPECTRA_FATAL("Unhandled PixelFormat in Image::SetChannel()");
        }
    }
...
#endif // SPECTRA_PATHTRACER_UTIL_IMAGE_H
