...
#include <spectra/pathtracer/util/sobolmatrices.h>
...
namespace spectra {
    ...
    inline SPECTRA_CPU_GPU uint64_t SobolIntervalToIndex(uint32_t log2Scale, uint64_t sampleIndex, Point2i p);
    ...
    SPECTRA_CPU_GPU inline Float RadicalInverse(int baseIndex, uint64_t a) {
    ...
    SPECTRA_CPU_GPU inline uint64_t InverseRadicalInverse(uint64_t inverse, int base, int nDigits) {
        uint64_t index = 0;
    ...
    SPECTRA_CPU_GPU inline Float ScrambledRadicalInverse(int baseIndex, uint64_t a, const DigitPermutation& perm) {
        unsigned int base = Primes[baseIndex];
        // We have to stop once reversedDigits is >= limit since otherwise the
        // next digit of |a| may cause reversedDigits to overflow.
        uint64_t limit = ~0ull / base - base;
    ...
    SPECTRA_CPU_GPU inline Float OwenScrambledRadicalInverse(int baseIndex, uint64_t a, uint32_t hash) {
        unsigned int base = Primes[baseIndex];
        // We have to stop once reversedDigits is >= limit since otherwise the
        // next digit of |a| may cause reversedDigits to overflow.
        uint64_t limit = ~0ull / base - base;
    ...
    SPECTRA_CPU_GPU inline uint32_t MultiplyGenerator(pstd::span<const uint32_t> C, uint32_t a) {
        uint32_t v = 0;
