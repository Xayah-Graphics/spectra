...
namespace spectra {
    ...
    SPECTRA_CPU_GPU pstd::optional<SquareMatrix<N>> Inverse(const SquareMatrix<N>& m) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < N; j++) {
                if (j != icol) {
                    ...
                    minv[j][icol] = 0;
                    for (int k = 0; k < N; k++) minv[j][k] = FMA(-minv[icol][k], save, minv[j][k]);
                }
            }
        }
        ...
        for (int j = N - 1; j >= 0; j--) {
            if (indxr[j] != indxc[j]) {
                for (int k = 0; k < N; k++) pstd::swap(minv[k][indxr[j]], minv[k][indxc[j]]);
            }
        }
    ...
    SPECTRA_CPU_GPU Float InvertCatmullRom(pstd::span<const Float> nodes, pstd::span<const Float> f, Float u) {
        ...
        auto eval = [&](Float t) -> std::pair<Float, Float> {
            ...
            Float Fhat = (2 * t3 - 3 * t2 + 1) * f0 + (-2 * t3 + 3 * t2) * f1 + (t3 - 2 * t2 + t) * d0 + (t3 - t2) * d1;

            // Set _fhat_ using Equation (\ref{eq:cubicspline-derivative})
            Float fhat = (6 * t2 - 6 * t) * f0 + (-6 * t2 + 6 * t) * f1 + (3 * t2 - 4 * t + 1) * d0 + (3 * t2 - 2 * t) * d1;
