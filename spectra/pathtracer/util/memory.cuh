#ifndef SPECTRA_PATHTRACER_UTIL_MEMORY_H
#define SPECTRA_PATHTRACER_UTIL_MEMORY_H
...
namespace spectra {
    ...
    public:
        ...
        void* do_allocate(size_t size, size_t alignment) {
            ...
            while (prevMax < currentBytes && !maxAllocatedBytes.compare_exchange_weak(prevMax, currentBytes));
            return ptr;
        }
    ...
    inline constexpr int ScratchBufferCacheLineSize = 128;

    // ScratchBuffer Definition
    class alignas(ScratchBufferCacheLineSize) ScratchBuffer {
    public:
        ...
        void* Alloc(size_t size, size_t align) {
            ...
            if (offset + size > allocSize) Realloc(size);
            void* p = ptr + offset;
            offset += size;
            return p;
        }
        ...
        typename AllocationTraits<T>::Array Alloc(size_t n = 1) {
            ...
            ElementType* ret  = (ElementType*) Alloc(n * sizeof(ElementType), alignof(ElementType));
            for (size_t i = 0; i < n; ++i) new (&ret[i]) ElementType();
            return ret;
        }

        void Reset() {
            for (const auto& buf : smallBuffers) Allocator().deallocate_bytes(buf.first, buf.second, align);
            smallBuffers.clear();
            offset = 0;
        }
    ...
    private:
        ...
        static constexpr int align = ScratchBufferCacheLineSize;
        char* ptr                  = nullptr;
...
#endif // SPECTRA_PATHTRACER_UTIL_MEMORY_H
