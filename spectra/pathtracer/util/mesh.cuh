#ifndef SPECTRA_PATHTRACER_UTIL_MESH_H
#define SPECTRA_PATHTRACER_UTIL_MESH_H
...
namespace spectra {
    ...
    struct HashIntPair {
        __host__ __device__ size_t operator()(std::pair<int, int> p) const {
    ...
    struct TriQuadMesh {
        ...
        TriQuadMesh Displace(Dist&& dist, Float maxDist, Disp&& displace, const FileLoc* loc = nullptr) const {
            ...
            if (outputMesh.n.empty()) outputMesh.ComputeNormals();
            ...
            for (int i = 0; i < oldTriIndices.size() / 3; ++i) outputMesh.Refine(dist, maxDist, oldTriIndices[3 * i], oldTriIndices[3 * i + 1], oldTriIndices[3 * i + 2], edgeSplit);

            // Displace
            displace(outputMesh.p.data(), outputMesh.n.data(), outputMesh.uv.data(), outputMesh.p.size());
    ...
    private:
        ...
        void Refine(Dist&& distance, Float maxDist, int v0, int v1, int v2, HashMap<std::pair<int, int>, int, HashIntPair>& edgeSplit) {
            ...
            if (edgeSplit.HasKey(edge)) {
                ...
                if (!n.empty()) {
                    ...
                    if (LengthSquared(nn) > 0) nn = Normalize(nn);
                    n.push_back(nn);
                }
                if (!uv.empty()) uv.push_back((uv[v[0]] + uv[v[1]]) / 2);
            }
...
#endif // SPECTRA_PATHTRACER_UTIL_MESH_H
