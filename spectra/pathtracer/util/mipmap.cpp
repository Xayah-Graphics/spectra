...
namespace spectra {
    ...
    */
    ...
    static SPECTRA_CONST Float MIPFilterLUT[MIPFilterLUTSize] = {
    ...
    T MIPMap::Filter(Point2f st, Vector2f dst0, Vector2f dst1) const {
        if (options.filter != FilterFunction::EWA) {
            ...
            Float width = 2 * std::max({std::abs(dst0[0]), std::abs(dst0[1]), std::abs(dst1[0]), std::abs(dst1[1])});
            // Compute MIP Map level for _width_ and handle very wide filter
            int nLevels = Levels();
            Float level = nLevels - 1 + Log2(std::max<Float>(width, 1e-8));
            if (level >= Levels() - 1) return Texel<T>(Levels() - 1, Point2i(0, 0));
            ...
            if (options.filter == FilterFunction::Point) {
                ...
                Point2i sti(pstd::round(st[0] * resolution[0] - 0.5f), pstd::round(st[1] * resolution[1] - 0.5f));
        ...
        if (shorterVecLength * options.maxAnisotropy < longerVecLength && shorterVecLength > 0) {
            Float scale = longerVecLength / (shorterVecLength * options.maxAnisotropy);
            dst1 *= scale;
            shorterVecLength *= scale;
        }
        if (shorterVecLength == 0) return Bilerp<T>(0, st);

        // Choose level of detail for EWA lookup and perform EWA filtering
        Float lod = std::max<Float>(0, Levels() - 1 + Log2(shorterVecLength));
        int ilod  = pstd::floor(lod);
        return Lerp(lod - ilod, EWA<T>(ilod, st, dst0, dst1), EWA<T>(ilod + 1, st, dst0, dst1));
    }
    ...
    MIPMap* MIPMap::CreateFromFile(const std::string& filename, const MIPMapFilterOptions& options, WrapMode wrapMode, ColorEncoding encoding, Allocator alloc) {
            ...
            if (rgbaDesc) {
                ...
                for (int y = 0; y < image.Resolution().y; ++y)
                    for (int x = 0; x < image.Resolution().x; ++x)
                        if (image.GetChannels({x, y}, rgbaDesc)[3] != 1) allOne = false;
                if (allOne)
        ...
        return alloc.new_object<MIPMap>(std::move(image), colorSpace, wrapMode, alloc, options);
    }
    ...
    Float MIPMap::Bilerp(int level, Point2f st) const {
        ...
        case 1: return pyramid[level].BilerpChannel(st, 0, wrapMode);
        case 3: return pyramid[level].Bilerp(st, wrapMode).Average();
        case 4:
            // Return alpha
            return pyramid[level].BilerpChannel(st, 3, wrapMode);
        default: SPECTRA_FATAL("Unexpected number of image channels: %d", pyramid[level].NChannels());
        }
    }
