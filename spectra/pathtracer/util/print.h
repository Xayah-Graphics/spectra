#ifndef SPECTRA_PATHTRACER_UTIL_PRINT_H
#define SPECTRA_PATHTRACER_UTIL_PRINT_H
...
namespace spectra
{
    ...
    template <typename T>
    inline std::ostream& operator<<(std::ostream& os, const cuda::std::optional<T>& opt)
    {
        if (opt.has_value())
    ...
    namespace detail
    {
...
#ifdef SPECTRA_INT64_IS_OWN_TYPE
        template <>
        ...
        formatOne(const char* fmt, T&& v)
        {
            SPECTRA_FATAL("Printf: Non-basic type %s passed for format string %s", typeid(v).name(),
                      fmt);
            return "";
        }
...
#pragma warning(disable : 4102)  // bogus "unreferenced label" warning for done: below
                                          ...
                                          Args&&... args)
        {
            ...
            if constexpr (std::is_integral_v<std::decay_t<T>>)
            {
                ...
                SPECTRA_FATAL("Non-integral type provided for %* format.");
            ...
            if constexpr (std::is_integral_v<std::decay_t<T>>)
            {
                if (isDFmt)
                {
                    nextFmt.replace(nextFmt.find('d'), 1,
                                    IntegerFormatTrait<std::decay_t<T>>::fmt());
                    *s += formatOne(nextFmt.c_str(), std::forward<T>(v));
                    goto done;
                }
            }
            else if (isDFmt)
                SPECTRA_FATAL("Non-integral type passed to %d format.");
            ...
            if (isSFmt)
            {
                ...
                *s += formatOne(nextFmt.c_str(), std::forward<T>(v));
                ...
                SPECTRA_FATAL("Excess values passed to Printf.");
...
#endif  // SPECTRA_PATHTRACER_UTIL_PRINT_H
