...
namespace pstd {
    ...
    namespace span_internal {
        ...
        template <typename C>
        __host__ __device__ inline constexpr auto GetDataImpl(C& c, char) noexcept -> decltype(c.data()) {
            return c.data();
        }

        template <typename C>
        __host__ __device__ inline constexpr auto GetData(C& c) noexcept -> decltype(GetDataImpl(c, 0)) {
            return GetDataImpl(c, 0);
        }
    } // namespace span_internal

    template <int&... ExplicitArgumentBarrier, typename T>
    __host__ __device__ inline constexpr span<T> MakeSpan(T* ptr, size_t size) noexcept {
        return span<T>(ptr, size);
    }

    template <int&... ExplicitArgumentBarrier, typename T>
    __host__ __device__ span<T> MakeSpan(T* begin, T* end) noexcept {
    ...
    __host__ __device__ inline constexpr auto MakeSpan(C& c) noexcept -> decltype(MakeSpan(span_internal::GetData(c), static_cast<size_t>(c.size()))) {
        return MakeSpan(span_internal::GetData(c), static_cast<size_t>(c.size()));
    }

    template <int&... ExplicitArgumentBarrier, typename T, size_t N>
    __host__ __device__ inline constexpr span<T> MakeSpan(T (&array)[N]) noexcept {
        return span<T>(array, N);
    }

    template <int&... ExplicitArgumentBarrier, typename T>
    __host__ __device__ inline constexpr span<const T> MakeConstSpan(T* ptr, size_t size) noexcept {
        return span<const T>(ptr, size);
    }

    template <int&... ExplicitArgumentBarrier, typename T>
    __host__ __device__ span<const T> MakeConstSpan(T* begin, T* end) noexcept {
    ...
    template <int&... ExplicitArgumentBarrier, typename C>
    __host__ __device__ inline constexpr auto MakeConstSpan(const C& c) noexcept -> decltype(MakeSpan(c)) {
        return MakeSpan(c);
    }

    template <int&... ExplicitArgumentBarrier, typename T, size_t N>
    __host__ __device__ inline constexpr span<const T> MakeConstSpan(const T (&array)[N]) noexcept {
    ...
    public:
        ...
        vector(const vector& other, const Allocator& alloc = {}) : alloc(alloc) {
            reserve(other.size());
            for (size_t i = 0; i < other.size(); ++i) this->alloc.template construct<T>(ptr + i, other[i]);
            nStored = other.size();
        }
        ...
        vector(InputIt first, InputIt last, const Allocator& alloc = {}) : alloc(alloc) {
            reserve(last - first);
            size_t i = 0;
            for (InputIt iter = first; iter != last; ++iter, ++i) this->alloc.template construct<T>(ptr + i, *iter);
            nStored = nAlloc;
        }
        ...
        vector& operator=(const vector& other) {
            ...
            clear();
            reserve(other.size());
            for (size_t i = 0; i < other.size(); ++i) alloc.template construct<T>(ptr + i, other[i]);
            nStored = other.size();
        ...
        void clear() {
            for (int i = 0; i < nStored; ++i) alloc.destroy(&ptr[i]);
            nStored = 0;
        }
        ...
        iterator insert(const_iterator pos, InputIt first, InputIt last) {
            if (pos == end()) {
                ...
                for (auto iter = first; iter != last; ++iter) push_back(*iter);
        ...
        void emplace_back(Args&&... args) {
            if (nAlloc == nStored) reserve(nAlloc == 0 ? 4 : 2 * nAlloc);
        ...
        void resize(size_type n) {
            if (n < size()) {
                for (size_t i = n; i < size(); ++i) alloc.destroy(ptr + i);
                if (n == 0) {
                    alloc.deallocate_object(ptr, nAlloc);
                    ptr    = nullptr;
                    nAlloc = 0;
                ...
                reserve(n);
                for (size_t i = nStored; i < n; ++i) alloc.construct(ptr + i);
            }
            nStored = n;
        }
