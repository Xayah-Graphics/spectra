...
namespace pstd {
    ...
    namespace span_internal {
        ...
        template <typename C>
        SPECTRA_CPU_GPU inline constexpr auto GetDataImpl(C& c, char) noexcept -> decltype(c.data()) {
            return c.data();
        }
    ...
    SPECTRA_CPU_GPU inline constexpr auto MakeSpan(C& c) noexcept -> decltype(MakeSpan(span_internal::GetData(c), static_cast<size_t>(c.size()))) {
    ...
    template <int&... ExplicitArgumentBarrier, typename C>
    SPECTRA_CPU_GPU inline constexpr auto MakeConstSpan(const C& c) noexcept -> decltype(MakeSpan(c)) {
        return MakeSpan(c);
    }
