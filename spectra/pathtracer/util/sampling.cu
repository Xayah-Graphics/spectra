...
namespace spectra {
    ...
    SPECTRA_CPU_GPU pstd::array<Float, 3> SampleSphericalTriangle(const pstd::array<Point3f, 3>& v, Point3f p, Point2f u, Float* pdf) {
        ...
        if (LengthSquared(n_ab) == 0 || LengthSquared(n_bc) == 0 || LengthSquared(n_ca) == 0) return {};
        n_ab = Normalize(n_ab);
        n_bc = Normalize(n_bc);
        n_ca = Normalize(n_ca);
    ...
    SPECTRA_CPU_GPU Point3f SampleSphericalRectangle(Point3f pRef, Point3f s, Vector3f ex, Vector3f ey, Point2f u, Float* pdf) {
        ...
        if (pdf) *pdf = std::max<Float>(0, 1 / solidAngle);
        if (solidAngle < 1e-3) return Point3f(s + u[0] * ex + u[1] * ey);
    ...
    SPECTRA_CPU_GPU Point2f InvertSphericalRectangleSample(Point3f pRef, Point3f s, Vector3f ex, Vector3f ey, Point3f pRect) {
        ...
        Float au = std::atan2(-(b1 * fu) - pstd::copysign(b0 * sqrt, fu * b0), b0 * b1 - sqrt * std::abs(fu));
        if (au > 0) au -= 2 * Pi;
        ...
        Float u1[2] = {(DifferenceOfProducts(h0, h0, h0, h1) - std::abs(h0 - h1) * std::sqrt(yvsq * (ddsq + yvsq)) / (ddsq + yvsq)) / Sqr(h0 - h1), (DifferenceOfProducts(h0, h0, h0, h1) + std::abs(h0 - h1) * std::sqrt(yvsq * (ddsq + yvsq)) / (ddsq + yvsq)) / Sqr(h0 - h1)};
        ...
        Point2f u = (std::abs(yz[0] - yv) < std::abs(yz[1] - yv)) ? Point2f(Clamp(u0, 0, 1), u1[0]) : Point2f(Clamp(u0, 0, 1), u1[1]);
    ...
    SPECTRA_CPU_GPU Float SampleCatmullRom(pstd::span<const Float> nodes, pstd::span<const Float> f, pstd::span<const Float> F, Float u, Float* fval, Float* pdf) {
        ...
        auto eval = [&](Float t) -> std::pair<Float, Float> {
            Fhat = EvaluatePolynomial(t, 0, f0, 0.5f * d0, (1.f / 3.f) * (-2 * d0 - d1) + f1 - f0, 0.25f * (d0 + d1) + 0.5f * (f0 - f1));
            fhat = EvaluatePolynomial(t, f0, d0, -2 * d0 - d1 + 3 * (f1 - f0), d0 + d1 + 2 * (f0 - f1));
            return {Fhat - u, fhat};
        };
    ...
    SPECTRA_CPU_GPU Float SampleCatmullRom2D(pstd::span<const Float> nodes1, pstd::span<const Float> nodes2, pstd::span<const Float> values, pstd::span<const Float> cdf, Float alpha, Float u, Float* fval, Float* pdf) {
        ...
        auto interpolate = [&](pstd::span<const Float> array, int idx) {
            Float v = 0;
            for (int i = 0; i < 4; ++i)
                if (weights[i] != 0) v += array[(offset + i) * nodes2.size() + idx] * weights[i];
            return v;
        };
        ...
        u *= maximum;
        int idx = FindInterval(nodes2.size(), [&](int i) { return interpolate(cdf, i) <= u; });
    ...
    AliasTable::AliasTable(pstd::span<const Float> weights, Allocator alloc) : bins(weights.size(), alloc) {
        // Normalize _weights_ to compute alias table PDF
        Float sum = std::accumulate(weights.begin(), weights.end(), 0.);
        CHECK_GT(sum, 0);
        for (size_t i = 0; i < weights.size(); ++i) bins[i].p = weights[i] / sum;
