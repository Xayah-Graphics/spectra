...
#include <spectra/pathtracer/util/lowdiscrepancy.h>  // yuck: for Hammersley generator...
...
namespace spectra
{
    ...
    public:
        ...
        explicit PiecewiseConstant2D(const Array2D<Float>& data, Allocator alloc = {})
            : PiecewiseConstant2D(
                pstd::span<const Float>(data.begin(), static_cast<size_t>(data.size())),
                data.XSize(), data.YSize(), alloc)
        {
        }
        ...
        PiecewiseConstant2D(const Array2D<Float>& data, Bounds2f domain, Allocator alloc = {})
            : PiecewiseConstant2D(
                pstd::span<const Float>(data.begin(), static_cast<size_t>(data.size())),
                data.XSize(), data.YSize(), domain, alloc)
        {
        }
    ...
    namespace detail
    {
        ...
        public:
            ...
            SPECTRA_CPU_GPU IndexingIterator(int i, int n, const Generator*) : i(i), n(n)
            {
            }
        ...
        public:
            template <typename Generator>
            SPECTRA_CPU_GPU RNGIterator(int i, int n,
    ...
    {
    public:
        using RNGGenerator<Uniform1D, detail::Uniform1DIter>::RNGGenerator;
    };

    class Uniform2D : public detail::RNGGenerator<Uniform2D, detail::Uniform2DIter>
    {
    public:
        using RNGGenerator<Uniform2D, detail::Uniform2DIter>::RNGGenerator;
    };

    class Uniform3D : public detail::RNGGenerator<Uniform3D, detail::Uniform3DIter>
    {
    public:
        using RNGGenerator<Uniform3D, detail::Uniform3DIter>::RNGGenerator;
    };
    ...
    {
    public:
        using IndexingGenerator<Hammersley2D,
    ...
    {
    public:
        using IndexingGenerator<Hammersley3D,
    ...
    {
    public:
        using RNGGenerator<Stratified1D, detail::Stratified1DIter>::RNGGenerator;
    };
    ...
    public:
        ...
        Stratified2D(int nx, int ny, uint64_t sequenceIndex = 0,
            ...
            : RNGGenerator<Stratified2D, detail::Stratified2DIter>(
    ...
    public:
        ...
        Stratified3D(int nx, int ny, int nz, uint64_t sequenceIndex = 0,
                     uint64_t seed = PCG32_DEFAULT_STATE)
            : RNGGenerator<Stratified3D, detail::Stratified3DIter>(
    ...
    namespace detail
    {
        ...
        public:
            ...
            Point3f operator*()
            {
                ...
                int iz = i / (nx * ny);
                return {
                    (ix + rng.Uniform<Float>()) / nx, (iy + rng.Uniform<Float>()) / ny,
    ...
    public:
              ...
              m_conditional_cdf(alloc)
        {
            if (build_cdf && !normalize)
                SPECTRA_FATAL("PiecewiseLinear2D: build_cdf implies normalize=true");
            ...
            {
                if (param_res[i] < 1)
                    SPECTRA_FATAL("PiecewiseLinear2D(): parameter resolution must be >= 1!");
