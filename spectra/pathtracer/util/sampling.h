...
namespace spectra {
    ...
    SPECTRA_CPU_GPU inline int SampleDiscrete(pstd::span<const Float> weights, Float u, Float* pmf = nullptr, Float* uRemapped = nullptr);
    ...
    SPECTRA_CPU_GPU
    pstd::array<Float, 3> SampleSphericalTriangle(const pstd::array<Point3f, 3>& v, Point3f p, Point2f u, Float* pdf = nullptr);

    SPECTRA_CPU_GPU
    Point2f InvertSphericalTriangleSample(const pstd::array<Point3f, 3>& v, Point3f p, Vector3f w);
    ...
    SPECTRA_CPU_GPU
    Float SampleCatmullRom(pstd::span<const Float> nodes, pstd::span<const Float> f, pstd::span<const Float> cdf, Float sample, Float* fval = nullptr, Float* pdf = nullptr);

    SPECTRA_CPU_GPU
    Float SampleCatmullRom2D(pstd::span<const Float> nodes1, pstd::span<const Float> nodes2, pstd::span<const Float> values, pstd::span<const Float> cdf, Float alpha, Float sample, Float* fval = nullptr, Float* pdf = nullptr);

    // Sampling Inline Functions
    SPECTRA_CPU_GPU inline Float BalanceHeuristic(int nf, Float fPdf, int ng, Float gPdf) {
        return (nf * fPdf) / (nf * fPdf + ng * gPdf);
    }

    SPECTRA_CPU_GPU inline Float PowerHeuristic(int nf, Float fPdf, int ng, Float gPdf) {
        Float f = nf * fPdf, g = ng * gPdf;
        if (IsInf(Sqr(f))) return 1;
        return Sqr(f) / (Sqr(f) + Sqr(g));
    }

    SPECTRA_CPU_GPU inline int SampleDiscrete(pstd::span<const Float> weights, Float u, Float* pmf, Float* uRemapped) {
    ...
    SPECTRA_CPU_GPU inline Float LinearPDF(Float x, Float a, Float b) {
        DCHECK(a >= 0 && b >= 0);
        if (x < 0 || x > 1) return 0;
    ...
    SPECTRA_CPU_GPU inline Float BilinearPDF(Point2f p, pstd::span<const Float> w) {
        DCHECK_EQ(4, w.size());
    ...
    SPECTRA_CPU_GPU inline Point2f SampleBilinear(Point2f u, pstd::span<const Float> w) {
        DCHECK_EQ(4, w.size());
        Point2f p;
    ...
    SPECTRA_CPU_GPU inline Point2f InvertBilinearSample(Point2f p, pstd::span<const Float> w) {
        return {InvertLinearSample(p.x, Lerp(p.y, w[0], w[2]), Lerp(p.y, w[1], w[3])), InvertLinearSample(p.y, w[0] + w[1], w[2] + w[3])};
    }

    SPECTRA_CPU_GPU inline Float VisibleWavelengthsPDF(Float lambda) {
        if (lambda < 360 || lambda > 830) return 0;
        return 0.0039398042f / Sqr(std::cosh(0.0072f * (lambda - 538)));
    }

    SPECTRA_CPU_GPU inline Float SampleVisibleWavelengths(Float u) {
        return 538 - 138.888889f * std::atanh(0.85691062f - 1.82750197f * u);
    }

    SPECTRA_CPU_GPU inline pstd::array<Float, 3> SampleUniformTriangle(Point2f u) {
        Float b0, b1;
        if (u[0] < u[1]) {
            b0 = u[0] / 2;
            b1 = u[1] - b0;
    ...
    SPECTRA_CPU_GPU inline Float SampleTent(Float u, Float r) {
    ...
    SPECTRA_CPU_GPU inline Float TentPDF(Float x, Float r) {
    ...
    SPECTRA_CPU_GPU inline Float ExponentialPDF(Float x, Float a) {
        DCHECK_GT(a, 0);
        return a * std::exp(-a * x);
    }

    SPECTRA_CPU_GPU inline Float SampleExponential(Float u, Float a) {
        DCHECK_GT(a, 0);
        return -std::log(1 - u) / a;
    }

    SPECTRA_CPU_GPU inline Float InvertExponentialSample(Float x, Float a) {
        DCHECK_GT(a, 0);
        return 1 - std::exp(-a * x);
    }

    SPECTRA_CPU_GPU inline Float NormalPDF(Float x, Float mu = 0, Float sigma = 1) {
        return Gaussian(x, mu, sigma);
    }

    SPECTRA_CPU_GPU inline Float SampleNormal(Float u, Float mu = 0, Float sigma = 1) {
    ...
    SPECTRA_CPU_GPU inline Point2f SampleTwoNormal(Point2f u, Float mu = 0, Float sigma = 1) {
    ...
    SPECTRA_CPU_GPU inline Float LogisticPDF(Float x, Float s) {
        x = std::abs(x);
        return std::exp(-x / s) / (s * Sqr(1 + std::exp(-x / s)));
    }

    SPECTRA_CPU_GPU inline Float SampleLogistic(Float u, Float s) {
        return -s * std::log(1 / u - 1);
    }

    SPECTRA_CPU_GPU inline Float InvertLogisticSample(Float x, Float s) {
        return 1 / (1 + std::exp(-x / s));
    }

    SPECTRA_CPU_GPU inline Float TrimmedLogisticPDF(Float x, Float s, Float a, Float b) {
        if (x < a || x > b) return 0;
        auto P = [&](Float x) { return InvertLogisticSample(x, s); };
        return Logistic(x, s) / (P(b) - P(a));
    }

    SPECTRA_CPU_GPU inline Float SampleTrimmedLogistic(Float u, Float s, Float a, Float b) {
        DCHECK_LT(a, b);
        auto P  = [&](Float x) { return InvertLogisticSample(x, s); };
        u       = Lerp(u, P(a), P(b));
    ...
    SPECTRA_CPU_GPU inline Float InvertTrimmedLogisticSample(Float x, Float s, Float a, Float b) {
        DCHECK(a <= x && x <= b);
        auto P = [&](Float x) { return InvertLogisticSample(x, s); };
        return (P(x) - P(a)) / (P(b) - P(a));
    }

    SPECTRA_CPU_GPU inline Float SmoothStepPDF(Float x, Float a, Float b) {
        if (x < a || x > b) return 0;
        DCHECK_LT(a, b);
        return (2 / (b - a)) * SmoothStep(x, a, b);
    }

    SPECTRA_CPU_GPU inline Float SampleSmoothStep(Float u, Float a, Float b) {
        DCHECK_LT(a, b);
    ...
    SPECTRA_CPU_GPU inline Float InvertSmoothStepSample(Float x, Float a, Float b) {
        Float t = (x - a) / (b - a);
        auto P  = [&](Float x) { return 2 * Pow<3>(t) - Pow<4>(t); };
        return (P(x) - P(a)) / (P(b) - P(a));
    }

    SPECTRA_CPU_GPU inline Point2f SampleUniformDiskPolar(Point2f u) {
    ...
    SPECTRA_CPU_GPU inline Point2f SampleUniformDiskConcentric(Point2f u) {
    ...
    SPECTRA_CPU_GPU inline Vector3f SampleUniformHemisphere(Point2f u) {
        Float z   = u[0];
    ...
    SPECTRA_CPU_GPU inline Float UniformHemispherePDF() {
        return Inv2Pi;
    }

    SPECTRA_CPU_GPU inline Point2f InvertUniformHemisphereSample(Vector3f w) {
        Float phi = std::atan2(w.y, w.x);
        if (phi < 0) phi += 2 * Pi;
        return Point2f(w.z, phi / (2 * Pi));
    }

    SPECTRA_CPU_GPU inline Vector3f SampleUniformSphere(Point2f u) {
        Float z   = 1 - 2 * u[0];
        Float r   = SafeSqrt(1 - Sqr(z));
        Float phi = 2 * Pi * u[1];
        return {r * std::cos(phi), r * std::sin(phi), z};
    ...
    SPECTRA_CPU_GPU inline Float UniformSpherePDF() {
        return Inv4Pi;
    }

    SPECTRA_CPU_GPU inline Point2f InvertUniformSphereSample(Vector3f w) {
        Float phi = std::atan2(w.y, w.x);
        if (phi < 0) phi += 2 * Pi;
        return Point2f((1 - w.z) / 2, phi / (2 * Pi));
    }

    SPECTRA_CPU_GPU inline Vector3f SampleCosineHemisphere(Point2f u) {
        Point2f d = SampleUniformDiskConcentric(u);
        Float z   = SafeSqrt(1 - Sqr(d.x) - Sqr(d.y));
        return Vector3f(d.x, d.y, z);
    }

    SPECTRA_CPU_GPU inline Float CosineHemispherePDF(Float cosTheta) {
        return cosTheta * InvPi;
    }

    SPECTRA_CPU_GPU inline Point2f InvertCosineHemisphereSample(Vector3f w) {
        return InvertUniformDiskConcentricSample({w.x, w.y});
    }

    SPECTRA_CPU_GPU inline Float UniformConePDF(Float cosThetaMax) {
        return 1 / (2 * Pi * (1 - cosThetaMax));
    }

    SPECTRA_CPU_GPU inline Vector3f SampleUniformCone(Point2f u, Float cosThetaMax) {
    ...
    SPECTRA_CPU_GPU inline Point2f InvertUniformConeSample(Vector3f w, Float cosThetaMax) {
        Float cosTheta = w.z;
    ...
    public:
        ...
        template <typename F>
        SPECTRA_CPU_GPU bool Add(F func, Float weight) {
    ...
    private:
        ...
        SPECTRA_CPU_GPU static Float SampleBisection(CDF P, Float u, Float min, Float max, int n) {
    ...
    public:
        ...
        SPECTRA_CPU_GPU PLSample Sample(Point2f sample, Ts... params) const {
        ...
        template <typename... Ts>
        SPECTRA_CPU_GPU PLSample Invert(Point2f sample, Ts... params) const {
            static_assert((std::is_arithmetic_v<Ts> && ...), "Additional parameters must be numeric values");
            static_assert(sizeof...(Ts) == Dimension, "Incorrect number of additional parameters passed");
            pstd::array<Float, Dimension> param = {params...};
         ...
         */
        template <typename... Ts>
        SPECTRA_CPU_GPU float Evaluate(Point2f pos, Ts... params) const {
            static_assert((std::is_arithmetic_v<Ts> && ...), "Additional parameters must be numeric values");
            static_assert(sizeof...(Ts) == Dimension, "Incorrect number of additional parameters passed");
            pstd::array<Float, Dimension> param = {params...};
    ...
    private:
        ...
        SPECTRA_CPU_GPU Float lookup(const float* data, uint32_t i0, uint32_t size, const float* param_weight) const {
        ...
        template <size_t Dim, std::enable_if_t<Dim == 0, int> = 0>
        SPECTRA_CPU_GPU Float lookup(const float* data, uint32_t index, uint32_t, const float*) const {
            return data[index];
        }
