#ifndef SPECTRA_PATHTRACER_UTIL_SCATTERING_H
#define SPECTRA_PATHTRACER_UTIL_SCATTERING_H
...
namespace spectra {
    ...
    __host__ __device__ inline Vector3f Reflect(Vector3f wo, Vector3f n) {
        return -wo + 2 * Dot(wo, n) * n;
    }

    __host__ __device__ inline bool Refract(Vector3f wi, Normal3f n, Float eta, Float* etap, Vector3f* wt) {
    ...
    __host__ __device__ inline Float HenyeyGreenstein(Float cosTheta, Float g) {
    ...
    __host__ __device__ inline Float FrComplex(Float cosTheta_i, pstd::complex<Float> eta) {
        ...
        Complex cosTheta_t  = pstd::sqrt(Complex(1) - sin2Theta_t);
    ...
    __host__ __device__ inline SampledSpectrum FrComplex(Float cosTheta_i, SampledSpectrum eta, SampledSpectrum k) {
        SampledSpectrum result;
        for (int i = 0; i < NSpectrumSamples; ++i) result[i] = FrComplex(cosTheta_i, pstd::complex<Float>(eta[i], k[i]));
        return result;
    }

    // BSSRDF Utility Declarations
    __host__ __device__ Float FresnelMoment1(Float invEta);
    __host__ __device__ Float FresnelMoment2(Float invEta);
    ...
    public:
        ...
        __host__ __device__ TrowbridgeReitzDistribution(Float ax, Float ay) : alpha_x(ax), alpha_y(ay) {
        ...
        __host__ __device__ inline Float D(Vector3f wm) const {
        ...
        __host__ __device__ bool EffectivelySmooth() const {
            return std::max(alpha_x, alpha_y) < 1e-3f;
        }

        __host__ __device__ Float G1(Vector3f w) const {
            return 1 / (1 + Lambda(w));
        }

        __host__ __device__ Float Lambda(Vector3f w) const {
        ...
        __host__ __device__ Float G(Vector3f wo, Vector3f wi) const {
            return 1 / (1 + Lambda(wo) + Lambda(wi));
        }

        __host__ __device__ Float D(Vector3f w, Vector3f wm) const {
            return G1(w) / AbsCosTheta(w) * D(wm) * AbsDot(w, wm);
        }

        __host__ __device__ Float PDF(Vector3f w, Vector3f wm) const {
            return D(w, wm);
        }

        __host__ __device__ Vector3f Sample_wm(Vector3f w, Point2f u) const {
...
#endif // SPECTRA_PATHTRACER_UTIL_SCATTERING_H
