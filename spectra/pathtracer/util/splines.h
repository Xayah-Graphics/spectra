...
namespace spectra
{
    ...
    SPECTRA_CPU_GPU inline pstd::array<Point3f, 7> SubdivideCubicBezier(
    ...
    SPECTRA_CPU_GPU inline pstd::array<Point3f, 4> CubicBezierControlPoints(
        pstd::span<const Point3f> cp, Float uMin, Float uMax)
    {
        return {
            BlossomCubicBezier(cp, uMin, uMin, uMin),
    ...
    SPECTRA_CPU_GPU inline pstd::array<Point3f, 4> ElevateQuadraticBezierToCubic(
        pstd::span<const Point3f> cp)
    {
        return {cp[0], Lerp(2.f / 3.f, cp[0], cp[1]), Lerp(1.f / 3.f, cp[1], cp[2]), cp[2]};
    }

    SPECTRA_CPU_GPU inline pstd::array<Point3f, 3> QuadraticBSplineToBezier(
        pstd::span<const Point3f> cp)
    {
    ...
    SPECTRA_CPU_GPU inline pstd::array<Point3f, 4> CubicBSplineToBezier(
        pstd::span<const Point3f> cp)
    {
