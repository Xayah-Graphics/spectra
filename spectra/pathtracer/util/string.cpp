...
namespace spectra {
    ...
    std::vector<Float> SplitStringToFloats(std::string_view str, char ch) {
        ...
        for (size_t i = 0; i < strs.size(); ++i)
            if (!Atof(strs[i], &floats[i])) return {};
        return floats;
    }
    ...
    std::vector<double> SplitStringToDoubles(std::string_view str, char ch) {
        ...
        for (size_t i = 0; i < strs.size(); ++i)
            if (!Atof(strs[i], &doubles[i])) return {};
        return doubles;
    }
    ...
    std::string UTF8FromUTF16(std::u16string str) {
        ...
        for (size_t i = 0; i < str.size(); ++i) {
            ...
            if (codepoint >= 0xd800 && codepoint <= 0xdbff) {
                if (i + 1 == str.size()) throw std::runtime_error(spectra::diagnostics::Format("Invalid UTF-16 string: missing low surrogate"));
                utf8proc_int32_t low = str[++i];
                if (low < 0xdc00 || low > 0xdfff) throw std::runtime_error(spectra::diagnostics::Format("Invalid UTF-16 string: malformed surrogate pair"));
            ...
            if (length <= 0) throw std::runtime_error(spectra::diagnostics::Format("Invalid UTF-16 code point: %d", codepoint));
    ...
    std::u16string UTF16FromUTF8(std::string str) {
        ...
        while (remaining > 0) {
            ...
            if (length < 0) throw std::runtime_error(spectra::diagnostics::Format("Invalid UTF-8 string: %s", utf8proc_errmsg(length)));
            if (codepoint <= 0xffff)
    ...
    std::string NormalizeUTF8(std::string str) {
        ...
        utf8proc_ssize_t length = utf8proc_map((const unsigned char*) str.data(), str.size(), &result, options);
        if (length < 0) throw std::runtime_error(spectra::diagnostics::Format("Unicode normalization error: %s: \"%s\"", utf8proc_errmsg(length), str));
