...
namespace spectra {
    ...
    namespace {
        SPECTRA_CPU_GPU inline bool IsNaN(Interval fi) {
    ...
    public:
        ...
        template <typename U>
        SPECTRA_CPU_GPU auto operator+(Child<U> c) const -> Child<decltype(T{} + U{})> {
            DCHECK(!c.HasNaN());
            return {x + c.x, y + c.y};
        }

        template <typename U>
        SPECTRA_CPU_GPU Child<T>& operator+=(Child<U> c) {
            DCHECK(!c.HasNaN());
            x += c.x;
            y += c.y;
            return static_cast<Child<T>&>(*this);
        ...
        template <typename U>
        SPECTRA_CPU_GPU auto operator*(U s) const -> Child<decltype(T{} * U{})> {
            return {s * x, s * y};
        }

        template <typename U>
        SPECTRA_CPU_GPU Child<T>& operator*=(U s) {
            DCHECK(!IsNaN(s));
            x *= s;
            y *= s;
            return static_cast<Child<T>&>(*this);
        ...
        template <typename U>
        SPECTRA_CPU_GPU auto operator/(U d) const -> Child<decltype(T{} / U{})> {
            DCHECK(d != 0 && !IsNaN(d));
            return {x / d, y / d};
        }

        template <typename U>
        SPECTRA_CPU_GPU Child<T>& operator/=(U d) {
            DCHECK_NE(d, 0);
            DCHECK(!IsNaN(d));
            x /= d;
            y /= d;
    ...
    public:
        ...
        Vector2() = default;
        SPECTRA_CPU_GPU
        Vector2(T x, T y) : Tuple2<spectra::Vector2, T>(x, y) {}

        template <typename U>
        SPECTRA_CPU_GPU explicit Vector2(Point2<U> p);

        template <typename U>
        SPECTRA_CPU_GPU explicit Vector2(Vector2<U> v) : Tuple2<spectra::Vector2, T>(T(v.x), T(v.y)) {}
    };
    ...
    public:
        ...
        Vector3() = default;
        SPECTRA_CPU_GPU
        Vector3(T x, T y, T z) : Tuple3<spectra::Vector3, T>(x, y, z) {}

        template <typename U>
        SPECTRA_CPU_GPU explicit Vector3(Vector3<U> v) : Tuple3<spectra::Vector3, T>(T(v.x), T(v.y), T(v.z)) {}

        template <typename U>
        SPECTRA_CPU_GPU explicit Vector3(Point3<U> p);
        template <typename U>
        SPECTRA_CPU_GPU explicit Vector3(Normal3<U> n);
    };
    ...
    public:
        ...
        template <typename T>
        SPECTRA_CPU_GPU explicit Vector3fi(Point3<T> p) : Vector3<Interval>(Interval(p.x), Interval(p.y), Interval(p.z)) {}

        SPECTRA_CPU_GPU Vector3fi(Vector3<Interval> pfi) : Vector3<Interval>(pfi) {}

        SPECTRA_CPU_GPU
        Vector3fi(Vector3f v, Vector3f e) : Vector3<Interval>(Interval::FromValueAndError(v.x, e.x), Interval::FromValueAndError(v.y, e.y), Interval::FromValueAndError(v.z, e.z)) {}
    ...
    public:
        ...
        SPECTRA_CPU_GPU
        Point2(T x, T y) : Tuple2<spectra::Point2, T>(x, y) {}

        template <typename U>
        SPECTRA_CPU_GPU explicit Point2(Point2<U> v) : Tuple2<spectra::Point2, T>(T(v.x), T(v.y)) {}

        template <typename U>
        SPECTRA_CPU_GPU explicit Point2(Vector2<U> v) : Tuple2<spectra::Point2, T>(T(v.x), T(v.y)) {}

        template <typename U>
        SPECTRA_CPU_GPU auto operator+(Vector2<U> v) const -> Point2<decltype(T{} + U{})> {
            DCHECK(!v.HasNaN());
            return {x + v.x, y + v.y};
        }

        template <typename U>
        SPECTRA_CPU_GPU Point2<T>& operator+=(Vector2<U> v) {
            DCHECK(!v.HasNaN());
            x += v.x;
            y += v.y;
            return *this;
        ...
        template <typename U>
        SPECTRA_CPU_GPU auto operator-(Point2<U> p) const -> Vector2<decltype(T{} - U{})> {
            DCHECK(!p.HasNaN());
            return {x - p.x, y - p.y};
        }
    ...
    public:
        ...
        Point3() = default;
        SPECTRA_CPU_GPU
        Point3(T x, T y, T z) : Tuple3<spectra::Point3, T>(x, y, z) {}
        ...
        template <typename U>
        SPECTRA_CPU_GPU explicit Point3(Point3<U> p) : Tuple3<spectra::Point3, T>(T(p.x), T(p.y), T(p.z)) {}

        template <typename U>
        SPECTRA_CPU_GPU explicit Point3(Vector3<U> v) : Tuple3<spectra::Point3, T>(T(v.x), T(v.y), T(v.z)) {}

        template <typename U>
        SPECTRA_CPU_GPU auto operator+(Vector3<U> v) const -> Point3<decltype(T{} + U{})> {
            DCHECK(!v.HasNaN());
            return {x + v.x, y + v.y, z + v.z};
        }

        template <typename U>
        SPECTRA_CPU_GPU Point3<T>& operator+=(Vector3<U> v) {
            DCHECK(!v.HasNaN());
            x += v.x;
            y += v.y;
            z += v.z;
        ...
        template <typename U>
        SPECTRA_CPU_GPU auto operator-(Point3<U> p) const -> Vector3<decltype(T{} - U{})> {
            DCHECK(!p.HasNaN());
    ...
    SPECTRA_CPU_GPU inline Point2f InvertBilinear(Point2f p, pstd::span<const Point2f> v);

    // https://www.iquilezles.org/www/articles/ibilinear/ibilinear.htm,
    // with a fix for perfect quads
    SPECTRA_CPU_GPU inline Point2f InvertBilinear(Point2f p, pstd::span<const Point2f> vert) {
    ...
    public:
        ...
        SPECTRA_CPU_GPU
        Point3fi(Point3f p, Vector3f e) : Point3<Interval>(Interval::FromValueAndError(p.x, e.x), Interval::FromValueAndError(p.y, e.y), Interval::FromValueAndError(p.z, e.z)) {}
        ...
        template <typename U>
        SPECTRA_CPU_GPU Point3fi operator+(Vector3<U> v) const {
            DCHECK(!v.HasNaN());
            return {x + v.x, y + v.y, z + v.z};
        }

        template <typename U>
        SPECTRA_CPU_GPU Point3fi& operator+=(Vector3<U> v) {
            DCHECK(!v.HasNaN());
            x += v.x;
            y += v.y;
            z += v.z;
        ...
        template <typename U>
        SPECTRA_CPU_GPU Point3fi operator-(Point3<U> p) const {
            DCHECK(!p.HasNaN());
            return {x - p.x, y - p.y, z - p.z};
        }
    ...
    public:
        ...
        Normal3() = default;
        SPECTRA_CPU_GPU
        Normal3(T x, T y, T z) : Tuple3<spectra::Normal3, T>(x, y, z) {}

        template <typename U>
        SPECTRA_CPU_GPU explicit Normal3(Normal3<U> v) : Tuple3<spectra::Normal3, T>(T(v.x), T(v.y), T(v.z)) {}

        template <typename U>
        SPECTRA_CPU_GPU explicit Normal3(Vector3<U> v) : Tuple3<spectra::Normal3, T>(T(v.x), T(v.y), T(v.z)) {}
    };
    ...
    template <typename T>
    template <typename U>
    SPECTRA_CPU_GPU Vector3<T>::Vector3(Normal3<U> n) : Tuple3<spectra::Vector3, T>(T(n.x), T(n.y), T(n.z)) {}
    ...
    SPECTRA_CPU_GPU inline Float Dot(Quaternion q1, Quaternion q2) {
        return Dot(q1.v, q2.v) + q1.w * q2.w;
    }

    SPECTRA_CPU_GPU inline Float Length(Quaternion q) {
        return std::sqrt(Dot(q, q));
    }

    SPECTRA_CPU_GPU inline Quaternion Normalize(Quaternion q) {
        DCHECK_GT(Length(q), 0);
        return q / Length(q);
    }

    SPECTRA_CPU_GPU inline Float AngleBetween(Quaternion q1, Quaternion q2) {
        if (Dot(q1, q2) < 0)
    ...
    SPECTRA_CPU_GPU inline Quaternion Slerp(Float t, Quaternion q1, Quaternion q2) {
    ...
    public:
        ...
        template <typename U>
        SPECTRA_CPU_GPU explicit Bounds2(const Bounds2<U>& b) {
            if (b.IsEmpty())
    ...
    public:
        ...
        template <typename U>
        SPECTRA_CPU_GPU explicit Bounds3(const Bounds3<U>& b) {
            if (b.IsEmpty())
                // Be careful about overflowing float->int conversions and the
                // like.
                *this = Bounds3<T>();
    ...
    SPECTRA_CPU_GPU inline Float SphericalTriangleArea(Vector3f a, Vector3f b, Vector3f c) {
        return std::abs(2 * std::atan2(Dot(a, Cross(b, c)), 1 + Dot(a, b) + Dot(a, c) + Dot(b, c)));
    }

    SPECTRA_CPU_GPU inline Float SphericalQuadArea(Vector3f a, Vector3f b, Vector3f c, Vector3f d);
    ...
    SPECTRA_CPU_GPU inline Vector3f SphericalDirection(Float sinTheta, Float cosTheta, Float phi) {
    ...
    SPECTRA_CPU_GPU inline Float SphericalTheta(Vector3f v) {
        return SafeACos(v.z);
    }

    SPECTRA_CPU_GPU inline Float SphericalPhi(Vector3f v) {
        Float p = std::atan2(v.y, v.x);
        return (p < 0) ? (p + 2 * Pi) : p;
    }

    SPECTRA_CPU_GPU inline Float CosTheta(Vector3f w) {
        return w.z;
    }

    SPECTRA_CPU_GPU inline Float Cos2Theta(Vector3f w) {
        return Sqr(w.z);
    }

    SPECTRA_CPU_GPU inline Float AbsCosTheta(Vector3f w) {
        return std::abs(w.z);
    }

    SPECTRA_CPU_GPU inline Float Sin2Theta(Vector3f w) {
        return std::max<Float>(0, 1 - Cos2Theta(w));
    }

    SPECTRA_CPU_GPU inline Float SinTheta(Vector3f w) {
        return std::sqrt(Sin2Theta(w));
    }

    SPECTRA_CPU_GPU inline Float TanTheta(Vector3f w) {
        return SinTheta(w) / CosTheta(w);
    }

    SPECTRA_CPU_GPU inline Float Tan2Theta(Vector3f w) {
        return Sin2Theta(w) / Cos2Theta(w);
    }

    SPECTRA_CPU_GPU inline Float CosPhi(Vector3f w) {
        Float sinTheta = SinTheta(w);
        return (sinTheta == 0) ? 1 : Clamp(w.x / sinTheta, -1, 1);
    }

    SPECTRA_CPU_GPU inline Float SinPhi(Vector3f w) {
        Float sinTheta = SinTheta(w);
        return (sinTheta == 0) ? 0 : Clamp(w.y / sinTheta, -1, 1);
    }

    SPECTRA_CPU_GPU inline Float CosDPhi(Vector3f wa, Vector3f wb) {
    ...
    SPECTRA_CPU_GPU inline bool SameHemisphere(Vector3f w, Vector3f wp) {
        return w.z * wp.z > 0;
    }
    ...
    SPECTRA_CPU_GPU inline bool Inside(const DirectionCone& d, Vector3f w) {
        return !d.IsEmpty() && Dot(d.w, Normalize(w)) >= d.cosTheta;
    }

    SPECTRA_CPU_GPU inline DirectionCone BoundSubtendedDirections(const Bounds3f& b, Point3f p) {
    ...
    SPECTRA_CPU_GPU inline Frame::Frame(Vector3f x, Vector3f y, Vector3f z) : x(x), y(y), z(z) {
