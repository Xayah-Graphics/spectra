...
namespace spectra
{
    ...
    public:
        ...
        template <typename U>
        SPECTRA_CPU_GPU explicit Vector2(Vector2<U> v)
            : Tuple2<spectra::Vector2, T>(T(v.x), T(v.y))
        {
        }
    };
    ...
    public:
        ...
        template <typename U>
        SPECTRA_CPU_GPU explicit Vector3(Vector3<U> v)
            : Tuple3<spectra::Vector3, T>(T(v.x), T(v.y), T(v.z))
        {
        }
    ...
    public:
        ...
        template <typename T>
        SPECTRA_CPU_GPU explicit Vector3fi(Point3<T> p)
            : Vector3<Interval>(Interval(p.x), Interval(p.y), Interval(p.z))
        {
        }
    ...
    public:
        ...
        template <typename U>
        SPECTRA_CPU_GPU explicit Point2(Vector2<U> v)
            : Tuple2<spectra::Point2, T>(T(v.x), T(v.y))
        {
        }
    ...
    public:
        ...
        template <typename U>
        SPECTRA_CPU_GPU explicit Point3(Point3<U> p)
            : Tuple3<spectra::Point3, T>(T(p.x), T(p.y), T(p.z))
        {
        }

        template <typename U>
        SPECTRA_CPU_GPU explicit Point3(Vector3<U> v)
            : Tuple3<spectra::Point3, T>(T(v.x), T(v.y), T(v.z))
        {
        }
    ...
    public:
        ...
        template <typename U>
        SPECTRA_CPU_GPU explicit Normal3(Normal3<U> v)
            : Tuple3<spectra::Normal3, T>(T(v.x), T(v.y), T(v.z))
        {
        }

        template <typename U>
        SPECTRA_CPU_GPU explicit Normal3(Vector3<U> v)
            : Tuple3<spectra::Normal3, T>(T(v.x), T(v.y), T(v.z))
        {
        }
    };
    ...
    template <typename T>
    SPECTRA_CPU_GPU inline auto Dot(Vector2<T> v1, Vector2<T> v2) ->
    ...
    template <typename T>
    SPECTRA_CPU_GPU inline auto AbsDot(Vector2<T> v1, Vector2<T> v2) ->
        typename TupleLength<T>::type
    {
        DCHECK(!v1.HasNaN() && !v2.HasNaN());
    ...
    template <typename T>
    SPECTRA_CPU_GPU inline auto Distance(Point2<T> p1, Point2<T> p2) ->
        typename TupleLength<T>::type
    {
        return Length(p1 - p2);
    }
    ...
    template <typename T>
    SPECTRA_CPU_GPU inline auto DistanceSquared(Point2<T> p1, Point2<T> p2) ->
        typename TupleLength<T>::type
    {
    ...
    template <typename T>
    SPECTRA_CPU_GPU inline auto Dot(Normal3<T> n, Vector3<T> v) ->
        typename TupleLength<T>::type
    {
    ...
    template <typename T>
    SPECTRA_CPU_GPU inline auto Dot(Vector3<T> v, Normal3<T> n) ->
        typename TupleLength<T>::type
    {
    ...
    template <typename T>
    SPECTRA_CPU_GPU inline auto Dot(Normal3<T> n1, Normal3<T> n2) ->
        typename TupleLength<T>::type
    {
    ...
    template <typename T>
    SPECTRA_CPU_GPU inline auto AbsDot(Normal3<T> n, Vector3<T> v) ->
        typename TupleLength<T>::type
    {
        DCHECK(!n.HasNaN() && !v.HasNaN());
    ...
    template <typename T>
    SPECTRA_CPU_GPU inline auto AbsDot(Vector3<T> v, Normal3<T> n) ->
        typename TupleLength<T>::type
    {
        using std::abs;
        DCHECK(!v.HasNaN() && !n.HasNaN());
    ...
    template <typename T>
    SPECTRA_CPU_GPU inline auto AbsDot(Normal3<T> n1, Normal3<T> n2) ->
        typename TupleLength<T>::type
    {
        using std::abs;
        DCHECK(!n1.HasNaN() && !n2.HasNaN());
    ...
    public:
        ...
        {
            return Point2<T>(spectra::Lerp(t.x, pMin.x, pMax.x),
                             spectra::Lerp(t.y, pMin.y, pMax.y));
        }
    ...
    public:
        ...
        {
            return Point3f(spectra::Lerp(t.x, pMin.x, pMax.x), spectra::Lerp(t.y, pMin.y, pMax.y),
                           spectra::Lerp(t.z, pMin.z, pMax.z));
        }
