...
namespace spectra {
    ...
    inline SPECTRA_CPU_GPU void EnqueueWorkAfterMiss(RayWorkItem r, MediumSampleQueue* mediumSampleQueue, EscapedRayQueue* escapedRayQueue) {
        if (r.ray.medium) {
    ...
    inline SPECTRA_CPU_GPU void RecordShadowRayResult(const ShadowRayWorkItem w, SOA<PixelSampleState>* pixelSampleState, bool foundIntersection) {
    ...
    inline SPECTRA_CPU_GPU void EnqueueWorkAfterIntersection(RayWorkItem r, Medium rayMedium, float tMax, SurfaceInteraction intr, MediumSampleQueue* mediumSampleQueue, RayQueue* nextRayQueue, HitAreaLightQueue* hitAreaLightQueue, MaterialEvalQueue* basicEvalMaterialQueue, MaterialEvalQueue* universalEvalMaterialQueue) {
        ...
        if (rayMedium) {
            ...
            mediumSampleQueue->Push(MediumSampleWorkItem{r.ray, r.depth, tMax, r.lambda, r.beta, r.r_u, r.r_l, r.pixelIndex, r.prevIntrCtx, r.specularBounce, r.anyNonSpecularBounces, r.etaScale, intr.areaLight, intr.pi, intr.n, intr.dpdu, intr.dpdv, -r.ray.d, intr.uv, intr.material, intr.shading.n, intr.shading.dpdu, intr.shading.dpdv, intr.shading.dndu, intr.shading.dndv, intr.faceIndex, mediumInterface});
            return;
        }
    ...
    inline SPECTRA_CPU_GPU void TraceTransmittance(ShadowRayWorkItem sr, SOA<PixelSampleState>* pixelSampleState, T trace, S spawnTo) {
        ...
        while (ray.d != Vector3f(0, 0, 0)) {
            ...
            if (ray.medium) {
                Float tEnd            = !result.hit ? tMax : (Distance(ray.o, Point3f(result.pHit)) / Length(ray.d));
                SampledSpectrum T_maj = SampleT_maj(ray, tEnd, rng.Uniform<Float>(), rng, lambda, [&](Point3f p, MediumProperties mp, SampledSpectrum sigma_maj, SampledSpectrum T_maj) {
