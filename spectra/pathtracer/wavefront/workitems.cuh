...
namespace spectra {
    ...
    public:
        ...
        struct GetSetIndirector {
            __host__ __device__ operator RaySamples() const {
                return (*(const SOA*) soa)[index];
            }

            __host__ __device__ void operator=(RaySamples rs) {
    ...
    struct GetBSSRDFAndProbeRayWorkItem {
        __host__ __device__ MaterialEvalContext GetMaterialEvalContext() const {
    ...
    struct MaterialEvalWorkItem {
        ...
        __host__ __device__ NormalBumpEvalContext GetNormalBumpEvalContext(Float dudx, Float dudy, Float dvdx, Float dvdy) const {
        ...
        __host__ __device__ MaterialEvalContext GetMaterialEvalContext(Float dudx, Float dudy, Float dvdx, Float dvdy, Normal3f ns, Vector3f dpdus) const {
            MaterialEvalContext ctx;
            ctx.wo        = wo;
            ctx.n         = n;
            ctx.ns        = ns;
...
#include "spectra_wavefront_workitems_soa.cuh"
