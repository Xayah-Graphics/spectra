...
namespace {
    ...
    [[nodiscard]] std::string generate_abi_module(slang::ISession& session, const std::vector<std::array<std::string, 2>>& type_entries) {
        ...
        for (const std::array<std::string, 2>& entry : type_entries) {
            ...
            Slang::ComPtr<slang::IBlob> diagnostic_blob{};
            slang::ProgramLayout* program_layout = module->getLayout(0, diagnostic_blob.writeRef());
            if (!program_layout) throw std::runtime_error(std::format("Reflecting Slang module '{}' failed\n{}", entry[0], diagnostic_text(diagnostic_blob)));
            slang::TypeReflection* type = program_layout->findTypeByName(entry[1].c_str());
            if (!type) throw std::runtime_error(std::format("Slang type '{}.{}' is not reflected", entry[0], entry[1]));
            slang::TypeLayoutReflection* layout = program_layout->getTypeLayout(type, slang::LayoutRules::DefaultStructuredBuffer);
