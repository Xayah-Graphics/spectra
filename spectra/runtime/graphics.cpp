...
namespace spectra {
    ...
    VulkanGraphics::VulkanGraphics(WindowPlatform& platform, const std::string_view application_name) : context{platform} {
        this->create_instance(application_name);
        this->select_physical_device();
        this->create_device();
    }

    void VulkanGraphics::create_instance(const std::string_view application_name) {
        ...
        constexpr std::array instance_extensions{vk::KHRSurfaceExtensionName, vk::KHRWin32SurfaceExtensionName};
        ...
        this->instance.surface  = vk::raii::SurfaceKHR{this->instance.instance, vk::Win32SurfaceCreateInfoKHR{{}, GetModuleHandleW(nullptr), this->context.platform.native_window}};
    }
    ...
    void VulkanGraphics::select_physical_device() {
        ...
        for (const vk::raii::PhysicalDevice& candidate : this->instance.instance.enumeratePhysicalDevices()) {
            ...
            if (!std::ranges::all_of(required_extensions, [&available_extensions](const char* required) { return std::ranges::contains(available_extensions, std::string_view{required}, [](const vk::ExtensionProperties& extension) { return std::string_view{extension.extensionName.data()}; }); })) continue;
            ...
            for (std::uint32_t index = 0; index < queue_families.size(); ++index) {
                ...
                if (!candidate.getSurfaceSupportKHR(index, *this->instance.surface)) continue;
    ...
    void VulkanGraphics::create_device() {
        ...
        this->device = vk::raii::Device{this->physical_device, vk::DeviceCreateInfo{{}, 1, &queue_create_info, 0, nullptr, static_cast<std::uint32_t>(required_extensions.size()), required_extensions.data(), nullptr, &enabled_features.get<vk::PhysicalDeviceFeatures2>()}};
