...
namespace spectra {
    ...
    struct GpuAllocator {
        ...
        [[nodiscard]] vk::DeviceMemory memory(const GpuAllocation& allocation) const noexcept {
            return *this->blocks[allocation.block_index]->memory;
        }
        ...
        [[nodiscard]] void* mapped(const GpuAllocation& allocation) const noexcept {
            if (!this->blocks[allocation.block_index]->mapped) return nullptr;
            return static_cast<std::byte*>(this->blocks[allocation.block_index]->mapped) + allocation.offset;
        }
    ...
    private:
        ...
        [[nodiscard]] GpuAllocation make_allocation(const std::uint32_t block_index, const vk::DeviceSize offset, const vk::DeviceSize size) noexcept {
    ...
    GpuAllocation::~GpuAllocation() {
        if (this->allocator) this->allocator->release(this->block_index, this->offset, this->size);
    }

    GpuAllocation::GpuAllocation(GpuAllocation&& other) noexcept : allocator(std::exchange(other.allocator, nullptr)), block_index(std::exchange(other.block_index, 0)), offset(std::exchange(other.offset, 0)), size(std::exchange(other.size, 0)) {}
    ...
    GpuResources::GpuResources(VulkanGraphics& graphics) : context{graphics}, allocator(std::make_unique<GpuAllocator>(graphics.physical_device, graphics.device)) {
    ...
    void* GpuResources::export_buffer_memory_handle(const GpuBuffer& buffer) const {
        if (!*buffer.external_memory) throw std::runtime_error("Spectra cannot export a non-external GPU buffer");
        return this->context.graphics.device.getMemoryWin32HandleKHR(vk::MemoryGetWin32HandleInfoKHR{*buffer.external_memory, vk::ExternalMemoryHandleTypeFlagBits::eOpaqueWin32});
    }
    ...
    void* GpuResources::export_timeline_semaphore_handle(const GpuExternalTimelineSemaphore& timeline) const {
        return this->context.graphics.device.getSemaphoreWin32HandleKHR(vk::SemaphoreGetWin32HandleInfoKHR{*timeline.semaphore, vk::ExternalSemaphoreHandleTypeFlagBits::eOpaqueWin32});
    }
