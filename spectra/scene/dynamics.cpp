...
namespace spectra {
    namespace {
        ...
        struct Win32Handle {
            ...
            Win32Handle() = default;
            explicit Win32Handle(const HANDLE handle) : handle(handle) {}
            Win32Handle(const Win32Handle&) = delete;
            Win32Handle(Win32Handle&& other) noexcept : handle(std::exchange(other.handle, nullptr)) {}
    ...
    DynamicWorld::ProviderLibrary::ProviderLibrary(const std::filesystem::path& library_path, const std::string_view expected_provider_id) : library_path(std::filesystem::weakly_canonical(library_path)) {
        ...
        if (!loaded_api || loaded_api->api_version != SPECTRA_PLUGIN_API_VERSION || loaded_api->struct_size != sizeof(SpectraPluginApi) || !loaded_api->describe_provider || !loaded_api->create_provider || !loaded_api->destroy_provider || !loaded_api->configure_port || !loaded_api->set_input_frame || !loaded_api->apply_parameters || !loaded_api->reset || !loaded_api->step || !loaded_api->publish_frame) {
            FreeLibrary(loaded);
            throw std::runtime_error(std::format("Provider Library has an incomplete Plugin API 11 entry: {}", this->library_path.string()));
        }
    ...
    DynamicWorld::ProviderLibrary::~ProviderLibrary() {
        if (this->library_handle) FreeLibrary(static_cast<HMODULE>(this->library_handle));
    }
    ...
    void DynamicWorld::collect_debug(void* context, const std::uint64_t port_index, const SpectraPluginDebugPrimitive* primitives, const std::uint64_t primitive_count) {
        ...
        for (std::uint64_t index = 0; index < primitive_count; ++index) {
            ...
            world.publication.publishing_frame->debug_primitives.emplace_back(world.configuration.setup.systems[system.scene_system_index].id, static_cast<dynamics::DebugPrimitiveKind>(source.kind), static_cast<dynamics::DebugDepthMode>(source.depth_mode), first_position, second_position, math::Float3{source.color.x, source.color.y, source.color.z}, source.radius * radius_scale, source.source_id, pick);
        }
    }
    ...
    void DynamicWorld::initialize(const std::filesystem::path& scene_path, const scene::Scene& source_scene) {
        ...
        for (const std::string& required_provider : required_providers) {
            ...
            dynamics::ProviderDescriptor provider{
                .id                = plugin_string(source_provider.id),
                .name              = plugin_string(source_provider.name),
                .interface_id      = plugin_string(source_provider.interface_id),
            ...
            provider.telemetry_descriptors.reserve(source_provider.telemetry_count);
            for (std::uint64_t telemetry_index = 0; telemetry_index < source_provider.telemetry_count; ++telemetry_index) {
                const SpectraPluginTelemetryDescriptor& telemetry = source_provider.telemetry_descriptors[telemetry_index];
                provider.telemetry_descriptors.emplace_back(plugin_string(telemetry.id), plugin_string(telemetry.name), plugin_string(telemetry.unit));
            }
            if (source_provider.telemetry_count != 0 && !library.plugin_api->read_telemetry) throw std::runtime_error(std::format("Provider '{}' declares telemetry without a telemetry callback", provider.id));
        ...
        this->systems.statuses.reserve(this->configuration.setup.systems.size());
        for (std::size_t system_index = 0; system_index < this->configuration.setup.systems.size(); ++system_index) {
            const scene::DynamicSystem& declared         = this->configuration.setup.systems[system_index];
            const dynamics::ProviderDescriptor& provider = this->provider_descriptor(declared.provider_id);
            dynamics::SystemStatus status{.system_id = declared.id, .name = declared.name, .provider_name = provider.name, .enabled = declared.enabled, .visible = declared.visible};
            status.parameters.reserve(provider.parameters.size());
            for (const dynamics::ParameterDescriptor& descriptor : provider.parameters) {
                const auto configured                    = std::ranges::find(declared.parameters, descriptor.id, &scene::DynamicParameterSetting::parameter_id);
                const scene::DynamicParameterValue value = configured == declared.parameters.end() ? descriptor.value : configured->value;
                status.parameters.emplace_back(descriptor.id, descriptor.name, descriptor.unit, descriptor.application_mode, value, descriptor.minimum, descriptor.maximum, descriptor.enumerators);
            }
            for (const dynamics::TelemetryValue& descriptor : provider.telemetry_descriptors) status.telemetry_values.emplace_back(descriptor.id, descriptor.name, descriptor.unit, 0.0);
            this->systems.statuses.emplace_back(std::move(status));
            ...
            for (const dynamics::ParameterDescriptor& parameter : this->systems.statuses[system_index].parameters) system.parameter_values.emplace_back(parameter.value);
    ...
    void DynamicWorld::configure_port(DynamicSystemRuntime& system, const std::size_t port_index) {
        ...
        std::vector<Win32Handle> exported_buffer_handles{};
        for (std::uint32_t slot_index = 0; slot_index < output_slot_count; ++slot_index) {
            ...
            for (DynamicPortBuffer& port_buffer : binding.buffer_slots[slot_index]) {
                void* exported_memory_handle{};
                if (binding.descriptor.memory_domain == dynamics::MemoryDomain::CudaExternal) {
                    exported_buffer_handles.emplace_back(static_cast<HANDLE>(this->context.runtime.resources.export_buffer_memory_handle(port_buffer.gpu_buffer)));
                    exported_memory_handle = exported_buffer_handles.back().handle;
                }
        ...
        Win32Handle exported_timeline_handle{};
        if (binding.descriptor.memory_domain == dynamics::MemoryDomain::CudaExternal) exported_timeline_handle = Win32Handle{static_cast<HANDLE>(this->context.runtime.resources.export_timeline_semaphore_handle(binding.timeline_semaphore))};
        const GpuDeviceIdentity gpu_identity = this->context.runtime.graphics.identity;
        SpectraPluginPortConfiguration configuration{binding.port_index, static_cast<SpectraPluginPortDirection>(binding.descriptor.direction), static_cast<SpectraPluginMemoryDomain>(binding.descriptor.memory_domain), plugin_slots.data(), plugin_slots.size(), exported_timeline_handle.handle, {}, {}, gpu_identity.node_mask};
    ...
    void DynamicWorld::publish_frame(const std::uint64_t simulation_step) {
        ...
        dynamics::DynamicFrame next{.publication_sequence = ++this->publication.sequence, .simulation_step = simulation_step};
        for (DynamicSystemRuntime& system : this->systems.runtimes) {
            for (DynamicPortRuntime& binding : system.ports)
            ...
            for (std::size_t telemetry_index = 0; telemetry_index < this->systems.statuses[system.scene_system_index].telemetry_values.size(); ++telemetry_index) this->systems.statuses[system.scene_system_index].telemetry_values[telemetry_index].value = system.plugin_api->read_telemetry(system.provider_instance, telemetry_index);
        }
        this->publication.publishing_system = nullptr;
        this->publication.publishing_frame  = nullptr;
    ...
    void DynamicWorld::step_to(const std::uint64_t target_step) {
        ...
        for (DynamicSystemRuntime& system : this->systems.runtimes) {
            ...
            system.completed_steps += step_count;
            system.total_step_milliseconds += elapsed;
            dynamics::SystemStatus& status   = this->systems.statuses[system.scene_system_index];
            status.completed_steps           = system.completed_steps;
            status.simulation_seconds        = static_cast<double>(system.completed_steps) * this->configuration.setup.clock.step_seconds;
            status.last_batch_milliseconds   = elapsed;
            status.average_step_milliseconds = system.total_step_milliseconds / static_cast<double>(system.completed_steps);
        }
    ...
    void DynamicWorld::reset_systems() {
        for (DynamicSystemRuntime& system : this->systems.runtimes) {
            ...
            status.simulation_seconds        = 0.0;
            status.last_batch_milliseconds   = 0.0;
            status.average_step_milliseconds = 0.0;
        }
        this->clock.simulation_step = 0;
    ...
    void DynamicWorld::apply_parameter_changes(const std::size_t system_index, const bool reset) {
        ...
        for (const dynamics::ParameterDescriptor& parameter : this->systems.statuses[system_index].parameters) destination.parameter_values.emplace_back(parameter.value);
...
namespace spectra {
    ...
    dynamics::SimulationTimeline DynamicWorld::timeline() const noexcept {
        return {this->clock.simulation_step, this->publication.frame.simulation_seconds, this->configuration.setup.clock.step_seconds};
    }
    ...
    std::vector<dynamics::SystemStatus>& DynamicWorld::system_statuses() noexcept {
        return this->systems.statuses;
    }

    std::span<const dynamics::ProviderDescriptor> DynamicWorld::available_providers() const noexcept {
        return this->providers.descriptors;
    }

    const std::optional<scene::DynamicSetup>& DynamicWorld::setup() const noexcept {
        return this->context.document.content.source.dynamic_setup;
    }
