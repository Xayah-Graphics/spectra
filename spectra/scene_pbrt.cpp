...
namespace xayah::scene {
    namespace {
        ...
        void RefreshCatalogCounts(PbrtSceneCatalog* catalog) {
            ...
            for (const PbrtSceneCatalogEntry& entry : catalog->entries) {
                ...
                case PbrtSceneCatalogEntryState::Ready: ++catalog->ready_count; break;
    ...
    PbrtSceneCatalog DiscoverPbrtSceneCatalog() {
        ...
        for (const std::filesystem::directory_entry& entry : std::filesystem::recursive_directory_iterator(catalog.root)) {
            ...
            PbrtSceneCatalogEntry catalogEntry{
                ...
                .state       = scanError.has_value() ? PbrtSceneCatalogEntryState::Invalid : PbrtSceneCatalogEntryState::Pending,
            };
    ...
    void ValidatePbrtSceneCatalogEntry(PbrtSceneCatalogEntry& entry) {
        ValidatePbrtSceneCatalogEntry(entry, std::stop_token{});
    }
    ...
    void ValidatePbrtSceneCatalogEntry(PbrtSceneCatalogEntry& entry, const std::stop_token stopToken) {
        entry.state = PbrtSceneCatalogEntryState::Pending;
        entry.revision = SceneRevision{};
        entry.info.reset();
        entry.issues.clear();
        try {
            SceneSnapshot scene = ParsePbrtSceneCatalogEntry(entry, stopToken);
            entry.revision     = scene.revision;
            entry.info         = DescribeScene(scene);
            entry.state        = PbrtSceneCatalogEntryState::Ready;
