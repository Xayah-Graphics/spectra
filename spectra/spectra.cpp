...
namespace spectra {
    namespace {
        ...
        [[nodiscard]] constexpr std::array<const char*, 15> required_device_extensions() noexcept {
            return {
    ...
    struct GpuAllocator {
        ...
        [[nodiscard]] void* mapped(const GpuAllocation& allocation) const noexcept {
            ...
            return static_cast<std::byte*>(this->blocks[allocation.block]->mapped) + allocation.offset;
        }
    ...
    private:
        ...
        [[nodiscard]] std::uint32_t find_memory_type(const std::uint32_t type_bits, const vk::MemoryPropertyFlags properties) const {
            const vk::PhysicalDeviceMemoryProperties memory_properties = this->physical_device->getMemoryProperties();
        ...
        [[nodiscard]] std::optional<vk::DeviceSize> allocate_range(Block& block, const vk::DeviceSize size, const vk::DeviceSize alignment) {
            for (std::size_t index = 0; index < block.free_ranges.size(); ++index) {
                ...
                if (offset + size < range.offset + range.size)
                    block.free_ranges.insert(block.free_ranges.begin() + index, Range{
                                                                                    offset + size,
        ...
        [[nodiscard]] GpuAllocation make_allocation(const std::uint32_t block, const vk::DeviceSize offset, const vk::DeviceSize size) noexcept {
    ...
    GpuAllocation::~GpuAllocation() {
        if (this->allocator) this->allocator->release(this->block, this->offset, this->size);
    }

    GpuAllocation::GpuAllocation(GpuAllocation&& other) noexcept : allocator(std::exchange(other.allocator, nullptr)), block(std::exchange(other.block, 0)), offset(std::exchange(other.offset, 0)), size(std::exchange(other.size, 0)) {}
    ...
    GpuBuffer::GpuBuffer(GpuBuffer&& other) noexcept : allocation(std::move(other.allocation)), external_memory(std::move(other.external_memory)), buffer(std::move(other.buffer)), address(std::exchange(other.address, 0)), size(std::exchange(other.size, 0)), mapped(std::exchange(other.mapped, nullptr)) {}
    ...
    GpuImage::GpuImage(GpuImage&& other) noexcept : allocation(std::move(other.allocation)), image(std::move(other.image)), view(std::move(other.view)), extent(std::exchange(other.extent, {})), format(std::exchange(other.format, {})), mip_levels(std::exchange(other.mip_levels, 1)) {}
    ...
    void Spectra::create_window(const std::string_view application_name, const vk::Extent2D initial_extent) {
        ...
        this->platform.window.reset(glfwCreateWindow(static_cast<int>(initial_extent.width), static_cast<int>(initial_extent.height), std::string{application_name}.c_str(), nullptr, nullptr));
    ...
    void Spectra::select_physical_device() {
        ...
        for (const vk::raii::PhysicalDevice& candidate : this->context.instance.enumeratePhysicalDevices()) {
            ...
            const bool extensions_available                                 = std::ranges::all_of(required_extensions, [&available_extensions](const char* required) { return std::ranges::contains(available_extensions, std::string_view{required}, [](const vk::ExtensionProperties& extension) { return std::string_view{extension.extensionName.data()}; }); });
            ...
            this->graphics.physical_device    = candidate;
            this->graphics.queue_family_index = graphics_family_index;
            break;
        }
    ...
    void Spectra::create_device() {
        ...
        const vk::PhysicalDeviceIDProperties& identity = properties.get<vk::PhysicalDeviceIDProperties>();
        std::ranges::copy(identity.deviceUUID, this->graphics.identity.uuid.begin());
        std::ranges::copy(identity.deviceLUID, this->graphics.identity.luid.begin());
    ...
    void Spectra::recreate_swapchain() {
        ...
        const vk::SurfaceCapabilitiesKHR capabilities       = this->graphics.physical_device.getSurfaceCapabilitiesKHR(*this->platform.surface);
        const std::vector<vk::SurfaceFormatKHR> formats     = this->graphics.physical_device.getSurfaceFormatsKHR(*this->platform.surface);
        const std::vector<vk::PresentModeKHR> present_modes = this->graphics.physical_device.getSurfacePresentModesKHR(*this->platform.surface);
    ...
    DescriptorHandle Spectra::allocate_resource_descriptor() {
        ...
        if (static_cast<vk::DeviceSize>(handle.index + 1u) * this->descriptors.resource_stride > this->descriptors.resource_heap.size - this->descriptors.properties.minResourceHeapReservedRange) throw std::runtime_error("Spectra resource descriptor heap is exhausted");
        return handle;
    }

    DescriptorHandle Spectra::allocate_sampler_descriptor() {
        ...
        if (static_cast<vk::DeviceSize>(handle.index + 1u) * this->descriptors.sampler_stride > this->descriptors.sampler_heap.size - this->descriptors.properties.minSamplerHeapReservedRange) throw std::runtime_error("Spectra sampler descriptor heap is exhausted");
        return handle;
    }

    void Spectra::release_resource_descriptor(const DescriptorHandle handle) noexcept {
        this->deferred.resource_indices[this->frames.index].push_back(handle.index);
    }

    void Spectra::release_sampler_descriptor(const DescriptorHandle handle) noexcept {
    ...
    void Spectra::push_data(const vk::raii::CommandBuffer& command_buffer, const std::span<const std::byte> data, const std::uint32_t offset) const noexcept {
